...
> {
  ...
  render() {
    ...
    return (
      <main className="grid h-screen place-items-center bg-[#12141a] p-8 text-[#ffb4b4]">
        <div className="grid max-w-[900px] gap-3">
          <pre className="whitespace-pre-wrap rounded-xl border border-[#3b2a2a] bg-[#1a0f10] p-5 text-xs">
            {this.state.error.stack ?? this.state.error.message}
          </pre>
          <button
            className="justify-self-start rounded-lg border border-[#5a3434] bg-[#251416] px-3 py-2 text-xs font-bold text-[#ffd6d6]"
            onClick={this.reset}
          >
            Retry render
          </button>
...
ReactDOM.createRoot(document.getElementById("root")!).render(
  <React.StrictMode>
    <ErrorBoundary>
      {new URLSearchParams(window.location.search).has("clipperExport") ? (
        <RenderedMediaExportApp />
      ) : (
        <App />
      )}
    </ErrorBoundary>
