...
import { configureMonacoTypeScriptLanguageService, cssImportDeclarationSource } from "./monacoLanguageService";
...
function createProgramDiagnostics(files: Record<string, string>) {
  ...
  const program = ts.createProgram(["/project/composition.ts", "/project/css-imports.d.ts"], options, host);
...
describe("monaco language service", () => {
  it("registers plain CSS import declarations for editor diagnostics", () => {
    ...
    expect(addExtraLib).toHaveBeenCalledWith(cssImportDeclarationSource, "file:///clipper/projects/css-imports.d.ts");
    expect(setDiagnosticsOptions).toHaveBeenCalledWith({ noSemanticValidation: false, noSyntaxValidation: false });
  });

  it("accepts relative CSS imports without hiding unrelated missing imports", () => {
    const diagnostics = createProgramDiagnostics({
      "/project/composition.ts": `
        ...
        missing;
      `,
      "/project/css-imports.d.ts": cssImportDeclarationSource,
    });
