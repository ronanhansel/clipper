...
function createProgramDiagnostics(files: Record<string, string>) {
  ...
  host.getSourceFile = (filePath, languageVersion) =>
    files[filePath] === undefined
      ? undefined
      : ts.createSourceFile(filePath, files[filePath], languageVersion, true);
  const program = ts.createProgram(
    ["/project/composition.ts", "/project/text-imports.d.ts"],
    options,
    host,
  );
  return ts
  ...
  it("accepts relative CSS and HTML imports without hiding unrelated missing imports", () => {
    ...
    expect(
      diagnostics.some((diagnostic) =>
        diagnostic.message.includes("light-warp-reflections.css"),
      ),
    ).toBe(false);
    expect(
      diagnostics.some((diagnostic) =>
        diagnostic.message.includes("cosmic-hair.html"),
      ),
    ).toBe(false);
