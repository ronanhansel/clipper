...
export type ExportWorkerResolutionMapping = {
  hd: number;
  qhd: number;
  uhd: number;
};
export type ExportTileResolutionMapping = {
  hd: number;
  qhd: number;
  uhd: number;
};
...
export type TimelineBlankContextTarget = {
  time: number;
  compositionLayerId?: string;
};
...
export type TimelineSelectionDrag = {
  startX: number;
  currentX: number;
  startY?: number;
  currentY?: number;
