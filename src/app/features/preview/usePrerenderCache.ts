...
import type { AdjustmentLayer, CompositionClip, ProjectManifest, Scene, TimelineLayerState, TransitionLayer } from "../../../core/types";
...
type PrerenderQueueItem = { start: number; durationMs: number; frameRange?: { startFrame: number; endFrame: number }; manualJobId?: number };
type ManualPrerenderJob = { errors: string[]; pending: number; queuedRanges: number; visibleRanges: number; resolve: (result: PrerenderCompositionResult) => void };
...
export type PrerenderCompositionResult = { queuedBlocks: number; visibleRanges: number; completed: boolean; error?: string };
export type PrerenderManualCompositionRange = { compositionId: string; start: number; end: number };
...
export function usePrerenderCache({ blockDurationMs, cacheResetToken, enabled, hasActiveComposition, isPlaying, manifestPath, project, scene, sceneDuration, sceneTime, tileHeight, timelineLayers, scheduleRanges = [] }: { blockDurationMs: number; cacheResetToken: number; enabled: boolean; hasActiveComposition: boolean; isPlaying: boolean; manifestPath: string; project: ProjectManifest; scene: Scene; sceneDuration: number; sceneTime: number; tileHeight: number; timelineLayers?: TimelineLayerState; scheduleRanges?: PrerenderManualCompositionRange[] }) {
  ...
  const latestRequestRef = useRef({ project, manifestPath, scene, sceneTime, sceneDuration, enabled, hasActiveComposition, isPlaying, tileHeight, blockDurationMs, timelineLayers, scheduleRanges });
  ...
  const fullCacheKey = JSON.stringify({ globalCacheKey, tileHeight, cacheResetToken });

  latestRequestRef.current = { project, manifestPath, scene, sceneTime, sceneDuration, enabled, hasActiveComposition, isPlaying, tileHeight, blockDurationMs, timelineLayers, scheduleRanges };

  useEffect(() => {
    const previousSignature = cacheSignatureRef.current;
    const fullReset = fullCacheKeyRef.current !== fullCacheKey;
    const signatureChanged = !previousSignature || JSON.stringify(previousSignature) !== JSON.stringify(cacheSignature);
    if (!fullReset && !signatureChanged) return;
    resolveManualJobs(manualJobsRef.current, "Prerender was interrupted by a project or cache change.");
    ...
    if (!previousSignature || fullReset) {
      ...
      queuedRef.current = queuedRef.current.filter((item) => !rangesOverlap(item.start, item.start + item.durationMs / 1000, invalidRanges));
  ...
  const requestCacheAtTime = useCallback((time: number, reason: PrerenderCacheInterestReason = "idle") => {
    ...
    const blockStarts = getPrerenderScheduleBlockStarts(time, request.sceneDuration, videoExportFrameRate, blockDurationSeconds, radiusBlocks, reason)
      .filter((blockStart) => request.scheduleRanges.length === 0 || blockIntersectsRanges(blockStart, blockStart + blockDurationSeconds, request.scheduleRanges));
    const nextQueue = queuedRef.current.filter((item) => shouldKeepQueuedBlock(item.start, time, request.sceneDuration, blockDurationSeconds));
    ...
    for (let index = blockStarts.length - 1; index >= 0; index -= 1) {
      ...
      if (!cachedBlockStartsRef.current.has(blockStart) && !renderingBlockStartsRef.current.has(blockStart) && !queueIncludesStart(nextQueue, blockStart)) {
        nextQueue.unshift({ start: blockStart, durationMs: request.blockDurationMs });
        queueChanged = true;
      }
    }
  ...
  const prerenderComposition = useCallback((compositionId: string): Promise<PrerenderCompositionResult> => {
    const request = latestRequestRef.current;
    if (!request.hasActiveComposition || request.sceneDuration <= 0) return Promise.resolve({ queuedBlocks: 0, visibleRanges: 0, completed: false });
    autoPrerenderPausedUntilRef.current = Date.now() + manualPrerenderAutoPauseMs;
    if (idleTimerRef.current !== null) window.clearTimeout(idleTimerRef.current);
    idleTimerRef.current = null;
    const ranges = getVisibleCompositionRanges(request.scene.compositions, compositionId, request.timelineLayers, request.sceneDuration);
    if (ranges.length === 0) return Promise.resolve({ queuedBlocks: 0, visibleRanges: 0, completed: false });
    const generation = generationRef.current;
    let queuedRanges = 0;
    const manualJobId = nextManualJobIdRef.current++;
    const nextQueue = [...queuedRef.current];
    for (const range of ranges) {
      const frameRange = getFrameRangeForTimeRange(range.start, range.end, request.sceneDuration, videoExportFrameRate);
      const rangeStart = frameRange.startFrame / videoExportFrameRate;
      if (isExactFrameRangeCached(cacheRef.current, frameRange) || renderingBlockStartsRef.current.has(rangeStart) || queueIncludesFrameRange(nextQueue, frameRange)) continue;
      nextQueue.push({ start: rangeStart, durationMs: (frameRange.endFrame - frameRange.startFrame) / videoExportFrameRate * 1000, frameRange, manualJobId });
      queuedRanges += 1;
    }
    if (queuedRanges === 0) return Promise.resolve({ queuedBlocks: 0, visibleRanges: ranges.length, completed: true });
    queuedRef.current = nextQueue;
    setState((current) => ({ ...current, version: current.version + 1 }));
    void processPrerenderQueue(generation);
    return new Promise((resolve) => {
      manualJobsRef.current.set(manualJobId, { errors: [], pending: queuedRanges, queuedRanges, visibleRanges: ranges.length, resolve });
    });
  }, [blockDurationSeconds]);
  ...
  async function processPrerenderQueue(generation: number) {
    ...
    while (generation === generationRef.current) {
      ...
      if (!nextItem.frameRange && cachedBlockStartsRef.current.has(nextBlockStart)) continue;
      renderingBlockStartsRef.current.add(nextBlockStart);

      setState((current) => ({ ...current, version: current.version + 1 }));
      const request = latestRequestRef.current;
      try {
        const frames = await withPrerenderTimeout(clipperHost.prerenderFrame(request.project, request.manifestPath, request.scene, nextBlockStart, request.sceneDuration, videoExportFrameRate, request.tileHeight, nextItem.durationMs, nextItem.frameRange), nextBlockStart);
        ...
        if (generation !== generationRef.current) break;
        finishManualQueueItem(nextItem, error instanceof Error ? error.message : String(error));
  ...
  function finishManualQueueItem(item: PrerenderQueueItem, error?: string) {
    ...
    job.resolve({ queuedBlocks: job.queuedRanges, visibleRanges: job.visibleRanges, completed: job.errors.length === 0, error: job.errors[0] });
  }
  ...
  function scheduleIdleExpansion(generation: number) {
    ...
    idleTimerRef.current = window.setTimeout(() => {
      ...
      for (const blockStart of nextBlocks) {
        if (cachedBlockStartsRef.current.has(blockStart) || renderingBlockStartsRef.current.has(blockStart) || queueIncludesStart(queuedRef.current, blockStart)) continue;
        queuedRef.current.push({ start: blockStart, durationMs: request.blockDurationMs });
        changed = true;
        break;
      }
      if (!changed) return;
...
function getVisibleCompositionRanges(compositions: CompositionClip[], compositionId: string, timelineLayers: TimelineLayerState | undefined, sceneDuration: number) {
...
function compositionMatchesId(composition: CompositionClip, compositionId: string) {
...
function blockIntersectsRanges(start: number, end: number, ranges: PrerenderManualCompositionRange[]) {
...
function getCoverageBlocks(cachedBlocks: Set<number>, renderingBlocks: Set<number>, queuedBlocks: PrerenderQueueItem[], blockDurationSeconds: number, sceneDuration: number): PrerenderCacheCoverage["blocks"] {
  const queuedByStart = new Map(queuedBlocks.map((item) => [item.start, item]));
  const starts = [...new Set([...cachedBlocks, ...renderingBlocks, ...queuedByStart.keys()])].sort((left, right) => left - right);
  return starts.map((start) => {
    const queuedItem = queuedByStart.get(start);
    return {
    start,
    duration: Math.min(queuedItem ? queuedItem.durationMs / 1000 : blockDurationSeconds, Math.max(sceneDuration - start, 0)),
...
function getFrameRangeForTimeRange(start: number, end: number, sceneDuration: number, frameRate: number) {
  const totalFrames = Math.max(1, Math.ceil(sceneDuration * frameRate));
  const startFrame = Math.min(Math.max(Math.ceil(start * frameRate), 0), totalFrames - 1);
  const endFrame = Math.min(Math.max(Math.ceil(end * frameRate), startFrame + 1), totalFrames);
  return { startFrame, endFrame };
}

function isExactFrameRangeCached(cache: Map<number, PrerenderCacheBlock>, frameRange: { startFrame: number; endFrame: number }) {
...
function queueIncludesFrameRange(queue: PrerenderQueueItem[], frameRange: { startFrame: number; endFrame: number }) {
  return queue.some((item) => item.frameRange?.startFrame === frameRange.startFrame && item.frameRange.endFrame === frameRange.endFrame);
}

function resolveManualJobs(jobs: Map<number, ManualPrerenderJob>, error: string) {
  for (const job of jobs.values()) {
    job.resolve({ queuedBlocks: job.queuedRanges, visibleRanges: job.visibleRanges, completed: false, error });
  }
  jobs.clear();
}
