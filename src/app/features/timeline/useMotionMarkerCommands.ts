...
import {
  ...
  motionBlockFromMarker,
  parseTimelineMarkerKey,
  timelineMarkerKey,
  timelineMoveKey,
...
import {
  applyCameraPreviewToElement,
  buildMotionMarkerCameraPreviewTransform,
} from "./motionCameraPreview";
...
export function useMotionMarkerCommands({
  ...
  const selectedObjectCenter = selectedObjectBounds
    ? centerOf(selectedObjectBounds)
    : null;
  const timelineMotionPart = {
    ...part,
  ...
  function getAbsoluteMotionMarkers() {
    return getMotionMarkerViews(scene).motionMarkers.map((marker) => ({
      ...marker,
      id: timelineMarkerKey(TIMELINE_MOTION_PART_ID, marker.id),
  ...
  function updateMotionMarker(
    ...
    if (partId === TIMELINE_MOTION_PART_ID) {
      updateSceneMotionMarkers((markers) => ({
        motionMarkers: markers.map((m) =>
          m.id === markerId ? updater(m, timelineMotionPart) : m,
        ),
      }));
      return;
    }
    updateSceneParts((parts) =>
      parts.map((item) => {
        ...
        return withMotionMarkers(
          item,
          motionMarkers.map((m) => (m.id === markerId ? updater(m, item) : m)),
        );
      }),
    );
  }

  function updateMotionMarkers(
    partId: string,
    updater: (markers: MotionMarker[], part: Part) => MotionMarker[],
  ) {
    if (partId === TIMELINE_MOTION_PART_ID) {
      updateSceneMotionMarkers((markers) => ({
        motionMarkers: updater(markers, timelineMotionPart),
      }));
      return;
    }
    updateSceneParts((parts) =>
      parts.map((item) => {
        ...
        return withMotionMarkers(
          item,
          updater(getMotionMarkerViews(item).motionMarkers, item),
        );
      }),
    );
  }

  function updateMotionMarkerFocusGroup(
    partId: string,
    markerId: string,
    focus: Point,
  ) {
    const absoluteMarkers = getAbsoluteMotionMarkers();
    const markerKeys = getMendedMarkerIds(
      absoluteMarkers,
      timelineMarkerKey(partId, markerId),
    );
    updateSceneMotionMarkers((markers) => {
      const nextMarkers = markers.map((m) =>
        markerKeys.has(timelineMarkerKey(TIMELINE_MOTION_PART_ID, m.id))
          ? { ...m, focus }
          : m,
      );
  ...
  function moveMotionMarker(
    ...
    if (
      sourcePartId === TIMELINE_MOTION_PART_ID ||
      targetPartId === TIMELINE_MOTION_PART_ID
    ) {
      updateSceneMotionMarkers((markers) => {
        ...
        return applySceneMotionMarkerOverwrite(
          [...markers.filter((m) => m.id !== markerId), nextMarker],
          new Set([markerId]),
    ...
    updateSceneParts((parts) => {
      ...
      const targetTimelinePart = timelineParts.find(
        (item) => item.id === targetPartId,
      );
      const marker = sourcePart
        ? getMotionMarkerViews(sourcePart).motionMarkers.find(
            (m) => m.id === markerId,
          )
        : undefined;
      if (!sourcePart || !targetTimelinePart || !marker) return parts;
      const segments = placeMotionMarkerOnTimeline(
        marker,
        targetTimelinePart.start + start,
        timelineParts,
        targetLayerId,
        targetPartId,
      );
      ...
      return parts.map((item) => {
        ...
        const nextMarkers = [
          ...itemMarkers.filter((current) => current.id !== markerId),
          ...itemSegments,
        ];
        if (
          itemSegments.length === 0 &&
          nextMarkers.length === itemMarkers.length
        )
          return item;
    ...
    if (sourcePartId !== targetPartId)
      setSelectedMotionMarker({ partId: targetPartId, markerId });
  }

  function moveMotionMarkers(moves: TimelineMarkerMove[]) {
    for (const move of moves)
      assignAvailableMotionLayerKind(move.targetLayerId, "motion");
    if (
      moves.some(
        (move) =>
          move.sourcePartId === TIMELINE_MOTION_PART_ID ||
          move.targetPartId === TIMELINE_MOTION_PART_ID,
      )
    ) {
      updateSceneMotionMarkers((markers) => {
        ...
        const nextMarkers = markers.map((marker) => {
          ...
          return move
            ? remapMovedMarkerMendIds(
                {
                  ...marker,
                  layerId: move.targetLayerId ?? marker.layerId,
                  start: move.start,
                },
                new Map(),
              )
      ...
      setSelectedMotionMarker(
        nextSelection.at(-1) ??
          (moves[0]
            ? { partId: TIMELINE_MOTION_PART_ID, markerId: moves[0].markerId }
            : null),
      );
      return;
    }
    updateSceneParts((parts) => {
      ...
      const movedMarkerKeys = new Map(
        moves.map((move) => [
          timelineMoveKey(move.sourcePartId, move.markerId),
          timelineMoveKey(move.targetPartId, move.markerId),
        ]),
      );
      const movedSegments = new Map<
        string,
        Array<{ partId: string; marker: MotionMarker }>
      >();
      ...
      for (const move of moves) {
        ...
        const targetTimelinePart = timelineParts.find(
          (item) => item.id === move.targetPartId,
        );
        const marker = sourcePart
          ? getMotionMarkerViews(sourcePart).motionMarkers.find(
              (m) => m.id === move.markerId,
            )
          : undefined;
        if (!sourcePart || !targetPart || !targetTimelinePart || !marker)
          continue;
        const segments = placeMotionMarkerOnTimeline(
          remapMovedMarkerMendIds(marker, movedMarkerKeys),
          targetTimelinePart.start + move.start,
          timelineParts,
          move.targetLayerId,
          move.targetPartId,
        );
        ...
        for (const segment of segments)
          targetMovesByPart.set(segment.partId, [
            ...(targetMovesByPart.get(segment.partId) ?? []),
            move,
          ]);
        removeKeysByPart.set(
          move.sourcePartId,
          (removeKeysByPart.get(move.sourcePartId) ?? new Set()).add(
            move.markerId,
          ),
        );
      }
      ...
      return parts.map((item) => {
        ...
        const itemMarkers = getMotionMarkerViews(item).motionMarkers;
        let nextMarkers = removeKeys
          ? itemMarkers.filter((current) => !removeKeys.has(current.id))
          : itemMarkers;
        for (const move of targetMoves) {
          const segments =
            movedSegments
              .get(timelineMoveKey(move.sourcePartId, move.markerId))
              ?.filter((segment) => segment.partId === item.id) ?? [];
          for (const segment of segments) insertedIds.add(segment.marker.id);
          nextMarkers = [
            ...nextMarkers.filter((current) => current.id !== move.markerId),
            ...segments.map((segment) => segment.marker),
          ];
        }
        return nextMarkers === itemMarkers
          ? item
          : applyMotionMarkerOverwrite(item, nextMarkers, insertedIds);
      });
    });
    const nextSelection = selectedMotionMarkers.map((selection) => {
      const move = moves.find(
        (item) =>
          item.sourcePartId === selection.partId &&
          item.markerId === selection.markerId,
      );
      return move
        ? { partId: move.targetPartId, markerId: selection.markerId }
        : selection;
    });
    if (nextSelection.length > 0) {
      ...
      setSelectedMotionMarker({
        partId: moves[0].targetPartId,
        markerId: moves[0].markerId,
      });
    }
  }
  ...
  function resizeMotionMarkers(resizes: TimelineMarkerResize[]) {
    ...
    updateSceneParts((parts) => {
      ...
      for (const resize of resizes) {
        ...
        const marker = sourcePart
          ? getMotionMarkerViews(sourcePart).motionMarkers.find(
              (m) => m.id === resize.markerId,
            )
          : undefined;
        if (!sourcePart || !marker) continue;
        const segments = placeMotionMarkerOnTimeline(
          { ...marker, duration: resize.duration },
          resize.absoluteStart,
          timelineParts,
          undefined,
          resize.sourcePartId,
        );
        resizedSegments.set(
          timelineMoveKey(resize.sourcePartId, resize.markerId),
          segments,
        );
        for (const segment of segments)
          targetResizesByPart.set(segment.partId, [
            ...(targetResizesByPart.get(segment.partId) ?? []),
            resize,
          ]);
        removeKeysByPart.set(
          resize.sourcePartId,
          (removeKeysByPart.get(resize.sourcePartId) ?? new Set()).add(
            resize.markerId,
          ),
        );
      }
      return parts.map((item) => {
        ...
        const itemMarkers = getMotionMarkerViews(item).motionMarkers;
        let nextMarkers = removeKeys
          ? itemMarkers.filter((marker) => !removeKeys.has(marker.id))
          : itemMarkers;
        for (const resize of targetResizes) {
          const segments =
            resizedSegments
              .get(timelineMoveKey(resize.sourcePartId, resize.markerId))
              ?.filter((segment) => segment.partId === item.id) ?? [];
          for (const segment of segments) insertedIds.add(segment.marker.id);
          nextMarkers = [
            ...nextMarkers.filter((marker) => marker.id !== resize.markerId),
            ...segments.map((segment) => segment.marker),
          ];
        }
        return nextMarkers === itemMarkers
  ...
  function deleteMotionMarker(partId: string, markerId: string) {
    if (partId === TIMELINE_MOTION_PART_ID) {
      updateSceneMotionMarkers((markers) => ({
        motionMarkers: markers.filter((m) => m.id !== markerId),
      }));
    } else {
      updateSceneParts((parts) =>
        parts.map((item) =>
          item.id === partId
            ? withMotionMarkers(
                item,
                getMotionMarkerViews(item).motionMarkers.filter(
                  (m) => m.id !== markerId,
                ),
              )
            : item,
  ...
  function addMotionMarker(kind: "zoom" | "pan" | "rotate") {
    ...
    const filtered =
      kind === "zoom"
        ? motionMarkers.filter((m) => (m as any).scale !== undefined)
        : motionMarkers.filter((m) => (m as any).scale === undefined);
    const placement = getAvailableMotionPlacement(
      filtered,
      part.duration,
      previewTime,
    );
    ...
    const markerIdPrefix =
      kind === "zoom" ? "zom" : kind === "rotate" ? "rot" : "trn";
    ...
    const block = effect.createDefaultBlock({
      ...
      focus: selectedObjectCenter ?? {
        x: FRAME_WIDTH / 2,
        y: FRAME_HEIGHT / 2,
      },
      position: selectedObjectCenter
        ? framePointToCameraTranslation(selectedObjectCenter)
        : { x: 0, y: 0 },
    });
  ...
  function addMotionEffect(
    ...
    if (canRetagLayer) {
      updateTimelineLayers(
        (state) => ({
          ...state,
          motionLayers: (state.motionLayers ?? []).map((layer) =>
            layer.id === layerId ? { ...layer, kind: "motion" } : layer,
          ),
        }),
        { history: true },
      );
    ...
    const duration = Math.min(
      markerDurationSeconds,
      Math.max(sceneDurationSeconds, 0.1),
    );
    const absoluteStart = roundToPrecision(
      Math.max(sceneTime, 0),
      timelinePrecision,
    );
    const markerIdPrefix =
      effect.kind === "zoom"
    ...
    updateSceneMotionMarkers(
      (markers) =>
        applySceneMotionMarkerOverwrite(
          [...markers, nextMarker],
          new Set([nextMarker.id]),
        ) as SceneMotionMarkerUpdate,
    );
    const selection = [
      { partId: TIMELINE_MOTION_PART_ID, markerId: nextMarker.id },
    ];
  ...
  function updateSelectedMotionSnap(
    key: "snapIn" | "snapOut",
    enabled: boolean,
  ) {
    const selectedIdsByPart = new Map<string, Set<string>>();
    for (const selection of selectedMotionMarkers)
      selectedIdsByPart.set(
        selection.partId,
        (selectedIdsByPart.get(selection.partId) ?? new Set()).add(
          selection.markerId,
        ),
      );
    function applySnapToggle(
      ...
      for (const marker of nextMarkers) {
        ...
        if (key === "snapIn" && marker.mendInId) {
          ...
          const clearedMarker = {
            ...updated.get(marker.id)!,
            mendInId: undefined,
            middleTransition: undefined,
            middleEase: undefined,
          };
          clearedMarker.params = clearMendParams(clearedMarker);
          if (clearedMarker.params)
            clearedMarker.params = clearMendVisualParams(clearedMarker.params);
          updated.set(marker.id, clearedMarker);
          if (previous?.mendOutId === marker.id) {
            const clearedPrevious = {
              ...updated.get(previous.id)!,
              mendOutId: undefined,
            };
        ...
        if (key === "snapOut" && marker.mendOutId) {
          ...
          if (next?.mendInId === marker.id) {
            ...
            if (clearedNext.params)
              clearedNext.params = clearMendVisualParams(clearedNext.params);
    ...
    updateSceneParts((parts) =>
      parts.map((item) => {
        ...
        const motionMarkers = applySnapToggle(
          getMotionMarkerViews(item).motionMarkers,
          selectedIds,
        );
  ...
  function snapMotionMiddle(targetPart = part) {
    const absoluteMarkers = getAbsoluteMotionMarkers();
    const targetPartSelectedIds = selectedMotionMarkers
      .filter((selection) => selection.partId === targetPart.id)
      .map((selection) =>
        timelineMarkerKey(selection.partId, selection.markerId),
      );
    const selectedIds = selectedMotionMarkers.map((selection) =>
      timelineMarkerKey(selection.partId, selection.markerId),
    );
    const selectedSnapIds = selectedIds.length > 1 ? selectedIds : [];
    const targetPartSnapIds =
      targetPartSelectedIds.length > 1 ? targetPartSelectedIds : [];
    const snap =
      getSelectedActiveMiddleMend(
        absoluteMarkers,
        selectedIds,
    ...
    const activeSelectedIds = new Set(
      selectedIds.length > 0 ? selectedIds : targetPartSelectedIds,
    );
    const nextSelection =
      snap.pairs.length > 1
        ? uniqueMarkerSelections(
            absoluteMarkers
              .filter((marker) => activeSelectedIds.has(marker.id))
    ...
    const mendedIds = new Set(
      snap.pairs.flatMap((pair) => [pair.previousId, pair.nextId]),
    );
    ...
    updateSceneMotionMarkers((markers) => {
      const nextMarkers = markers.map((marker) => {
        ...
        const nextMarker = {
          ...
          mendInId: normalizeTimelineMotionMendId(bounds.mendInId),
          mendOutId: normalizeTimelineMotionMendId(bounds.mendOutId),
          middleTransition: bounds.middleTransition,
          middleEase: bounds.middleEase,
          params: clearMendParams(marker),
        };
        ...
        if (middleSnapActive && nextMarker.params) {
          ...
          if (clearedVisual)
            nextMarker.params = clearedVisual as typeof nextMarker.params;
        }
        return nextMarker;
      });
      return { motionMarkers: nextMarkers };
  ...
  function normalizeTimelineMotionMendId(id: string | undefined) {
    if (!id) return undefined;
    const parsed = parseTimelineMarkerKey(id);
    return parsed.partId === TIMELINE_MOTION_PART_ID ? parsed.markerId : id;
  }

  function clearMendParams(marker: MotionMarker) {
    if (!marker.params) return marker.params;
    return {
      ...marker.params,
      mendInId: marker.mendInId,
      mendOutId: marker.mendOutId,
  ...
  function updateMotionMiddleTransition(
    _targetPart: Part,
    ...
    updateSceneMotionMarkers((markers) => ({
      motionMarkers: markers.map((marker) => {
        if (
          !nextMarkerIds.has(
            timelineMarkerKey(TIMELINE_MOTION_PART_ID, marker.id),
          )
        )
          return marker;
        const nextMarker = {
          ...marker,
          middleTransition:
            mode === "transition" ? ("transition" as const) : undefined,
        } as MotionMarker;
        if (nextMarker.params) nextMarker.params = clearMendParams(nextMarker);
        if (mode === "instant" && nextMarker.params)
          nextMarker.params = clearMendVisualParams(nextMarker.params);
        return nextMarker;
      }),
    }));
  }

  function updateMotionMiddleEase(
    _targetPart: Part,
