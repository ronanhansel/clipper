...
import {
  ...
  removeTimelineAdjustmentLayerMarkers,
  removeTimelineMotionLayerMarkers,
} from "../../../core/timeline";
import {
  getTimelineStateLayers,
  insertTimelineStateLayer,
...
import {
  createAdjustmentTimelineLayer,
  createBlankAdjustmentLayer,
  createBlankCompositionLayer,
  createBlankMotionLayer,
  createCompositionTimelineLayer,
  createMotionTimelineLayer,
  timelineClipFromPart,
} from "./timelineLayerHelpers";
...
export function useTimelineLayerCommands({
  ...
  function addMotionLayer(
    ...
    updateTimelineLayers(
      (state) => ({
        ...state,
        motionLayers: insertTimelineStateLayer(
          state.motionLayers ?? defaultTimelineLayerState.motionLayers!,
          newLayer,
          targetLayerId,
          placement,
        ),
  ...
  function addAdjustmentTimelineLayer(
    targetLayerId?: string,
    placement: "before" | "after" = "after",
  ) {
    ...
    updateTimelineLayers(
      (state) => ({
        ...state,
        adjustmentLayers: insertTimelineStateLayer(
          state.adjustmentLayers ?? defaultTimelineLayerState.adjustmentLayers!,
          newLayer,
          targetLayerId,
          placement,
        ),
  ...
  function addCompositionTimelineLayer(
    targetLayerId?: string,
    placement: "before" | "after" = "after",
  ) {
    ...
    updateTimelineLayers(
      (state) => ({
        ...state,
        compositionLayers: insertTimelineStateLayer(
  ...
  function removeCompositionTimelineLayer(layerId: string) {
    const layers = getTimelineStateLayers(
      timelineLayers,
      "comp",
      defaultTimelineLayerState,
    );
    const hasCompositions = scene.compositions.some(
      (composition) => (composition.layerId ?? "comp") === layerId,
    );
    const nextCompositionLayers =
      layers.length > 1
          ...
          timeline.id === scene.id
            ? {
                ...
                clips: timeline.clips.filter(
                  (clip) => (clip.layerId ?? "comp") !== layerId,
                ),
              }
            : timeline,
        ),
  ...
  function removeAdjustmentTimelineLayer(layerId: string) {
    ...
    const hasLayers = (scene.adjustmentLayers ?? []).some(
      (layer) => getAdjustmentLayerRowId(layer) === layerId,
    );
    const nextAdjustmentLayers =
      layers.length > 1
        ? layers.filter((layer) => layer.id !== layerId)
        : [createBlankAdjustmentLayer()];
    updateProject(
        ...
        timelines: (current.timelines ?? []).map((timeline) =>
          timeline.id === scene.id
            ? {
                ...
                adjustmentLayers: removeTimelineAdjustmentLayerMarkers(
  ...
  function removeMotionLayer(layerId: string) {
    ...
    const nextMotionLayers =
      motionLayers.length > 1
        ? motionLayers.filter((layer) => layer.id !== layerId)
        : [createBlankMotionLayer()];
    updateProject(
      (current) => ({
        ...current,
        timelines: (current.timelines ?? []).map((timeline) => {
          ...
          const clipsById = new Map(
            removeTimelineMotionLayerMarkers(
              ...
              layerId,
            ).map((item) => [item.id, timelineClipFromPart(item)]),
          );
          return {
            ...timeline,
            timelineLayers: {
            ...
            motionMarkers: getCanonicalMotionMarkers(timeline).filter(
  ...
  function motionLayerHasMarkers(layerId: string) {
    ...
    return motionViews.motionMarkers.some((marker) =>
      isMotionMarkerOnLayerId(marker, layerId),
    );
  }

  function assignAvailableMotionLayerKind(
    layerId: string | undefined,
    _kind: Exclude<TimelineMotionLayerKind, "empty">,
  ) {
    if (!layerId) return;
    const layer = motionLayers.find((item) => item.id === layerId);
    if (!layer || (layer.kind !== "empty" && motionLayerHasMarkers(layerId)))
      return;
    updateTimelineLayers(
      (state) => ({
        ...state,
        motionLayers: (state.motionLayers ?? []).map((item) =>
          item.id === layerId ? { ...item, kind: "motion" } : item,
        ),
      }),
      { history: true },
    );
