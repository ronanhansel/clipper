import {
  defaultAdjustmentEffectPackage,
...
type UpdateSceneAdjustmentLayers = (
...
export function useAdjustmentLayerCommands({
  ...
  function updateAdjustmentLayer(
    layerId: string,
  ...
  function moveAdjustmentLayer(
    layerId: string,
    start: number,
    targetLayerId?: string,
  ) {
    updateAdjustmentLayer(layerId, (layer) => ({
      ...layer,
  ...
  function addAdjustmentLayer() {
    addAdjustmentLayerAt(
      ...
      timelineLayers.adjustmentLayers?.[0]?.id,
    );
  }

  function addAdjustmentLayerAt(
    ...
    const layer = effect.createDefaultLayer({
    ...
    updateSceneAdjustmentLayers((layers) =>
      applyAdjustmentLayerOverwrite([...layers, layer], new Set([layer.id])),
    );
  ...
  function snapAdjustmentMiddle() {
    const markers: TimelineMendMarker[] = (scene.adjustmentLayers ?? []).map(
      (layer) => ({
        id: layer.id,
        start: layer.start,
    ...
    const selectedIds = selectedAdjustmentLayerId
      ? [selectedAdjustmentLayerId]
      : [];
    const snap =
      getSelectedActiveMiddleMend(
    ...
    const nextLayerId =
      snap.pairs[snap.pairs.length - 1]?.nextId ?? snap.pairs[0]?.previousId;
    if (nextLayerId) {
  ...
  function startAdjustmentPointPick(
    layerId: string,
    control: AdjustmentEffectPointControl,
  ) {
    if (
      pointPickAdjustment?.layerId === layerId &&
      pointPickAdjustment.control.xKey === control.xKey &&
      pointPickAdjustment.control.yKey === control.yKey
    ) {
