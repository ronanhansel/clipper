...
export type EditorPanelResizeKind =
  | "left"
  | "compose-left"
  | "right"
  | "timeline";
...
export function useEditorPanelResize({
  projectRef,
  updateEditorState,
}: UseEditorPanelResizeInput) {
  const editorPanelResizeRef = useRef<EditorPanelResizeDrag | null>(null);
  const [previewEditorLayout, setPreviewEditorLayout] =
    useState<EditorLayoutState | null>(null);

  function clampEditorLayout(layout: EditorLayoutState): EditorLayoutState {
    const availablePanelWidth = Math.max(
      window.innerWidth - editorPanelLayoutLimits.centerMin,
      editorPanelLayoutLimits.leftMin + editorPanelLayoutLimits.rightMin,
    );
    const rightPanelWidth = Math.round(
      clamp(
        layout.rightPanelWidth,
        editorPanelLayoutLimits.rightMin,
        Math.min(
          editorPanelLayoutLimits.rightMax,
          availablePanelWidth - editorPanelLayoutLimits.leftMin,
        ),
      ),
    );
    const leftPanelWidth = Math.round(
      clamp(
        ...
        editorPanelLayoutLimits.leftMin,
        Math.min(
          editorPanelLayoutLimits.leftMax,
          availablePanelWidth - rightPanelWidth,
        ),
      ),
    );
    const timelineMax = Math.max(
      editorPanelLayoutLimits.timelineMin,
      Math.min(
        editorPanelLayoutLimits.timelineMax,
        window.innerHeight - 48 - 220,
      ),
    );
  ...
  function updateEditorLayout(layout: EditorLayoutState) {
    ...
    const currentLayout =
      projectRef.current.editorState?.layout ?? defaultEditorLayoutState;
  ...
  function onEditorPanelResizeMove(event: globalThis.PointerEvent) {
    ...
    const nextLayout = clampEditorLayout({
      ...drag.initialLayout,
      leftPanelWidth:
        drag.kind === "left" || drag.kind === "compose-left"
          ? drag.initialLayout.leftPanelWidth + deltaX
          : drag.initialLayout.leftPanelWidth,
      rightPanelWidth:
        drag.kind === "right"
          ? drag.initialLayout.rightPanelWidth - deltaX
          : drag.initialLayout.rightPanelWidth,
      timelineHeight:
        drag.kind === "timeline"
          ? drag.initialLayout.timelineHeight - deltaY
          : drag.initialLayout.timelineHeight,
    });
  ...
  function startEditorPanelResize(
    ...
    const initialLayout = clampEditorLayout(
      projectRef.current.editorState?.layout ?? defaultEditorLayoutState,
    );
    const drag: EditorPanelResizeDrag = {
      kind,
    ...
    document.body.style.cursor =
      kind === "timeline" ? "row-resize" : "col-resize";
  ...
  useEffect(() => {
    if (
      ...
      previewEditorLayout &&
      editorLayoutsEqual(previewEditorLayout, committedEditorLayout)
    )
      setPreviewEditorLayout(null);
  }, [
    committedEditorLayout.leftPanelWidth,
    committedEditorLayout.rightPanelWidth,
    committedEditorLayout.timelineHeight,
    previewEditorLayout,
  ]);
