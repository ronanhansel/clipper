...
export function getProjectFolderSiblingNames(
  ...
  fallbackTimelineDirectory: string,
) {
  const names = new Set<string>();
  const addChildFolderName = (path: string) => {
    if (path === parentFolderPath || !path.startsWith(`${parentFolderPath}/`))
      return;
    const name = path
      .slice(parentFolderPath.length + 1)
      .split("/")[0]
      ?.trim();
    if (name) names.add(name);
  };
  for (const folder of project.compositionFolders ?? [])
    addChildFolderName(folder);
  for (const composition of project.compositionLibrary ?? [])
    addChildFolderName(getDirectoryPath(composition.filePath));
  for (const timeline of project.timelines ?? [])
    addChildFolderName(
      getDirectoryPath(
        timeline.filePath ??
          `${fallbackTimelineDirectory}/${timeline.id}.timeline.json`,
      ),
    );
  return names;
}

export function createCompositionInLibrary(
  project: ProjectManifest,
  compositionSources: Record<string, string>,
  _basePart: Part,
  filePath: string,
): CompositionLibraryMutationResult {
...
export function createCompositionFolderInProject(
  project: ProjectManifest,
  parentFolderPath: string,
  fallbackTimelineDirectory: string,
) {
  const folderName = nextNumberedName(
    "New folder",
    getProjectFolderSiblingNames(
      project,
      parentFolderPath,
      fallbackTimelineDirectory,
    ),
  );
  ...
  return {
    ...
    project: {
      ...project,
      compositionFolders: Array.from(
        new Set([...(project.compositionFolders ?? []), folderPath]),
      ),
    },
  };
}

export function renameCompositionInProject(
  project: ProjectManifest,
  compositionSources: Record<string, string>,
  compositionId: string,
  name: string,
  fallbackLibrary: Part[],
): CompositionLibraryMutationResult | null {
  const composition = fallbackLibrary.find((item) => item.id === compositionId);
  const nextName = name.trim();
  if (!composition || !nextName) return null;
  const nextFilePath = compositionFilePathWithName(
      ...
      replacePartInProject(
        {
          ...project,
          compositionSources: nextSources,
          compositionLibrary: project.compositionLibrary ?? fallbackLibrary,
        },
        compositionId,
        (item) => ({ ...item, filePath: nextFilePath }),
      ),
      composition.filePath,
      nextFilePath,
    ),
...
export function moveCompositionInProject(
  project: ProjectManifest,
  compositionSources: Record<string, string>,
  compositionId: string,
  folderPath: string,
  ...
  const composition = fallbackLibrary.find((item) => item.id === compositionId);
  if (!composition || getDirectoryPath(composition.filePath) === folderPath)
    return null;
  const fileName = composition.filePath.slice(
    composition.filePath.lastIndexOf("/") + 1,
  );
...
export function updateCompositionFilePathsInProject(
  project: ProjectManifest,
  compositionSources: Record<string, string>,
  pathMoves: Array<{ oldPath: string; newPath: string }>,
  fallbackLibrary: Part[],
): CompositionLibraryMutationResult | null {
...
function remapMovedFilePath(
  ...
  for (const move of pathMoves) {
    ...
    else if (nextPath.startsWith(`${move.oldPath}/`))
      nextPath = `${move.newPath}${nextPath.slice(move.oldPath.length)}`;
  }
  return nextPath;
}

function relinkEditorStatePaths(
  project: ProjectManifest,
  oldPath: string,
  nextPath: string,
): ProjectManifest {
  ...
  return {
    ...
    editorState: {
      ...editorState,
      selectedSceneId:
        editorState.selectedSceneId === oldPath
          ? nextPath
          : editorState.selectedSceneId,
      selectedTimelineId:
        editorState.selectedTimelineId === oldPath
          ? nextPath
          : editorState.selectedTimelineId,
      selectedPartId:
        editorState.selectedPartId === oldPath
          ? nextPath
      ...
      editorSession: editorState.editorSession
        ? {
            ...
            activeTabId:
              editorState.editorSession.activeTabId === oldPath
                ? nextPath
                : editorState.editorSession.activeTabId,
            tabs: editorState.editorSession.tabs.map((tab) =>
              tab.id === oldPath || tab.filePath === oldPath
                ? { ...tab, id: nextPath, filePath: nextPath }
                : tab,
            ),
          }
        : editorState.editorSession,
...
export function duplicateCompositionInProject(
  project: ProjectManifest,
  compositionSources: Record<string, string>,
  compositionId: string,
  fallbackLibrary: Part[],
  ...
  const nextSources = {
    ...compositionSources,
    [duplicate.filePath]: duplicateSource,
  };
  return {
    compositionSources: nextSources,
    project: {
      ...
      compositionSources: nextSources,
      compositionLibrary: [
        ...(project.compositionLibrary ?? fallbackLibrary),
        duplicate,
      ],
    },
  };
...
export function deleteCompositionFileFromProject(
  project: ProjectManifest,
  compositionSources: Record<string, string>,
  compositionId: string,
  fallbackLibrary: Part[],
  ...
  const composition = fallbackLibrary.find((item) => item.id === compositionId);
  if (!composition) return null;
  const { [composition.filePath]: _removed, ...nextSources } =
    compositionSources;
  return {
    compositionSources: nextSources,
...
export function relinkCompositionInProject(
  project: ProjectManifest,
  compositionSources: Record<string, string>,
  compositionId: string,
  nextFilePath: string,
  ...
  const rest = Object.fromEntries(
    Object.entries(compositionSources).filter(
      ([path]) => path !== composition.filePath,
    ),
  );
  const nextSources = { ...rest, [nextFilePath]: source };
  const { source: _source, ...restoredComposition } = parsedComposition
    ? {
        ...parsedComposition,
        id: composition.id,
