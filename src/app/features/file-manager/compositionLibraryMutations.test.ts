    ...
    timelines: [
      {
        ...
        clips: [{ id: "clip", compositionId: item.id, duration: 5 }],
      },
    ],
  };
}
...
describe("composition library mutations", () => {
  ...
  it("updates OS-renamed composition paths by stable id without rewriting timeline clips", () => {
      ...
      [
        {
          ...
          newPath: "compositions/B renamed.composition.ts",
        },
      ],
      [item],
    );
    ...
    expect(result?.project.compositionLibrary?.[0]).toMatchObject({
      id: "composition-b",
      filePath: "compositions/B renamed.composition.ts",
    });
    expect(result?.project.timelines?.[0].clips[0]?.compositionId).toBe(
      "composition-b",
    );
    expect(result?.compositionSources).toEqual({
      "compositions/B renamed.composition.ts": "source",
    });
  });
  ...
  it("updates timeline composition copies when an OS composition path changes", () => {
    ...
    expect(result?.project.compositions?.[0]).toMatchObject({
      id: "composition-b",
      filePath: "compositions/B renamed.composition.ts",
    });
    expect(result?.project.timelines?.[0].clips[0]?.compositionId).toBe(
  ...
  it("updates compositions inside OS-moved folders without rewriting timeline clips", () => {
    const item = composition(
      "composition-b",
      "compositions/folder/B.composition.ts",
    );
    const result = updateCompositionFilePathsInProject(
      projectWithClip(item),
      { [item.filePath]: "source" },
    ...
    expect(result?.compositionSources).toEqual({
      "compositions/renamed/B.composition.ts": "source",
    });
  });
  ...
  it("relinks a missing composition by updating its path only", () => {
    ...
    const result = relinkCompositionInProject(
      ...
      {},
      item.id,
      "compositions/found/B.composition.ts",
      "source",
      [item],
      composition(item.id, "compositions/found/B.composition.ts"),
    );

    expect(result?.project.compositionLibrary?.[0]).toMatchObject({
      id: "composition-b",
      filePath: "compositions/found/B.composition.ts",
      sourceMissing: undefined,
    });
    expect(result?.project.compositionLibrary?.[0]).not.toHaveProperty(
      "source",
    );
    expect(result?.project.timelines?.[0].clips[0]?.compositionId).toBe(
      "composition-b",
  ...
  it("creates composition library entries without embedded source", () => {
    ...
    const result = createCompositionInLibrary(
      ...
      {},
      item,
      "compositions/new.composition.ts",
    );

    expect(result.project.compositionLibrary?.at(-1)).not.toHaveProperty(
      "source",
    );
    expect(
      result.compositionSources["compositions/new.composition.ts"],
    ).toContain("new Composition");
    vi.unstubAllGlobals();
  });
});
