...
describe("MoveCommand", () => {
  ...
  it("rolls back already-applied moves when a later move fails", async () => {
    ...
    await expect(new MoveCommand([
      { oldPath: "/project/compositions/A", newPath: "/project/compositions/B" },
      { oldPath: "/project/compositions/C", newPath: "/project/compositions/D" },
    ]).execute()).rejects.toThrow("move failed");

    expect(clipperHost.renameFile).toHaveBeenNthCalledWith(1, "/project/compositions/A", "/project/compositions/B");
