...
describe("export service", () => {
  ...
  it("prepares rendered media with mp4 extension when format is mp4", async () => {
    const project: ProjectManifest = {
      ...
      scenes: [],
      compositionLibrary: [
        { id: "clip", filePath: "clip.ts", duration: 1, frame, background, objects: [], snapshot: [], motionMarkers: [] },
      ],
      timelines: [
        {
          id: "scene",
          clips: [
            { id: "c1", compositionId: "clip", start: 0, duration: 1, layerId: "l1" },
          ],
          timelineLayers: { compositionLayers: [{ id: "l1" }] },
        },
      ],
      editorState: { timeline: { displacement: 0, zoom: 1 }, timelineMode: "composition", timelineLayers: { compositionLayers: [{ id: "l1" }] } },
    };
    const { exportService } = await import("./exportService");

    const prepared = exportService.prepareRenderedMediaExport({ project, sceneId: "scene", mediaExportFormat: "mp4" });
  ...
  it("prepares rendered media with webm extension when format is webm", async () => {
    ...
    const prepared = exportService.prepareRenderedMediaExport({ project, sceneId: "scene", mediaExportFormat: "webm" });
    ...
    expect(hostMocks.renderVideoExport).toHaveBeenLastCalledWith("export-id", expect.stringMatching(/\.webm$/), expect.any(Object), "clipper/projects/export/project.json", expect.any(Object), 30, expect.any(Number), 270, true, FRAME_WIDTH, FRAME_HEIGHT, "webm", "high", undefined, undefined, "renderer", "safe", 1);
  });
});
