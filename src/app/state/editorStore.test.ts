...
describe("editorStore editor tabs", () => {
  it("restores the most recently closed pinned tab without persisting runtime source", () => {
    ...
    store
      .getState()
      .openEditorTab({ id: "b", filePath: "b.ts", language: "typescript" });
    store.getState().closeEditorTab("a");

    expect(store.getState().closedEditorTabs[0]).toEqual({
      id: "a",
      filePath: "a.ts",
      language: "typescript",
      isPinned: true,
    ...
    expect(store.getState().editorTabs.find((tab) => tab.id === "a")).toEqual({
      id: "a",
      filePath: "a.ts",
      language: "typescript",
      isPinned: true,
  ...
  it("restores temporary tabs through the existing temporary replacement slot", () => {
    ...
    store.getState().openTemporaryEditorTab({
      id: "temp-a",
      filePath: "temp-a.ts",
      language: "typescript",
    });
    ...
    expect(store.getState().restoreClosedEditorTab()).toBe(true);
    expect(store.getState().editorTabs).toEqual([
      {
        id: "temp-a",
        filePath: "temp-a.ts",
        language: "typescript",
  ...
  it("skips closed tabs that were reopened before restore", () => {
    ...
    store
      .getState()
      .openEditorTab({ id: "a", filePath: "a.ts", language: "typescript" });
    store
      .getState()
      .openEditorTab({ id: "b", filePath: "b.ts", language: "typescript" });
    store.getState().closeEditorTab("a");
    ...
    expect(store.getState().restoreClosedEditorTab()).toBe(true);
    expect(store.getState().activeEditorTabId).toBe("a");
    expect(store.getState().editorTabs.map((tab) => tab.id)).toEqual([
      "b",
      "a",
    ]);
  });
