...
function tabsFromEditorSession(
  editorState: EditorState | undefined,
): EditorTab[] {
  return (
    editorState?.editorSession?.tabs.map((tab: PersistedEditorTab) => ({
      ...tab,
      isPinned: true,
    })) ?? []
  );
...
export type EditorStoreActions = {
  ...
  setVideoExportProgress: (
    progress: Setter<VideoExportProgress | null>,
  ) => void;
  ...
  openEditorTab: (
    tab: Omit<EditorTab, "isPinned"> & { isPinned?: boolean },
  ) => void;
...
function createFieldSetter<T extends keyof EditorStoreState>(
  set: StoreApi<EditorStore>["setState"],
  field: T,
) {
  return (setter: Setter<EditorStoreState[T]>) =>
    set((state) => {
      const nextValue = resolveSetter(state[field], setter);
      return Object.is(nextValue, state[field])
        ? state
        : ({ [field]: nextValue } as Pick<EditorStoreState, T>);
    });
}
...
function getInitialState(project: ProjectManifest): EditorStoreState {
  ...
  return {
    ...
    selectedParts: editorState?.selectedPartId
    ...
    defaultNewMarkerDurationSeconds:
      editorState?.defaultNewMarkerDurationSeconds ??
      defaultNewMarkerDurationSeconds,
    timelineEndPaddingFraction:
      editorState?.timelineEndPaddingFraction ??
      defaultTimelineEndPaddingFraction,
    timelinePrecision:
      editorState?.timelinePrecision ?? defaultTimelinePrecision,
    pausePlaybackOnScrub:
      editorState?.pausePlaybackOnScrub ?? defaultPausePlaybackOnScrub,
...
export function createEditorStore(project: ProjectManifest) {
  return createStore<EditorStore>((set, get) => ({
    ...
    setPositionPickTranslationMarker: createFieldSetter(
      set,
      "positionPickTranslationMarker",
    ),
    setSelectedAdjustmentLayerId: createFieldSetter(
      set,
      "selectedAdjustmentLayerId",
    ),
    setSelectedAdjustmentLayers: createFieldSetter(
      set,
      "selectedAdjustmentLayers",
    ),
    setSelectedTransitionLayerId: createFieldSetter(
      set,
      "selectedTransitionLayerId",
    ),
    setSelectedTransitionLayers: createFieldSetter(
      set,
      "selectedTransitionLayers",
    ),
    ...
    setDefaultNewMarkerDurationSeconds: createFieldSetter(
      set,
      "defaultNewMarkerDurationSeconds",
    ),
    setTimelineEndPaddingFraction: createFieldSetter(
      set,
      "timelineEndPaddingFraction",
    ),
    ...
    openEditorTab: (tab) =>
      set((state) => {
        ...
        const remainingTabs = state.editorTabs.filter(
          (item) => item.id !== tab.id,
        );
        ...
        return {
          editorTabs,
          closedEditorTabs: state.closedEditorTabs.filter(
            (closedTab) => closedTab.id !== tab.id,
          ),
          activeEditorTabId: tab.id,
        };
      }),
    openTemporaryEditorTab: (tab) =>
      set((state) => {
        const existingTab = state.editorTabs.find((item) => item.id === tab.id);
        if (existingTab) {
          const editorTabs = state.editorTabs.map((item) =>
        ...
        const temporaryIndex = state.editorTabs.findIndex(
          (item) => !item.isPinned,
        );
        const editorTabs =
          temporaryIndex >= 0
            ? state.editorTabs.map((item, index) =>
                index === temporaryIndex ? temporaryTab : item,
              )
            : [...state.editorTabs, temporaryTab];
        return {
          editorTabs,
          closedEditorTabs: state.closedEditorTabs.filter(
            (closedTab) => closedTab.id !== tab.id,
    ...
    pinEditorTab: (tabId) =>
      set((state) =>
        state.editorTabs.some((tab) => tab.id === tabId && !tab.isPinned)
          ? {
    ...
    selectEditorTab: (tabId) =>
      set((state) =>
        ...
        !state.editorTabs.some((tab) => tab.id === tabId)
          ? state
    ...
    closeEditorTab: (tabId) =>
      set((state) => {
        ...
        const closedEditorTabs = [
          closedTab,
          ...state.closedEditorTabs.filter((tab) => tab.id !== tabId),
        ].slice(0, closedEditorTabStackLimit);
        if (state.activeEditorTabId !== tabId)
          return { editorTabs, closedEditorTabs };
        const nextActiveTab =
          editorTabs[Math.min(tabIndex, editorTabs.length - 1)] ?? null;
        return {
          editorTabs,
          closedEditorTabs,
          activeEditorTabId: nextActiveTab?.id ?? null,
        };
      }),
    closeCompositionEditorTabs: () =>
      set((state) => {
        ...
        if (
          ...
          editorTabs.some((tab) => tab.id === state.activeEditorTabId)
        )
          return { editorTabs };
    ...
    restoreClosedEditorTab: () => {
      const state = get();
      const restoreIndex = state.closedEditorTabs.findIndex(
        (tab) => !state.editorTabs.some((openTab) => openTab.id === tab.id),
      );
      if (restoreIndex < 0) {
      ...
      const remainingClosedTabs = state.closedEditorTabs.filter(
        (_, index) =>
          index > restoreIndex ||
          (index < restoreIndex &&
            !state.editorTabs.some(
              (openTab) => openTab.id === state.closedEditorTabs[index].id,
            )),
      );
      const editorTabs = restoredTab.isPinned
        ? [...state.editorTabs, restoredTab]
        : (() => {
            const temporaryIndex = state.editorTabs.findIndex(
              (tab) => !tab.isPinned,
            );
            return temporaryIndex >= 0
              ? state.editorTabs.map((tab, index) =>
                  index === temporaryIndex ? restoredTab : tab,
                )
              : [...state.editorTabs, restoredTab];
          })();
      set({
        editorTabs,
        closedEditorTabs: remainingClosedTabs,
        activeEditorTabId: restoredTab.id,
      });
      return true;
    },
    applyEditorState: (editorState, fallbackSceneId, options) =>
      set((state) => ({
        ...
        selectedParts: editorState.selectedPartId
          ? [{ partId: editorState.selectedPartId }]
          : [],
        selectedMotionMarker: options?.preserveMarkerSelection
          ? state.selectedMotionMarker
          : (editorState.selectedMotionMarker ?? null),
        selectedMotionMarkers: options?.preserveMarkerSelection
          ? state.selectedMotionMarkers
          : editorState.selectedMotionMarker
            ? [editorState.selectedMotionMarker]
            : [],
        selectedComposeObjectIds: editorState.selectedComposeObjectIds ?? [],
        ...
        frameZoomBarOpen:
          ...
          defaultPreviewViewportState.zoomBarOpen,
        framePreviewScale:
          editorState.preview?.scale ?? defaultFramePreviewScale,
        leftPanelTab: editorState.leftPanelTab ?? "assets",
        rightPanelTab: normalizeRightPanelTab(editorState.rightPanelTab),
        editorTabs: tabsFromEditorSession(editorState),
        closedEditorTabs: [],
...
export function EditorStoreProvider({
  children,
  project,
}: PropsWithChildren<{ project: ProjectManifest }>) {
