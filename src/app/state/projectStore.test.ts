...
describe("projectStore", () => {
  it("seeds saved snapshots from persisted project shape", () => {
    const project: ProjectManifest = {
      ...
      compositionLibrary: [],
      compositionSources: {
        "compositions/comp.composition.ts": "export const composition = {};",
      },
      timelines: [
        {
          id: "timeline",
    ...
    expect(store.getState().savedProjectSnapshot).toBe(
      getProjectContentSnapshot(persistedProject),
    );
    expect(store.getState().savedCompositionSourcesSnapshot).toBe(
      JSON.stringify(persistedProject.compositionSources ?? {}),
    );
  });
});
