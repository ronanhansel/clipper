...
import { classifyProjectAutosaveWrite, classifyProjectFileChange, type ProjectDiskSnapshotBundle, type ProjectSnapshotPair } from "./projectFileChangeClassifier";

const snapshot = (value: string): ProjectSnapshotPair => ({ project: `project:${value}`, compositionSources: `sources:${value}` });
const disk = (full: string, fileContent = full): ProjectDiskSnapshotBundle => ({ full: snapshot(full), fileContent: snapshot(fileContent) });

describe("classifyProjectFileChange", () => {
  it("ignores watcher events for unchanged file-manager content", () => {
    expect(classifyProjectFileChange({
...
describe("classifyProjectAutosaveWrite", () => {
  it("marks target disk snapshots as saved without another write", () => {
    expect(classifyProjectAutosaveWrite({
  ...
  it("conflicts when project metadata changed outside the app", () => {
    expect(classifyProjectAutosaveWrite({
      diskSnapshots: { ...disk("external", "external-content"), projectMetadata: snapshot("external-metadata") },
      savedFileContentSnapshots: snapshot("saved-content"),
  ...
  it("updates only the file-content baseline when editor state remains unsaved", () => {
    expect(classifyProjectFileChange({
      ...
      diskSnapshots: disk("disk-full-with-old-editor-state", "current-content"),
      hasUnsavedAppChanges: true,
      savedFileContentSnapshots: snapshot("old-content"),
