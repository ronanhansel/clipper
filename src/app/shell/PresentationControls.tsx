...
export function PresentationControls({ controlsVisible, isPlaying, sceneDurationSeconds, scrubberStyle, time, jumpToEnd, jumpToStart, pausePlaybackForPresentationScrub, resumePlaybackAfterPresentationScrub, scrubPresentationTime, stepSceneTime, togglePlayback }: PresentationControlsProps) {
  return (
    <div className={`pointer-events-none fixed inset-x-0 bottom-8 z-[2147483647] flex justify-center px-6 transition-opacity duration-200 ${controlsVisible ? "opacity-100" : "opacity-0"}`} data-clipper-presentation-controls>
