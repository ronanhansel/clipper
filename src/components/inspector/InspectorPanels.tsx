import { AlignCenter, AlignJustify, AlignLeft, AlignRight, Bold, Copy, Italic, Strikethrough, Trash2, Underline, X } from "lucide-react";
...
export function ObjectAnimationInspector({ object, onChange }: { object: FrameObject; onChange: (updater: (object: FrameObject) => FrameObject) => void }) {
        ...
        <div key={anim.id} className="grid gap-2.5 rounded-[10px] border border-[#2d313b] bg-[#171920] p-3">
          ...
          <label className={`grid gap-1.5 ${mutedCaps}`}>Ease
            <Select value={animationEaseSelectValue(anim.options.ease)} onValueChange={(value) => updateAnimation(anim.id, (a) => ({ ...a, options: { ...a.options, ease: value as MotionEase } }))}>
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent><TooltipProvider delayDuration={1000} skipDelayDuration={0}><SelectGroup><EaseSelectItems /></SelectGroup></TooltipProvider></SelectContent>
            </Select>
          </label>
  ...
  return (
    <div className="grid gap-3">
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        {markerKind === "rotate" ? <label className={`grid gap-1.5 ${mutedCaps}`}>Rotation<Input type="number" numberScrubMode="preview" numberScrubCommitThrottleMs={16} resetValue={15} step={1} value={marker.rotation ?? 0} onNumberScrubStart={onPreviewScrubStart} onNumberScrubPreview={(v) => previewRotation(v)} onNumberScrubEnd={finishPreviewScrub} onChange={(event) => updateRotation(event.target.value)} /></label> : markerKind === "perspective" ? <>
          <label className={`grid gap-1.5 ${mutedCaps}`}>Z<Input type="number" numberScrubMode="preview" numberScrubCommitThrottleMs={16} resetValue={0} step={1} value={marker.perspective?.z ?? 0} onNumberScrubStart={onPreviewScrubStart} onNumberScrubPreview={(v) => previewPerspective("z", v)} onNumberScrubEnd={finishPreviewScrub} onChange={(event) => updatePerspective("z", event.target.value)} /></label>
          <label className={`grid gap-1.5 ${mutedCaps}`}>Tilt X<Input type="number" numberScrubMode="preview" numberScrubCommitThrottleMs={16} resetValue={8} step={1} value={marker.perspective?.rotateX ?? 0} onNumberScrubStart={onPreviewScrubStart} onNumberScrubPreview={(v) => previewPerspective("rotateX", v)} onNumberScrubEnd={finishPreviewScrub} onChange={(event) => updatePerspective("rotateX", event.target.value)} /></label>
        </> : null}
      </div>
      {markerKind === "zoom" ? <Coordinate2DField label="Focus" pickLabel="Pick focus from frame" picking={pickingFocus} x={{ max: FRAME_WIDTH, min: 0, numberScrubMode: "preview", onChange: (value) => updateFocus("x", value), onNumberScrubEnd: finishPreviewScrub, onNumberScrubPreview: (value) => previewFocus("x", value), onNumberScrubStart: onPreviewScrubStart, resetValue: FRAME_WIDTH / 2, step: 1, value: marker.focus?.x ?? FRAME_WIDTH / 2 }} y={{ max: FRAME_HEIGHT, min: 0, numberScrubMode: "preview", onChange: (value) => updateFocus("y", value), onNumberScrubEnd: finishPreviewScrub, onNumberScrubPreview: (value) => previewFocus("y", value), onNumberScrubStart: onPreviewScrubStart, resetValue: FRAME_HEIGHT / 2, step: 1, value: marker.focus?.y ?? FRAME_HEIGHT / 2 }} onPick={onPickFocus} /> : null}
      {markerKind === "pan" ? <Coordinate2DField disabledReason={positionDisabledReason} label="Position" pickLabel={positionDisabledReason ?? "Pick pan target from frame"} picking={pickingPosition} x={{ disabled: Boolean(positionDisabledReason), numberScrubMode: "preview", onChange: (value) => updatePosition("x", value), onNumberScrubEnd: finishPreviewScrub, onNumberScrubPreview: (value) => previewPosition("x", value), onNumberScrubStart: onPreviewScrubStart, resetValue: 0, step: 1, value: marker.position?.x ?? 0 }} y={{ disabled: Boolean(positionDisabledReason), numberScrubMode: "preview", onChange: (value) => updatePosition("y", value), onNumberScrubEnd: finishPreviewScrub, onNumberScrubPreview: (value) => previewPosition("y", value), onNumberScrubStart: onPreviewScrubStart, resetValue: 0, step: 1, value: marker.position?.y ?? 0 }} onPick={onPickPosition} /> : null}
      {markerKind === "zoom" ? <label className={`grid gap-1.5 ${mutedCaps}`}>Scale<div className="grid grid-cols-[1fr_52px] items-center gap-2 rounded-[10px] border border-[#2d313b] bg-[#171920] px-2.5 py-2"><input aria-label="Zoom scale" className="h-1.5 min-w-0 accent-[#37d6c2] [appearance:none] rounded-full bg-[#2d313b] [&::-webkit-slider-runnable-track]:h-1.5 [&::-webkit-slider-runnable-track]:rounded-full [&::-webkit-slider-runnable-track]:bg-[#2d313b] [&::-webkit-slider-thumb]:mt-[-5px] [&::-webkit-slider-thumb]:h-4 [&::-webkit-slider-thumb]:w-4 [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:border [&::-webkit-slider-thumb]:border-[#37d6c2] [&::-webkit-slider-thumb]:bg-[var(--clipper-accent)] [&::-moz-range-track]:h-1.5 [&::-moz-range-track]:rounded-full [&::-moz-range-track]:bg-[#2d313b] [&::-moz-range-thumb]:h-4 [&::-moz-range-thumb]:w-4 [&::-moz-range-thumb]:rounded-full [&::-moz-range-thumb]:border [&::-moz-range-thumb]:border-[#37d6c2] [&::-moz-range-thumb]:bg-[var(--clipper-accent)]" type="range" min={1} max={5} step={0.01} value={draftScale} onChange={(event) => updateDraftScale(event.target.value)} onPointerDown={onPreviewScrubStart} onPointerUp={() => commitScale()} onKeyDown={onPreviewScrubStart} onKeyUp={() => commitScale()} onBlur={() => commitScale()} /><span className="text-right text-xs font-extrabold text-[#dfe2ea] tabular-nums">{draftScale.toFixed(2)}</span></div></label> : null}
      {markerKind === "perspective" ? <label className={`grid gap-1.5 ${mutedCaps}`}>Tilt Y<Input type="number" numberScrubMode="preview" numberScrubCommitThrottleMs={16} resetValue={0} step={1} value={marker.perspective?.rotateY ?? 0} onNumberScrubStart={onPreviewScrubStart} onNumberScrubPreview={(v) => previewPerspective("rotateY", v)} onNumberScrubEnd={finishPreviewScrub} onChange={(event) => updatePerspective("rotateY", event.target.value)} /></label> : null}
