...
function getClipperAccent() {
  ...
  const rgb = getClipperCssVariable("--clipper-accent-rgb")
    .split(/\s+/)
    .join(",");
...
export function EditorPane({
  ...
  const latestViewportStateRef = useRef<CodeViewportState>(
    editorViewportStateCache.get(sourceId) ??
      viewportState ?? { scrollLeft: 0, scrollTop: 0 },
  );
  const editorOptions = getMonacoOptionsForDocument({ ...document, source });

  useEffect(() => {
    closeActiveTabRef.current = () => onCloseTab(document.id);
  }, [document.id, onCloseTab]);
  useEffect(() => {
    restoreClosedTabRef.current = onRestoreClosedTab;
  }, [onRestoreClosedTab]);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    latestViewportStateRef.current = editorViewportStateCache.get(sourceId) ??
      viewportState ?? { scrollLeft: 0, scrollTop: 0 };
  ...
  function persistEditorViewportState() {
    saveMonacoViewState();
    editorViewportStateCache.set(
      sourceIdRef.current,
      latestViewportStateRef.current,
    );
    viewportStateChangeRef.current(
      sourceIdRef.current,
      latestViewportStateRef.current,
    );
  }
  ...
  function saveMonacoViewState() {
    ...
    if (viewState)
      editorMonacoViewStateCache.set(sourceIdRef.current, viewState);
  }
      ...
      .catch((error: unknown) => {
        setError(
          error instanceof Error
            ? error.message
            : "Unable to apply editor changes.",
        );
      });
  }
  ...
  return (
    <div
      className="grid h-full min-h-0 w-full overflow-hidden bg-[#12141a]"
      style={{ gridTemplateRows }}
    >
      <div
        className="clipper-hidden-scrollbar flex min-w-0 items-end overflow-x-auto border-b border-[#2d313b] bg-[#171920] px-2 pt-2 text-xs"
        ...
        {tabs.map((tab) => {
          ...
          return (
            <button
              key={tab.id}
              type="button"
              className={`group flex max-w-[220px] shrink-0 items-center gap-2 rounded-t-[3px] border border-b-0 px-3 py-2 text-left font-bold transition-colors ${selected ? "border-[#2d313b] bg-[#12141a] text-[#dfe2ea]" : "border-transparent bg-transparent text-[#8c929f] hover:bg-[#20232c] hover:text-[#dfe2ea]"}`}
              title={tab.filePath}
            ...
            >
              ...
              {tab.unsupportedReason ? (
                <span className="text-[#f0b35c]">!</span>
              ) : null}
              <span
                className="rounded-[3px] px-1 text-[#565b66] opacity-70 hover:bg-[#2d313b] hover:text-[#dfe2ea] group-hover:opacity-100"
                role="button"
                tabIndex={-1}
        ...
        {document.fileRemoved ? (
          ...
          <div className="grid h-full place-items-center p-8 text-center">
            <div className="max-w-[520px] rounded-[18px] border border-[#2d313b] bg-[#171920] p-7 shadow-[0_18px_60px_rgba(0,0,0,0.25)]">
              ...
              <div className="mt-4 break-all text-xs text-[#565b66]">
                {document.filePath}
              </div>
            </div>
          </div>
      ...
      {error ? (
        <div className="border-t border-[#3b2a2a] bg-[#1a0f10] px-3.5 py-2 text-xs text-[#ffb4b4] break-words">
          {error}
        </div>
      ) : null}
    </div>
