...
export function CodePane({ part, source: externalSource, viewportState, active = true, projectDirectory, onSaveAll, onSourceChange, onViewportStateChange }: { part: Part; source: string | undefined; viewportState?: CodeViewportState; active?: boolean; projectDirectory?: string; onSaveAll: () => Promise<void>; onSourceChange: (source: string) => Promise<void>; onViewportStateChange: (sourceId: string, state: CodeViewportState) => void }) {
  ...
  return <div className="grid h-full min-h-0 w-full overflow-hidden bg-[#12141a]" style={{ gridTemplateRows }}><div className="flex min-w-0 items-center gap-2 border-b border-[#2d313b] bg-[#171920] px-3.5 py-3 text-xs"><span className="font-extrabold text-[var(--clipper-accent)]">Project code</span><span className="text-[#565b66]">/</span><span className="min-w-0 truncate text-[#dfe2ea]">{getDisplayNameFromPath(part.filePath)}</span></div>
