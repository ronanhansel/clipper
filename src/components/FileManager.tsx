...
type FileManagerContextValue = FileManagerProps & {
  ...
  registerProjectFileCreator: (
    creator: ((folderPath?: string) => void) | null,
  ) => void;
...
function FileManagerProvider({
  children,
  ...props
}: PropsWithChildren<FileManagerProps>) {
  const [contextMenu, setContextMenu] = useState<ContextMenuState>(null);
  const [selectedNodeIds, setSelectedNodeIds] = useState<string[]>([]);
  const selectedNodeId = selectedNodeIds[0] ?? null;
  const treeFocusClearerRef = useRef<(() => void) | null>(null);
  const projectFileCreatorRef = useRef<((folderPath?: string) => void) | null>(
    null,
  );
  const clearTreeFocus = useCallback(() => treeFocusClearerRef.current?.(), []);
  const requestProjectFileCreation = useCallback(
    (folderPath?: string) => projectFileCreatorRef.current?.(folderPath),
    [],
  );
  const registerTreeFocusClearer = useCallback(
    (clearer: (() => void) | null) => {
      treeFocusClearerRef.current = clearer;
    },
    [],
  );
  const registerProjectFileCreator = useCallback(
    (creator: ((folderPath?: string) => void) | null) => {
  ...
  const requestNodeExpansion = useCallback(
    (nodeId: string) => nodeExpanderRef.current?.(nodeId),
    [],
  );
  const registerNodeExpander = useCallback(
    (expander: ((nodeId: string) => void) | null) => {
      nodeExpanderRef.current = expander;
    },
    [],
  );
  const setSelectedNodeId = useCallback(
    (nodeId: string | null) => setSelectedNodeIds(nodeId ? [nodeId] : []),
    [],
  );
  const value = useMemo<FileManagerContextValue>(
    () => ({
      ...
      selectedNodeIds,
      clearTreeFocus,
      requestProjectFileCreation,
      registerTreeFocusClearer,
      registerProjectFileCreator,
      requestNodeExpansion,
      registerNodeExpander,
      setContextMenu,
      setSelectedNodeId,
      setSelectedNodeIds,
    }),
...
function FileManagerPanel() {
  ...
  function openProjectMenu(event: ReactMouseEvent<HTMLElement>) {
    ...
    setContextMenu({
      ...
      items: [
        ...
        {
          label: "New composition folder",
          action: () => onCreateCompositionFolder(),
        },
        ...
        { label: "Reveal assets in Finder", action: onRevealAssetRoot },
        {
          ...
          children: getAssetSortMenuItems(null, onSortAssets),
        },
      ],
    });
  }
  ...
  function deleteSelectedFileManagerNodes(
    event: Pick<
    ...
    const nodes = getTopLevelSelectedFileTreeNodes(
      buildUnifiedFileTree(
        compositions,
        compositionFolders,
        compositionRootPath,
        timelines,
    ...
    event.preventDefault();
    event.stopPropagation();
    deleteFileTreeNodes(nodes, timelines.length, {
      onDeleteAsset,
      onDeleteComposition,
      onDeleteCompositionFolder,
      onDeleteProjectFile,
  ...
  return (
    <section
      ...
      className="min-h-0 min-w-0 overflow-auto rounded-[14px] border border-dashed border-[#303646] bg-[#151821] p-3"
      onContextMenu={openProjectMenu}
      onKeyDown={handleFileManagerKeyDown}
      onPointerDown={handleFileManagerPointerDown}
    >
      <div className="mb-2 flex items-center justify-between px-0.5">
        <h3 className="text-[13px] text-[#aeb3c1]">File Manager</h3>
        <button
          className="grid h-7 w-7 place-items-center rounded-md text-[#dfe2ea] hover:bg-[#20232c]"
          aria-label="Create file manager item"
...
function UnifiedFileManagerTree() {
  const {
    ...
    onRenameAsset,
    onRenameComposition,
    onRenameCompositionFolder: onRenameFolder,
    onRenameProjectFile,
    onRenameTimeline,
    registerNodeExpander,
    registerProjectFileCreator,
    ...
    setSelectedNodeIds: onSelectNodes,
    onSelectTimeline,
  } = useFileManager();
  const rawTree = useMemo(
    () =>
      buildUnifiedFileTree(compositions, folders, rootPath, timelines, assets),
    [assets, compositions, folders, rootPath, timelines],
  );
  const [tree, setTree] = useState(() =>
    syncFileTreeToSavedState(rawTree, fileManagerState?.tree),
  );
  const initialOpenState = useMemo(
    () =>
      hasUnifiedFileManagerOpenState(fileManagerState?.openState)
        ? (fileManagerState?.openState ?? {})
        : getFileTreeOpenState(tree),
    [fileManagerState?.openState, tree],
  );
  const visibleRowCount = countFileTreeNodes(tree);
  const rowDropHeight =
    visibleRowCount * FILE_MANAGER_ROW_HEIGHT + FILE_MANAGER_TOP_DROP_PADDING;
  ...
  const nativeTreeRef = useRef<NativeTreeApi<FileManagerTreeNode> | undefined>(
    undefined,
  );
  ...
  const externalDragRef = useRef<{
    node: Extract<FileManagerTreeNode, { kind: "composition" }>;
  ...
  const [dropParentId, setDropParentId] = useState<string | null | undefined>(
    undefined,
  );
  ...
  const {
    ...
    getScrollElement: () => getFileManagerScrollElement(treeRef.current),
    axis: "y",
  });
  ...
  const applyTreeMove = useCallback(
    (dragIds: string[], parentId: string | null, index: number) => {
      const nextTree = moveFileTreeNodes(
        latestTreeRef.current,
        dragIds,
        parentId,
        index,
      ...
      setTree(nextTree);
      onApplyTreeSnapshot(
        createFileManagerTreeSnapshot(nextTree, rootPath, {
          ...fileManagerState?.openState,
          ...nativeTreeRef.current?.openState,
        }),
      );
    },
    [fileManagerState?.openState, onApplyTreeSnapshot, rootPath],
  );
  ...
  const createProjectFile = useCallback(
    (folderPath?: string) => {
      ...
      const nextNode: FileManagerTreeNode = {
        id: nodeId,
        kind: "project-file",
        path: filePath,
      ...
      latestTreeRef.current = insertProjectFileTreeNode(
        latestTreeRef.current,
        folderPath || "",
        nextNode,
      );
  ...
  useEffect(() => {
    setTree((current) => {
      const orderedTree = syncFileTreeToPreviousOrder(
        rawTree,
        orderReferenceTreeRef.current ?? current,
      );
      const nextTree = syncFileTreeToSavedState(
        orderedTree,
  ...
  useEffect(() => {
    registerNodeExpander((nodeId: string) => {
      ...
      window.setTimeout(
        () =>
          onFileManagerStateChange(
            createFileManagerState(latestTreeRef.current, {
              ...fileManagerState?.openState,
              ...api.openState,
            }),
          ),
  ...
  function updateMarqueeSelection(currentX: number, currentY: number) {
    ...
    const api = nativeTreeRef.current;
    if (!start || !api) return;
    const scrollElement = getFileManagerScrollElement(treeRef.current);
    ...
    const nextIds = api.visibleNodes.flatMap((node) => {
      ...
      const rowTop =
        node.rowIndex * FILE_MANAGER_ROW_HEIGHT + FILE_MANAGER_TOP_DROP_PADDING;
    ...
    api.setSelection({
      ids: nextIds,
      anchor: nextIds[0] ?? null,
      mostRecent: nextIds.at(-1) ?? null,
    });
  ...
  function handleMarqueePointerDown(event: ReactPointerEvent<HTMLDivElement>) {
    ...
    const startScrollTop =
      getFileManagerScrollElement(treeRef.current)?.scrollTop ?? 0;
    nativeTreeRef.current?.deselectAll();
    nativeTreeRef.current?.onBlur();
    onSelectNodes([]);
  ...
  function suppressEmptyTreePointerFocus(
  ...
  const applyExternalCompositionDragMove = useCallback(
    (currentMouse: { x: number; y: number }, shiftKey: boolean) => {
      ...
      dispatchExternalCompositionDrag(
        external.node,
        currentMouse,
        "move",
        shiftKey,
  ...
  const ensureExternalCompositionDrag = useCallback(
    (
      compositionNode: Extract<FileManagerTreeNode, { kind: "composition" }>,
  ...
  useEffect(() => {
    ...
    function updateExternalDrag(event: globalThis.DragEvent) {
      ...
      if (
        !externalDragRef.current &&
        isDragEventInsideElement(event, treeRef.current)
      )
        return;
  ...
  function handleRename({ id, name }: { id: string; name: string }) {
    ...
    if (!node) return;
    if (node.kind === "asset-file" || node.kind === "asset-folder")
      onRenameAsset(node.asset.id, name);
    if (node.kind === "project-folder") onRenameFolder(node.path, name);
    if (node.kind === "project-file") onRenameProjectFile(node.path, name);
    if (node.kind === "composition")
      onRenameComposition(node.composition.id, name);
    if (node.kind === "timeline") onRenameTimeline(node.timeline.id, name);
  }

  function handleActivate(
    node: NativeTreeNodeApi<FileManagerTreeNode>,
    event: ReactMouseEvent<HTMLDivElement>,
  ) {
    if (event.shiftKey || event.metaKey || event.ctrlKey) return;
    if (node.data.kind === "timeline" && event.detail >= 2)
      onSelectTimeline(node.data.timeline.id);
    if (node.data.kind === "composition")
      onOpenCompositionFile?.(node.data.composition.id, { temporary: false });
    if (node.data.kind === "project-file")
      onOpenProjectFile?.(node.data.path, { temporary: false });
    if (node.data.kind === "asset-file") {
      const assetPath = getAssetPath(assets, node.data.asset.id, assetsPath);
      if (assetPath) onOpenProjectFile?.(assetPath, { temporary: false });
    }
  }
  ...
  function disableDrop({
    ...
    parentNode: NativeTreeNodeApi<FileManagerTreeNode> | { isRoot: true };
    dragNodes: NativeTreeNodeApi<FileManagerTreeNode>[];
    index: number;
  }) {
    ...
    const siblings = getFileTreeChildren(
      tree,
      parentNodeData?.id ?? null,
    ).filter((node) => !draggedIds.has(node.id));
    const disabled =
      !draggedNodes.length ||
      draggedNodes.some(
        (draggedNode) =>
          !canDropFileTreeNode(draggedNode, parentNodeData, siblings, index),
      );
  ...
  return (
    <div
      ref={treeRef}
      className={`group/filetree relative ${isDroppingRoot ? "bg-[var(--clipper-accent-muted-surface)] shadow-[0_0_0_1px_rgba(255,255,255,0.05)_inset,0_0_0_2px_var(--clipper-accent)]" : ""}`}
      onPointerDownCapture={suppressEmptyTreePointerFocus}
      onPointerDown={handleMarqueePointerDown}
      onPointerMove={handleMarqueePointerMove}
      onPointerUp={finishMarquee}
      onPointerCancel={finishMarquee}
    >
      {marquee ? <FileManagerMarquee marquee={marquee} /> : null}
      <NativeTree<FileManagerTreeNode>
        ref={nativeTreeRef}
        data={tree}
        disableDrop={disableDrop}
        getDropTarget={({ dragIds, localX, localY, tree: api, width }) =>
          getPointerFileTreeDrop(
            api,
            latestTreeRef.current,
            dragIds,
        ...
        height={treeHeight}
        idAccessor="id"
        indent={FILE_MANAGER_INDENT}
        initialOpenState={initialOpenState}
        isInternal={isFileTreeFolderNode}
        movable
        ...
        openByDefault={!fileManagerState?.openState}
        paddingBottom={FILE_MANAGER_BOTTOM_DROP_PADDING}
        paddingTop={FILE_MANAGER_TOP_DROP_PADDING}
        rowHeight={FILE_MANAGER_ROW_HEIGHT}
        width="100%"
        renderDragPreview={(previewProps) => (
          <UnifiedTreeDragPreview
            {...previewProps}
      ...
      >
        {(nodeProps) => {
          ...
          return (
            <UnifiedTreeNode
              {...nodeProps}
              onCreateProjectFile={createProjectFile}
            />
          );
        }}
      </NativeTree>
...
function getFileManagerScrollElement(tree: HTMLElement | null) {
  return tree?.closest<HTMLElement>("[data-file-manager-panel]") ?? null;
}
...
function getFileManagerNodeCopyPath(
  node: FileManagerTreeNode,
  assets: AssetItem[],
  assetsPath: string,
) {
  if (node.kind === "asset-file" || node.kind === "asset-folder")
    return getAssetPath(assets, node.asset.id, assetsPath);
  if (node.kind === "composition") return node.composition.filePath;
  if (node.kind === "timeline")
    return node.timeline.filePath || node.timeline.id;
  return node.path;
}
...
function UnifiedTreeNode({
  ...
  onCreateProjectFile,
  style,
}: NativeTreeNodeRendererProps<FileManagerTreeNode> & {
  onCreateProjectFile: (folderPath?: string) => void;
}) {
  const {
    assets,
    assetsPath,
    ...
    onCreateFolder: onCreateAssetFolder,
    onCreateComposition,
    onCreateCompositionFolder: onCreateFolder,
    onCreateTimeline,
    onDeleteAsset,
    onDeleteComposition,
    onDeleteCompositionFolder: onDeleteFolder,
    onDeleteProjectFile,
    onDeleteTimeline,
    onDuplicateAsset,
  ...
  const displayName =
    data.kind === "composition"
      ? getDisplayNameFromPath(data.composition.filePath)
      : data.kind === "timeline"
        ? getDisplayNameFromPath(data.timeline.filePath || data.timeline.id)
  ...
  function openMenu(event: ReactMouseEvent<HTMLDivElement>) {
    ...
    if (node.isSelected && selectedNodes.length > 1) {
      const selectedCompositions = selectedNodes.filter(
        (
          selectedNode,
        ): selectedNode is Extract<
          FileManagerTreeNode,
          { kind: "composition" }
        > => selectedNode.kind === "composition",
      );
      const selectedAssets = selectedNodes.filter(
        (
          selectedNode,
        ): selectedNode is Extract<
          FileManagerTreeNode,
          { kind: "asset-file" | "asset-folder" }
        > =>
          selectedNode.kind === "asset-file" ||
          selectedNode.kind === "asset-folder",
      );
      const selectedTimelines = selectedNodes.filter(
        (
          selectedNode,
        ): selectedNode is Extract<FileManagerTreeNode, { kind: "timeline" }> =>
          selectedNode.kind === "timeline",
      );
      const canDeleteSelection =
        selectedTimelines.length === 0 ||
        timelines.length - selectedTimelines.length >= 1;
      onOpenMenu({
        x: event.clientX,
        y: event.clientY,
        items: [
          {
            label: `Add ${selectedCompositions.length} to timeline`,
            action: () =>
              selectedCompositions.forEach((selectedNode) =>
                onAddComposition(selectedNode.composition.id),
              ),
            disabled: selectedCompositions.length === 0,
          },
          {
            label: `Copy ${selectedNodes.length} paths`,
            action: () =>
              void copyPathsToClipboard(
                selectedNodes
                  .map((selectedNode) =>
                    getFileManagerNodeCopyPath(
                      selectedNode,
                      assets,
                      assetsPath,
                    ),
          ...
          {
            label: `Duplicate ${selectedAssets.length + selectedCompositions.length} items`,
            action: () => {
              selectedAssets.forEach((selectedNode) =>
                onDuplicateAsset(selectedNode.asset.id),
              );
              selectedCompositions.forEach((selectedNode) =>
                onDuplicateComposition(selectedNode.composition.id),
              );
            },
            disabled: selectedAssets.length + selectedCompositions.length === 0,
          },
          {
            label: `Delete ${selectedNodes.length} items`,
            action: () =>
              deleteFileTreeNodes(selectedNodes, timelines.length, {
                onDeleteAsset,
                onDeleteComposition,
                onDeleteCompositionFolder: onDeleteFolder,
                onDeleteProjectFile,
    ...
    if (data.kind === "asset-file" || data.kind === "asset-folder") {
      const parentFolderId =
        data.kind === "asset-folder"
          ? data.asset.id
          : getParentAssetId(assets, data.asset.id);
      onOpenMenu({
        x: event.clientX,
        y: event.clientY,
        items: [
          { label: "Rename", action: () => node.edit() },
          { label: "Copy path", action: () => onCopyAsset(data.asset.id) },
          { label: "Duplicate", action: () => onDuplicateAsset(data.asset.id) },
          {
            label: "New folder",
            action: () => {
              onCreateAssetFolder(parentFolderId ?? undefined);
              if (parentFolderId)
                requestNodeExpansion(assetNodeId(parentFolderId));
            },
          },
          {
            label: "Sort by",
            children: getAssetSortMenuItems(parentFolderId, onSortAssets),
          },
          {
            label: "Delete",
            action: () => onDeleteAsset(data.asset.id),
            danger: true,
          },
        ],
      });
      ...
      onOpenMenu({
        ...
        items: [
          { label: "New file", action: () => onCreateProjectFile(data.path) },
          {
            ...
            action: () => {
              onCreateComposition(data.path);
              requestNodeExpansion(projectFolderNodeId(data.path));
            },
          },
          {
            label: "New timeline",
            action: () => {
              onCreateTimeline(data.path);
              requestNodeExpansion(projectFolderNodeId(data.path));
            },
          },
          {
          ...
          },
          {
            ...
            action: () =>
              void clipperHost.openAgentTerminal(data.path, agentProvider),
          },
          {
            label: "Copy path",
            action: () => void copyPathsToClipboard([data.path]),
          },
          { label: "Rename", action: () => node.edit() },
          {
            label: "Reveal in Finder",
            action: () => onRevealCompositionFolder(data.path),
          },
          {
            label: "Delete",
            action: () => onDeleteFolder(data.path),
            danger: true,
          },
        ],
      });
      ...
      onOpenMenu({
        ...
        items: [
          { label: "Rename", action: () => node.edit() },
          {
            ...
            action: () =>
              void copyPathsToClipboard([
                data.timeline.filePath || data.timeline.id,
              ]),
          },
          {
            label: "Delete",
            action: () => onDeleteTimeline(data.timeline.id),
            danger: true,
    ...
    if (data.kind === "project-file") {
      onOpenMenu({
        ...
        items: [
          ...
          {
            label: "Delete",
            action: () => onDeleteProjectFile(data.path),
            danger: true,
          },
        ],
      });
      ...
      items: [
        {
          ...
          action: () => onAddComposition(data.composition.id),
        },
        {
          label: "Toggle timeline prerender",
          action: () => onPrerenderComposition?.(data.composition.id),
          disabled: !onPrerenderComposition,
        },
        { label: "Rename", action: () => node.edit() },
        {
          label: "Duplicate",
          action: () => onDuplicateComposition(data.composition.id),
        },
        {
          label: "Copy path",
          action: () => onCopyCompositionPath(data.composition.id),
        },
        {
          label: "Reveal in Finder",
          action: () => onRevealComposition(data.composition.id),
        },
        {
          ...
          action: () =>
            onFindMediaRequestChange?.({
              ...
              fileName:
                data.composition.filePath.split("/").pop() ||
                data.composition.filePath,
            }),
        },
        {
          label: "Composition",
        ...
        {
          label: "Delete",
          action: () => onDeleteComposition(data.composition.id),
          danger: true,
        },
      ],
    });
  ...
  const Icon =
    data.kind === "asset-file" || data.kind === "project-file"
      ? FileIcon
      : data.kind === "timeline"
        ? ChartNoAxesGantt
        : data.kind === "composition"
  ...
  const contentClass =
    data.kind === "composition"
      ? data.composition.sourceMissing
        ? "text-[#7f1d1d]"
        : "text-[#38d996]"
      : "text-current";

  function hideNativeCompositionDragImage(
  ...
  const dropBlockClass = node.willReceiveDropWithin
  ...
  return (
    <div
      ...
      style={style}
      className={`relative box-border grid h-full min-w-0 cursor-pointer select-none grid-cols-[16px_18px_minmax(0,1fr)_auto] items-center gap-1.5 border px-1.5 text-[13px] ${node.isDragging ? "border-[var(--clipper-accent)] bg-[var(--clipper-accent-muted-surface)] opacity-60" : dropBlockClass || (node.isSelected || (node.isFocused && node.tree.hasFocus) ? "border-transparent bg-[#242733]" : "border-transparent hover:bg-[#20232c]")}`}
      onClick={(event) => {
        if (
          !event.metaKey &&
          !event.shiftKey &&
          isFileTreeFolderNode(node.data)
        )
          node.toggle();
      }}
      onContextMenu={openMenu}
      onDragStartCapture={hideNativeCompositionDragImage}
    >
      {node.isInternal ? (
        <button
...
function UnifiedTreeDragPreview({
  hideGhost,
  id,
  isDragging,
  mouse,
  ...
  const displayName =
    node.kind === "composition"
      ? getDragPreviewDisplayName(node.composition.filePath)
      : node.kind === "timeline"
        ? getDragPreviewDisplayName(node.timeline.filePath || node.timeline.id)
...
function dispatchExternalCompositionDrag(
  node: Extract<FileManagerTreeNode, { kind: "composition" }>,
  mouse: { x: number; y: number },
...
function buildProjectFileTree(
  ...
  const root: Extract<ProjectFileTreeNode, { kind: "folder" }> = {
    id: folderNodeId(""),
    kind: "folder",
    path: "",
    name: "",
  ...
  for (const folder of folders)
    ensureCompositionFolder(
      root,
      rootPath,
      relativeCompositionPath(folder, rootPath).split("/").filter(Boolean),
    );
  for (const composition of compositions) {
    const parts = relativeCompositionPath(composition.filePath, rootPath)
      .split("/")
      .filter(Boolean);
    ...
    folder.children.push({
      id: compositionNodeId(composition.id),
      kind: "composition",
      name: getDisplayNameFromPath(composition.filePath),
      composition,
    });
  }
  for (const timeline of timelines) {
    const folderPath = getTimelineFolderPath(timeline, rootPath);
    const parts = relativeCompositionPath(folderPath, rootPath)
      .split("/")
      .filter(Boolean);
    ensureCompositionFolder(root, rootPath, parts).children.push({
      id: timelineNodeId(timeline.id),
      kind: "timeline",
      name: getDisplayNameFromPath(timeline.filePath || timeline.id),
      timeline,
    });
  }
...
function buildUnifiedFileTree(
  ...
  return [
    ...buildProjectFileTree(compositions, folders, rootPath, timelines).map(
      projectNodeToFileNode,
    ),
...
function projectNodeToFileNode(node: ProjectFileTreeNode): FileManagerTreeNode {
  if (node.kind === "folder")
    return {
      id: projectFolderNodeId(node.path),
      kind: "project-folder",
      path: node.path,
      name: node.name,
      children: node.children.map(projectNodeToFileNode),
    };
  if (node.kind === "project-file")
    return {
      id: projectFileNodeId(node.path),
      kind: "project-file",
      path: node.path,
      name: node.name,
    };
  if (node.kind === "composition")
    return {
      id: compositionNodeId(node.composition.id),
      kind: "composition",
      name: getDisplayNameFromPath(node.composition.filePath),
      composition: node.composition,
    };
  return {
    id: timelineNodeId(node.timeline.id),
    kind: "timeline",
    name: getDisplayNameFromPath(node.timeline.filePath || node.timeline.id),
...
function assetToFileNode(asset: AssetItem): FileManagerTreeNode {
  if (asset.kind === "folder")
    return {
      ...
      asset,
      name: asset.name,
      children: (asset.children ?? []).map(assetToFileNode),
    };
  return {
    id: assetNodeId(asset.id),
    kind: "asset-file",
...
function getTimelineFolderPath(timeline: TimelineDocument, rootPath: string) {
  ...
  return directory && timeline.filePath.startsWith(`${rootPath}/`)
    ? directory
    : rootPath;
}

function ensureCompositionFolder(
  root: Extract<ProjectFileTreeNode, { kind: "folder" }>,
  rootPath: string,
  parts: string[],
) {
  let current = root;
  ...
  for (const part of parts) {
    ...
    let next = current.children.find(
      (node): node is Extract<ProjectFileTreeNode, { kind: "folder" }> =>
...
function hasUnifiedFileManagerOpenState(
  openState: Record<string, boolean> | undefined,
) {
  return Boolean(
    openState &&
    Object.keys(openState).some(
      (id) => id.startsWith("project-folder:") || id.startsWith("asset:"),
    ),
  );
}

function isFileTreeFolderNode(
  node: FileManagerTreeNode,
): node is Extract<
  FileManagerTreeNode,
  { kind: "project-folder" | "asset-folder" }
> {
...
function countFileTreeNodes(nodes: FileManagerTreeNode[]): number {
  return nodes.reduce(
    (total, node) =>
      total +
      1 +
      (node.kind === "project-folder" || node.kind === "asset-folder"
        ? countFileTreeNodes(node.children)
        : 0),
    0,
  );
}
...
function getTopLevelSelectedFileTreeNodes(
  nodes: FileManagerTreeNode[],
  selectedIds: string[],
): FileManagerTreeNode[] {
  ...
  for (const node of nodes) {
    ...
    if (node.kind === "project-folder" || node.kind === "asset-folder")
      selectedNodes.push(
        ...getTopLevelSelectedFileTreeNodes(node.children, selectedIds),
      );
  }
  return selectedNodes;
}

function getTopLevelSelectedNodeData(
  selectedNodes: NativeTreeNodeApi<FileManagerTreeNode>[],
): FileManagerTreeNode[] {
  return selectedNodes
    .filter(
      (selectedNode) =>
        !selectedNodes.some(
          (candidate) =>
            candidate !== selectedNode && candidate.isAncestorOf(selectedNode),
        ),
    )
    .map((selectedNode) => selectedNode.data);
}

function deleteFileTreeNodes(
  nodes: FileManagerTreeNode[],
  ...
  const selectedTimelineCount = nodes.filter(
    (node) => node.kind === "timeline",
  ).length;
  const canDeleteTimelines =
    selectedTimelineCount === 0 || timelineCount - selectedTimelineCount >= 1;
  for (const node of nodes) {
    if (node.kind === "asset-file" || node.kind === "asset-folder")
      actions.onDeleteAsset(node.asset.id);
    if (node.kind === "project-folder")
      actions.onDeleteCompositionFolder(node.path);
    if (node.kind === "project-file") actions.onDeleteProjectFile(node.path);
    if (node.kind === "composition")
      actions.onDeleteComposition(node.composition.id);
    if (node.kind === "timeline" && canDeleteTimelines)
      actions.onDeleteTimeline(node.timeline.id);
  }
}

function getFileTreeChildren(
  nodes: FileManagerTreeNode[],
  parentId: string | null,
): FileManagerTreeNode[] {
  if (!parentId) return nodes;
  const parent = findFileTreeNode(nodes, parentId);
  if (parent?.kind === "project-folder" || parent?.kind === "asset-folder")
    return parent.children;
  return [];
}

function canDropFileTreeNode(
  draggedNode: FileManagerTreeNode,
  parentNode: FileManagerTreeNode | null,
  siblings: FileManagerTreeNode[],
  index: number,
) {
  if (
    parentNode &&
    (draggedNode.id === parentNode.id ||
      isFileTreeNodeDescendantOf(draggedNode, parentNode.id))
  )
    return false;

  if (
    draggedNode.kind === "asset-file" ||
    draggedNode.kind === "asset-folder"
  ) {
    if (parentNode && parentNode.kind !== "asset-folder") return false;
    if (parentNode?.kind === "asset-folder") return true;
    return !!getUnifiedAssetReorderTarget(
      siblings.filter((node) => node.id !== draggedNode.id),
      index,
    );
  }
...
function isFileTreeNodeDescendantOf(
  node: FileManagerTreeNode,
  ancestorId: string,
): boolean {
  if (node.kind !== "project-folder" && node.kind !== "asset-folder")
  ...
  return node.children.some(
    (child) =>
      child.id === ancestorId || isFileTreeNodeDescendantOf(child, ancestorId),
  );
}

export function getPointerFileTreeDrop(
  api: NativeTreeApi<FileManagerTreeNode>,
  tree: FileManagerTreeNode[],
  dragIds: string[],
  localX: number,
  localY: number,
  width: number,
): FileManagerDropTarget | null {
  const visibleNodes = api.visibleNodes;
  if (localX < 0 || localX > width) return null;
  const rowBottom =
    FILE_MANAGER_TOP_DROP_PADDING +
    visibleNodes.length * FILE_MANAGER_ROW_HEIGHT;
  if (
    !visibleNodes.length ||
    localY < FILE_MANAGER_TOP_DROP_PADDING ||
    localY > rowBottom
  ) {
    const index = localY < FILE_MANAGER_TOP_DROP_PADDING ? 0 : tree.length;
    return canDropFileTreeTarget(tree, dragIds, null, index)
      ? { dragIds, parentId: null, index }
      : null;
  }
  ...
  const rowIndex = Math.max(
    ...
    Math.min(
      ...
      Math.floor(
        (localY - FILE_MANAGER_TOP_DROP_PADDING) / FILE_MANAGER_ROW_HEIGHT,
      ),
    ),
  );
  ...
  const yInRow =
    localY - FILE_MANAGER_TOP_DROP_PADDING - rowIndex * FILE_MANAGER_ROW_HEIGHT;
  const inTopHalf = yInRow < FILE_MANAGER_ROW_HEIGHT / 2;
  const inMiddle =
    yInRow > FILE_MANAGER_ROW_HEIGHT / 4 &&
    yInRow < FILE_MANAGER_ROW_HEIGHT * 0.75;
  const atTop = !inMiddle && inTopHalf;
  const hoverLevel = Math.round(
    Math.max(0, localX - FILE_MANAGER_INDENT) / FILE_MANAGER_INDENT,
  );
  const drop = getNodePointerDropTarget(
    node,
    hoverLevel,
    inTopHalf,
    inMiddle,
  ...
  if (drop && canDropFileTreeTarget(tree, dragIds, drop.parentId, drop.index))
    return { ...drop, dragIds };
  const fallback = getNearestValidFileTreeDrop(tree, dragIds, drop);
  if (fallback) return { ...fallback, dragIds };
  const visibleFallback = getNearestValidVisibleFileTreeDrop(
    tree,
    dragIds,
    visibleNodes,
    rowIndex,
...
function getNearestValidFileTreeDrop(
  tree: FileManagerTreeNode[],
  dragIds: string[],
  drop: Omit<FileManagerDropTarget, "dragIds"> | null,
): Omit<FileManagerDropTarget, "dragIds"> | null {
  if (!drop) return null;
  ...
  for (let distance = 1; distance <= siblings.length + 1; distance += 1) {
    ...
    if (
      before >= 0 &&
      canDropFileTreeTarget(tree, dragIds, drop.parentId, before)
    )
      return { ...drop, index: before };
    const after = drop.index + distance;
    if (
      after <= siblings.length &&
      canDropFileTreeTarget(tree, dragIds, drop.parentId, after)
    )
      return { ...drop, index: after };
  }
  return null;
}

function getNearestValidVisibleFileTreeDrop(
  tree: FileManagerTreeNode[],
  dragIds: string[],
  visibleNodes: NativeTreeNodeApi<FileManagerTreeNode>[],
  rowIndex: number,
): Omit<FileManagerDropTarget, "dragIds"> | null {
  for (let distance = 0; distance < visibleNodes.length; distance += 1) {
    ...
    if (before && Number.isInteger(before.childIndex)) {
      const drops = [
        {
          parentId: fileTreeNodeParentId(before.parent),
          index: before.childIndex,
        },
      ...
      for (const drop of drops)
        if (
          drop.index >= 0 &&
          canDropFileTreeTarget(tree, dragIds, drop.parentId, drop.index)
        )
          return drop;
    }
    ...
    if (after && Number.isInteger(after.childIndex)) {
      ...
      const drops = [
        {
          parentId: fileTreeNodeParentId(after.parent),
          index: after.childIndex + 1,
        },
...
function getRootFileTreeIndex(tree: FileManagerTreeNode[], nodeId: string) {
  return tree.findIndex(
    (node) => node.id === nodeId || isFileTreeNodeDescendantOf(node, nodeId),
  );
}

function getNodePointerDropTarget(
  node: NativeTreeNodeApi<FileManagerTreeNode>,
  hoverLevel: number,
  inTopHalf: boolean,
  inMiddle: boolean,
  ...
  const [above, below] = getFileTreeNodesAroundCursor(
    node,
    inTopHalf,
    inMiddle,
    atTop,
  ...
  if (above.isLeaf || above.isClosed)
    return walkFileTreeDropUpFrom(
      above,
      clampFileTreeLevel(hoverLevel, below?.level ?? 0, above.level),
    );
  if (above.isOpen && !above.children?.length) {
    const level = clampFileTreeLevel(hoverLevel, 0, above.level + 1);
    return level > above.level
      ? { parentId: above.id, index: 0 }
      : walkFileTreeDropUpFrom(above, level);
  }
  return { parentId: above.id, index: 0 };
}

function getFileTreeNodesAroundCursor(
  node: NativeTreeNodeApi<FileManagerTreeNode>,
  inTopHalf: boolean,
  inMiddle: boolean,
  atTop: boolean,
): [
  NativeTreeNodeApi<FileManagerTreeNode> | null,
  NativeTreeNodeApi<FileManagerTreeNode> | null,
] {
  if (node.isInternal) {
...
function walkFileTreeDropUpFrom(
  node: NativeTreeNodeApi<FileManagerTreeNode>,
  level: number,
): Omit<FileManagerDropTarget, "dragIds"> {
  let drop = node;
  while (drop.parent && drop.level > level) drop = drop.parent;
  return {
    parentId: fileTreeNodeParentId(drop.parent),
    index: drop.childIndex + 1,
  };
}

function fileTreeNodeParentId(
  node: NativeTreeNodeApi<FileManagerTreeNode> | null | undefined,
) {
...
function canDropFileTreeTarget(
  tree: FileManagerTreeNode[],
  dragIds: string[],
  parentId: string | null,
  index: number,
  ...
  const siblings = getFileTreeChildren(tree, parentId).filter(
    (node) => !draggedIdSet.has(node.id),
  );
  return (
    draggedNodes.length === dragIds.length &&
    draggedNodes.every((draggedNode) =>
      canDropFileTreeNode(draggedNode, parentNode, siblings, index),
    )
  );
}

export function moveFileTreeNodesForTest(
  nodes: FileManagerTreeNode[],
  dragIds: string[],
  parentId: string | null,
  index: number,
...
export function createFileManagerTreeSnapshotForTest(
  nodes: FileManagerTreeNode[],
  rootPath: string,
...
function moveFileTreeNodes(
  ...
  const siblingIds = getFileTreeChildren(nodes, parentId).map(
    (node) => node.id,
  );
  const withoutDragged = removeFileTreeNodesForMove(nodes, dragIdSet, removed);
  const insertionIndex = getFileTreeMoveInsertionIndex(
    siblingIds,
    dragIdSet,
    index,
  );
  const insertNodes = renameOnCollision
    ? renameFileTreeNodesForDestination(
        removed,
        getFileTreeChildren(withoutDragged, parentId),
      )
    : removed;
  return insertFileTreeNodesForMove(
    withoutDragged,
    parentId,
    insertionIndex,
    insertNodes,
...
function renameFileTreeNodesForDestination(
  nodes: FileManagerTreeNode[],
  siblings: FileManagerTreeNode[],
): FileManagerTreeNode[] {
  const siblingNames = new Set(
    siblings.map(getFileTreeNodeCollisionName).filter(Boolean),
  );
  return nodes.map((node) =>
    renameFileTreeNodeForDestination(node, siblingNames),
  );
}

function renameFileTreeNodeForDestination(
  node: FileManagerTreeNode,
  ...
  if (node.kind === "project-file") {
    const fileName = nextAvailableFileTreeName(
      getFileName(node.path),
      siblingNames,
    );
    return fileName === getFileName(node.path)
      ? node
      : {
          ...node,
          name: fileName,
          path: replaceFileName(node.path, fileName),
          id: projectFileNodeId(replaceFileName(node.path, fileName)),
        };
  }
  if (node.kind === "composition") {
    const fileName = nextAvailableFileTreeName(
      getFileName(node.composition.filePath),
      siblingNames,
    );
    return fileName === getFileName(node.composition.filePath)
      ? node
      : {
          ...node,
          name: fileName,
          composition: {
            ...node.composition,
            filePath: replaceFileName(node.composition.filePath, fileName),
          },
        };
  }
  const fileName = nextAvailableFileTreeName(
    getFileName(node.timeline.filePath || `${node.timeline.id}.timeline.json`),
    siblingNames,
  );
  return fileName ===
    getFileName(node.timeline.filePath || `${node.timeline.id}.timeline.json`)
    ...
    : {
        ...
        timeline: {
          ...
          filePath: replaceFileName(
            node.timeline.filePath || `${node.timeline.id}.timeline.json`,
            fileName,
          ),
        },
      };
...
function getFileTreeNodeCollisionName(node: FileManagerTreeNode) {
  ...
  if (node.kind === "composition")
    return getFileName(node.composition.filePath);
  return getFileName(
    node.timeline.filePath || `${node.timeline.id}.timeline.json`,
  );
}

export function getFileTreeMoveInsertionIndex(
  siblingIds: string[],
  dragIds: ReadonlySet<string>,
  rawIndex: number,
) {
  const draggedBeforeIndex = siblingIds.filter(
    (id, originalIndex) => dragIds.has(id) && originalIndex < rawIndex,
  ).length;
  return Math.max(0, rawIndex - draggedBeforeIndex);
}

function removeFileTreeNodesForMove(
  nodes: FileManagerTreeNode[],
  dragIds: Set<string>,
  removed: FileManagerTreeNode[],
): FileManagerTreeNode[] {
  return nodes.flatMap((node) => {
    ...
    if (node.kind === "project-folder" || node.kind === "asset-folder")
      return [
        {
          ...node,
          children: removeFileTreeNodesForMove(node.children, dragIds, removed),
        } as FileManagerTreeNode,
      ];
    return [node];
  });
}

function insertFileTreeNodesForMove(
  nodes: FileManagerTreeNode[],
  parentId: string | null,
  index: number,
  insertNodes: FileManagerTreeNode[],
): FileManagerTreeNode[] {
  if (!parentId)
    return [...nodes.slice(0, index), ...insertNodes, ...nodes.slice(index)];
  return nodes.map((node) => {
    if (
      (node.kind === "project-folder" || node.kind === "asset-folder") &&
      node.id === parentId
    ...
    if (node.kind === "project-folder" || node.kind === "asset-folder")
      return {
        ...node,
        children: insertFileTreeNodesForMove(
          node.children,
          parentId,
          index,
          insertNodes,
...
function createFileManagerTreeSnapshot(
  nodes: FileManagerTreeNode[],
  rootPath: string,
  openState?: Record<string, boolean>,
): FileManagerTreeSnapshot {
  const snapshot: FileManagerTreeSnapshot = {
    assets: [],
    compositionFilePaths: {},
    compositionFolders: [],
    compositionOrder: [],
    fileManagerState: createFileManagerState(nodes, openState),
    timelineFilePaths: {},
    timelineOrder: [],
  };
  ...
  for (const node of nodes) {
    if (node.kind === "asset-file" || node.kind === "asset-folder")
      snapshot.assets.push(assetFromFileTreeNode(node, assetSiblingNames));
    else
      collectProjectTreeSnapshot(node, rootPath, snapshot, projectSiblingNames);
  }
  return snapshot;
}

function createFileManagerState(
  nodes: FileManagerTreeNode[],
  openState?: Record<string, boolean>,
): FileManagerState {
  return {
    tree: nodes.map(fileTreeNodeToStateNode),
    openState: openState ?? {},
  };
}

function fileTreeNodeToStateNode(
  node: FileManagerTreeNode,
): FileManagerStateNode {
  if (node.kind === "project-folder" || node.kind === "asset-folder")
    return {
      id: node.id,
      children: node.children.map(fileTreeNodeToStateNode),
    };
  if (node.kind === "project-file") return { id: node.id, filePath: node.path };
  return { id: node.id };
}

function collectProjectTreeSnapshot(
  node: FileManagerTreeNode,
  parentPath: string,
  ...
  if (node.kind === "project-folder") {
    ...
    for (const child of node.children)
      collectProjectTreeSnapshot(child, folderPath, snapshot, childNames);
    return;
  }
  if (node.kind === "project-file") {
    const fileName = nextAvailableFileTreeName(
  ...
  if (node.kind === "composition") {
    ...
    snapshot.compositionFilePaths[node.composition.id] =
      `${parentPath}/${fileName}`;
    return;
  }
  if (node.kind === "timeline") {
...
function nextAvailableFileTreeName(
  fileName: string,
  siblingNames: Set<string>,
) {
...
function assetFromFileTreeNode(
  node: Extract<FileManagerTreeNode, { kind: "asset-file" | "asset-folder" }>,
  siblingNames: Set<string>,
): AssetItem {
  const name = nextAvailableFileTreeName(node.asset.name, siblingNames);
  ...
  return {
    ...
    children: node.children.flatMap((child) =>
      child.kind === "asset-file" || child.kind === "asset-folder"
        ? [assetFromFileTreeNode(child, childNames)]
        : [],
    ),
  };
}

function insertProjectFileTreeNode(
  nodes: FileManagerTreeNode[],
  parentPath: string,
  projectFile: Extract<FileManagerTreeNode, { kind: "project-file" }>,
): FileManagerTreeNode[] {
  if (!parentPath) return [...nodes, projectFile];
  return nodes.map((node) => {
    if (node.kind === "project-folder" && node.path === parentPath)
      return { ...node, children: [...node.children, projectFile] };
    if (node.kind === "project-folder" || node.kind === "asset-folder")
      return {
        ...node,
        children: insertProjectFileTreeNode(
          node.children,
          parentPath,
          projectFile,
        ),
...
function syncFileTreeToSavedState(
  rawNodes: FileManagerTreeNode[],
  stateNodes?: FileManagerStateNode[],
): FileManagerTreeNode[] {
  ...
  for (const stateNode of stateNodes) {
    ...
    const rawNode =
      rawIndex >= 0
        ? remaining.splice(rawIndex, 1)[0]
        : stateNode.filePath
          ? {
              id: projectFileNodeId(stateNode.filePath),
              kind: "project-file" as const,
              path: stateNode.filePath,
              name: getFileName(stateNode.filePath),
            }
          : null;
    if (!rawNode) continue;
    if (
      (rawNode.kind === "project-folder" || rawNode.kind === "asset-folder") &&
      stateNode.children
    )
      next.push({
        ...rawNode,
        children: syncFileTreeToSavedState(
          rawNode.children,
          stateNode.children,
        ),
      } as FileManagerTreeNode);
...
function syncFileTreeToPreviousOrder(
  rawNodes: FileManagerTreeNode[],
  previousNodes: FileManagerTreeNode[],
): FileManagerTreeNode[] {
  ...
  for (const previousNode of previousNodes) {
    const rawIndex = remaining.findIndex((node) =>
      isSameFileTreeNode(node, previousNode),
    );
...
function syncFileTreeNodeToPreviousOrder(
  rawNode: FileManagerTreeNode,
  previousNode: FileManagerTreeNode,
): FileManagerTreeNode {
  if (
    (rawNode.kind === "project-folder" || rawNode.kind === "asset-folder") &&
    (previousNode.kind === "project-folder" ||
      previousNode.kind === "asset-folder")
  ) {
    return {
      ...rawNode,
      children: syncFileTreeToPreviousOrder(
        rawNode.children,
...
function isSameFileTreeNode(
  node: FileManagerTreeNode,
  other: FileManagerTreeNode,
) {
  if (node.id === other.id) return true;
  return (
    node.kind === "project-folder" &&
    other.kind === "project-folder" &&
...
function getUnifiedTimelineReorderTarget(
  siblings: FileManagerTreeNode[],
  index: number,
): { timeline: TimelineDocument; action: "before" | "after" } | null {
  for (let nextIndex = index; nextIndex < siblings.length; nextIndex += 1) {
    const node = siblings[nextIndex];
    if (node.kind === "timeline")
      return { timeline: node.timeline, action: "before" };
  }
  for (let previousIndex = index - 1; previousIndex >= 0; previousIndex -= 1) {
    const node = siblings[previousIndex];
    if (node.kind === "timeline")
      return { timeline: node.timeline, action: "after" };
  }
  return null;
}

function getUnifiedCompositionReorderTarget(
  siblings: FileManagerTreeNode[],
  index: number,
): { composition: CompositionClip; action: "before" | "after" } | null {
  for (let nextIndex = index; nextIndex < siblings.length; nextIndex += 1) {
    const node = siblings[nextIndex];
    if (node.kind === "composition")
      return { composition: node.composition, action: "before" };
  }
  for (let previousIndex = index - 1; previousIndex >= 0; previousIndex -= 1) {
    const node = siblings[previousIndex];
    if (node.kind === "composition")
      return { composition: node.composition, action: "after" };
  }
  return null;
}

function getUnifiedFolderReorderTarget(
  siblings: FileManagerTreeNode[],
  index: number,
): {
  folder: Extract<FileManagerTreeNode, { kind: "project-folder" }>;
  action: "before" | "after";
} | null {
  for (let nextIndex = index; nextIndex < siblings.length; nextIndex += 1) {
    const node = siblings[nextIndex];
    if (node.kind === "project-folder")
      return { folder: node, action: "before" };
  }
  for (let previousIndex = index - 1; previousIndex >= 0; previousIndex -= 1) {
    const node = siblings[previousIndex];
    if (node.kind === "project-folder")
      return { folder: node, action: "after" };
  }
  return null;
}

function getUnifiedAssetReorderTarget(
  siblings: FileManagerTreeNode[],
  index: number,
): { asset: AssetItem; action: "before" | "after" } | null {
  for (let nextIndex = index; nextIndex < siblings.length; nextIndex += 1) {
    const node = siblings[nextIndex];
    if (node.kind === "asset-file" || node.kind === "asset-folder")
      return { asset: node.asset, action: "before" };
  }
  for (let previousIndex = index - 1; previousIndex >= 0; previousIndex -= 1) {
    const node = siblings[previousIndex];
    if (node.kind === "asset-file" || node.kind === "asset-folder")
      return { asset: node.asset, action: "after" };
  }
  return null;
}
...
function getAssetSortMenuItems(
  ...
  return [
    {
      label: "Folders first",
      action: () => onSortAssets(parentFolderId, "folders-first"),
    },
    { label: "A to Z", action: () => onSortAssets(parentFolderId, "name-asc") },
    {
      label: "Z to A",
      action: () => onSortAssets(parentFolderId, "name-desc"),
    },
  ];
}
