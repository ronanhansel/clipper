...
function timelineEdgeIndicatorClass(side: "left" | "right", mended: boolean) {
  ...
  const gradient = mended
    ? "bg-[linear-gradient(180deg,#46f4e6,#1bb8ac)]"
    : "bg-[linear-gradient(180deg,#fb72c6,#db2777)]";
...
export function MotionLane({
  ...
  onEndSelection: (event: PointerEvent<HTMLDivElement>) => void;
  onOpenBlankContextMenu: (event: ReactMouseEvent<HTMLElement>) => void;
  ...
  const motionMarkerEdges = useMemo(() => {
    ...
    for (const timelinePart of timeline) {
      for (const marker of timelinePart.motionMarkers) {
        edges.set(
          timelineBlockPreviewKey("motion", timelinePart.id, marker.id),
          {
            left: isTimelineMarkerMendedEdge(
              timeline,
              timelinePart,
              marker,
              "start",
            ),
            right: isTimelineMarkerMendedEdge(
              timeline,
              timelinePart,
              marker,
              "end",
  ...
  return (
    <TimelineLayerLane
      ...
      onPointerMove={onMoveSelection}
      onPointerUp={onEndSelection}
      ...
      {timeline.flatMap((timelinePart) =>
        timelinePart.motionMarkers
          ...
          .map((marker) => {
            ...
            const rightMended =
              motionMarkerEdges.get(markerKey)?.right ?? false;
            const leftIndicator =
              leftMended || marker.snapIn ? (
                <span
                  className={timelineEdgeIndicatorClass(
                    "left",
                    leftMended && !marker.snapIn,
                  )}
                />
            ...
            return (
              <TimelineBlock
                variant="motion"
                squareLeft={leftMended}
                squareRight={rightMended}
                dataAttributes={{
                  "data-timeline-marker-kind": "motion",
                  "data-timeline-motion-kind": effectKind,
                  "data-timeline-marker-part-id": timelinePart.id,
                  "data-timeline-marker-id": marker.id,
                }}
                locked={locked}
                selected={
                  selectedMotionKeys.has(`${timelinePart.id}:${marker.id}`) ||
                  (marker.id === selectedMotionMarkerId &&
                    timelinePart.id === selectedMotionMarkerPartId)
                }
                key={`${effectKind}-${timelinePart.id}-${marker.id}`}
                style={{
                  left: `${((timelinePart.start + previewMarker.start) / sceneDuration) * 100}%`,
                  width: `calc(${(previewMarker.duration / sceneDuration) * 100}% + var(--clipper-timeline-resize-width, 0px))`,
                }}
                onClick={() => onSelectMotionMarker(timelinePart.id, marker.id)}
                onPointerDown={(event) =>
                  onUpdateMotionFromPointer(event, timelinePart, marker, "move")
                }
                onLeftResize={(event) =>
                  onUpdateMotionFromPointer(
                    event,
                ...
                onRightResize={(event) =>
                  onUpdateMotionFromPointer(event, timelinePart, marker, "end")
                }
                onContextMenu={(event) =>
              ...
              >
                ...
                {marker.kind === "pan" && marker.followId ? (
                  <Link2
                    className="pointer-events-none absolute bottom-1 left-1 text-white/85"
                    size={9}
                    strokeWidth={2.5}
                  />
                ) : null}
...
function getMotionMarkerTimelineLabel(
  marker: { name?: string },
  effectId: string,
) {
  return (
    marker.name?.trim() || getMotionEffectPackage(effectId)?.label || "Motion"
  );
}
