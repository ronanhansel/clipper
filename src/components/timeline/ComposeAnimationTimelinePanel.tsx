...
import {
  ...
  useTimelineRowResize,
} from "./useTimelineRowResize";
...
export const ComposeAnimationTimelinePanel = memo(
  function ComposeAnimationTimelinePanel({
    ...
    const [resizePreviewRowHeights, setResizePreviewRowHeights] =
      useState<Record<string, number> | null>(null);
    const rowHeights =
      resizePreviewRowHeights ?? timelineLayers.rowHeights ?? {};
    const layerRowHeights = layers.map((layer) =>
      getTimelineRowHeight(rowHeights, layer.id),
    );
    const layerRowStarts = layerRowHeights.reduce<number[]>(
      (starts, height, index) => [
        ...starts,
        index === 0 ? 0 : starts[index - 1] + layerRowHeights[index - 1],
      ],
      [],
    );
    const laneRowsStyle = {
      gridTemplateRows:
        layerRowHeights.map((height) => `${height}px`).join(" ") || "58px",
    };
    const laneContentHeight = Math.max(
      layerRowHeights.reduce((total, height) => total + height, 0),
      58,
    );
    const contentWidth =
    ...
    const scrubSnapBoundaries = useMemo(
      () => getComposeAnimationSnapBoundaries(layers, timelineDuration),
      [layers, timelineDuration],
    );
    ...
    const { startTimelineRowResize: startLayerRowResize } =
      useTimelineRowResize({
          ...
          <>
            ...
            {layers.map((layer, index) => (
              <span
                ...
                key={`compose-layer-accent-${layer.id}`}
                style={{
            ...
            {layers.length > 0 ? (
              <LayerResizeSeparator
                key={`compose-label-separator-${layers[0].id}-top`}
                top={0}
                onPointerDown={(event) =>
                  startLayerRowResize(event, layers[0].id, "top")
                }
              />
            ) : null}
            {layers.slice(1).map((layer, index) => (
              <LayerResizeSeparator
                key={`compose-label-separator-${layer.id}`}
                top={layerRowStarts[index + 1]}
                onPointerDown={(event) =>
                  startLayerRowResize(event, layer.id, "top")
                }
              />
            ))}
            {layers.map((layer, index) => (
              <LayerLabel
                ...
                hidden={false}
                locked={false}
                compactControls={layerRowHeights[index] < 44}
                hideLockControl
                name={layer.name}
          ...
          <>
            {layers.map((layer) => (
              <MotionLane
                ...
                hidden={false}
                locked={false}
                timeline={composeMotionTimeline}
                sceneDuration={timelineDuration}
                overflowVisible={false}
                timelineBlockPreviews={timelineBlockPreviews}
                motionSelectionDrag={null}
                motionSelectionBoxRef={{ current: null }}
                selectedMotionKeys={selectedComposeMotionKeys}
                selectedMotionMarkerId={null}
                selectedMotionMarkerPartId={null}
                onEffectDragOver={() => undefined}
                onEffectDrop={() => undefined}
                ...
                onOpenBlankContextMenu={(event) => event.preventDefault()}
                onSelectMotionMarker={(_partId, markerId) => {
                  const animMatch = markerId.match(/^(.+)\/anim\/(.+)$/);
                  const layerId = animMatch ? animMatch[1] : markerId;
                  const selectedLayer = layers.find(
                    (item) => item.id === layerId,
                  );
                  if (selectedLayer) selectLayer(selectedLayer);
                }}
                onOpenNodeContextMenu={(event) => event.preventDefault()}
                onUpdateMotionFromPointer={updateComposeMotionFromPointer}
              />
            ))}
          </>
        )}
