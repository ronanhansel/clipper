...
export function buildDirectTimelineModel({
  mode,
  rowHeights,
  timelineLayers,
}: {
  ...
  const compositionRows = (
    timelineLayers.compositionLayers !== undefined
  ...
  const adjustmentRows = (
    timelineLayers.adjustmentLayers !== undefined
    ...
    return {
      key: layer.id,
      accent: adjustmentTimelineAccent,
      name: layer.name || effect?.label || "Adjustment",
  ...
  const transitionRows = (
    timelineLayers.transitionLayers !== undefined
    ...
    return {
      key: layer.id,
      accent: transitionTimelineAccent,
      name: layer.name || effect?.label || "Transition",
      hidden: Boolean(layer.hidden),
      locked: Boolean(layer.locked),
      effect,
    };
  });
  const directLayerRows: DirectTimelineLayerRow[] = [
    ...
    ...adjustmentRows.map((row) => ({
      key: row.key,
      category: "adjust" as const,
      accent: row.accent,
    })),
    ...motionLayers.map((layer) => ({
      key: layer.id,
      category: "motion" as const,
      accent: motionTimelineAccent,
    })),
  ...
  const directBaseLayerRowHeights = directLayerRows.map((row) =>
    getTimelineRowHeight(rowHeights, row.key),
  );
  const directLayerRowHeightByKey = new Map(
    directLayerRows.map((row, index) => [
      row.key,
  ...
  const layerRowHeights = isCompositionMode
    ...
    : layerRows.map(
        (row) =>
          directLayerRowHeightByKey.get(row.key) ??
          getTimelineRowHeight(rowHeights, row.key),
      );
  ...
  const layerLayout: TimelineLayerLayout = {
    rows: layerRows,
    starts: layerRowStarts,
    heights: layerRowHeights,
  };
  const laneRowsStyle = {
    gridTemplateRows: layerRowHeights.map((height) => `${height}px`).join(" "),
  };
  const laneContentHeight = layerRowHeights.reduce(
