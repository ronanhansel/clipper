import { memo, startTransition, useEffect, useLayoutEffect, useMemo, useRef, useState, type DragEvent, type MouseEvent as ReactMouseEvent, type PointerEvent } from "react";
...
import { buildLinearTimeline, getAdjustmentLayerRowId, getAdjustmentPlacement, getMendedMarkerDragItems, getMotionMarkerLayerId, getScrubSnapBoundaries, getTimelineMarkerDragSnapBoundaries, getTimelinePartAtTime, getTimelineTicks, getTopTimelineItemAtTime, isMotionMarkerOnLayerId, isTimelineMarkerMendedEdge, timelineDisplayDuration as getTimelineDisplayDuration, uniqueTimelineDragItems, type TimelineMarkerDragItem, type TimelineMarkerMove, type TimelineMarkerResize } from "../../core/timeline";
import { getTimelineBlockTiming, getTimelineDragDeltaSeconds, getTimelineGroupMoveTiming, getTimelineSnapGuideTime, type TimelineBlockTimingResult } from "../../core/timelineBlockTiming";
...
import { applyTimelineBlockPreview, clearTimelineBlockPreview, computeBulkLayerTargets, getLayerMoveDragPreview, getTimelineBlockLayerPreview, getTimelineLayerDragPreview, getTimelineLayerRowAtClientY, getTimelineLayerRowAtClientYClamped, moveTimelineStateLayer, renameTimelineStateLayer, resolveTimelineMoveSourceLayer, toggleTimelineStateLayerHidden, toggleTimelineStateLayerLocked, type TimelineLayerCategory } from "../../core/timelineLayers";
import { getAbsoluteTimelineMarkerResizeMarkers, getTimelineMarkerMoveState, getTimelineMarkerResizeCommits, getTimelineMarkerResizePreviewMap, getTimelineMarkerResizeState, uniqueAbsoluteTimelineMarkers, uniqueTimelineResizeTargets, type AbsoluteTimelineMarker, type TimelinePartMotionView } from "../../core/timelineMarkerInteraction";
import type { AdjustmentEffectId, AdjustmentLayer, MotionBlockEffectKind, MotionEffectId, MotionEffectKind, MotionMarker, Part, TimelinePart, TransitionLayer } from "../../core/types";
...
type TimelineBlankGapSelection = { category: TimelineLayerCategory; rowKey: string; start: number; end: number; previewDelta?: number };
type TimelineGapShiftMoves = NonNullable<TimelinePanelProps["onShiftTimelineGapMarkers"]> extends (moves: infer Moves) => void ? Moves : never;
type PendingTimelineGapSlideAnimation = { moves: TimelineGapShiftMoves; fromX: number };

export const DirectTimelinePanel = memo(function DirectTimelinePanel({ timelineName, timeline, motionMarkers = [], timelineLayers, adjustmentLayers, transitionLayers = [], timelineViewportState, mode, selectedPartId, selectedParts, selectedMotionMarkerPartId, selectedMotionMarkerId, selectedMotionMarkers, selectedAdjustmentLayerId, selectedAdjustmentLayers, selectedTransitionLayerId, selectedTransitionLayers = [], sceneDuration, currentSceneTime, isPlaying, playbackPlayheadRef, scrubbingRef, fastSelectEnabled, scrubCommitThrottleMs, defaultNewMarkerDurationSeconds, timelineEndPaddingFraction, timelinePrecision, scrubSnapEnabled, prerenderCacheCoverage, prerenderedCompositionIds, prerenderedCompositionRanges = [], onScrub, onScrubStart, onScrubEnd, onModeChange, onTimelineViewportStateChange, onTimelineLayersChange, onAddCompositionLayer, onRemoveCompositionLayer, onAddAdjustmentLayer, onRemoveAdjustmentLayer, onAddMotionLayer, onRemoveMotionLayer, onSelectPart, onOpenComposePart, onSelectMotionMarker, onSelectMotionMarkers, onSelectAdjustmentLayer, onSelectAdjustmentLayers, onSelectTransitionLayer, onSelectTransitionLayers, onSelectTimelineNodes, onClearTimelineSelection, onOpenNodeContextMenu, onOpenBlankContextMenu, onMoveAdjustmentLayer, onMoveAdjustmentLayers, onUpdateAdjustmentLayer, onReorderPart, onMoveComposition, onMoveCompositions, onUpdateComposition, onMoveMotionMarker, onMoveMotionMarkers, onUpdateMotionMarkers, onResizeMotionMarkers, onAddComposition, onOpenTimeline, onAddAdjustmentEffect, onAddMotionEffect, onAddTransitionEffect, onMoveTransitionLayer, onMoveTransitionLayers, onShiftTimelineGapMarkers, onUpdateTransitionLayer }: TimelinePanelProps) {
  ...
  const [selectedTimelineGap, setSelectedTimelineGap] = useState<TimelineBlankGapSelection | null>(null);
  ...
  const pendingGapSlideAnimationRef = useRef<PendingTimelineGapSlideAnimation | null>(null);
  ...
  const contentWidth = timelineDisplayDuration * defaultTimelinePixelsPerSecond * timelineZoom;
  ...
  function getTimelineContentRect(): DOMRect | null {
    const content = timelineViewportRef.current?.querySelector<HTMLElement>('[data-timeline-content]');
  ...
  function getTimelineLayerContainerRect(): DOMRect | undefined {
    return getTimelineContentRect() ?? timelineViewportRef.current?.firstElementChild?.getBoundingClientRect();
  }
  ...
  function startTimelineSelection(event: PointerEvent<HTMLDivElement>) {
    ...
    event.preventDefault();
    event.currentTarget.setPointerCapture(event.pointerId);
    const rect = getTimelineLayerContainerRect() ?? event.currentTarget.getBoundingClientRect();
    const startContentX = event.clientX - rect.left;
  ...
  function getTimelineBlankGapAtPointer(event: PointerEvent<HTMLDivElement>): TimelineBlankGapSelection | null {
    ...
    const time = clamp(((event.clientX - rect.left) / rect.width) * timelineDisplayDuration, 0, timelineDisplayDuration);
    const markers = getTimelineRowMarkerRanges(row.row.category, row.row.key).sort((left, right) => left.start - right.start || left.end - right.end);
    let previousEnd: number | null = null;
    for (const marker of markers) {
      if (previousEnd !== null && time > previousEnd && time < marker.start) return { category: row.row.category, rowKey: row.row.key, start: previousEnd, end: marker.start };
      previousEnd = previousEnd === null ? marker.end : Math.max(previousEnd, marker.end);
    }
    return null;
  }

  function getTimelineRowMarkerRanges(category: TimelineLayerCategory, rowKey: string) {
    if (category === "transition") return transitionLayers.filter((layer) => (layer.layerId ?? layer.effect.effectId) === rowKey).map((layer) => ({ start: layer.start, end: layer.start + layer.duration }));
    if (category === "adjust") return adjustmentLayers.filter((layer) => getAdjustmentLayerRowId(layer) === rowKey).map((layer) => ({ start: layer.start, end: layer.start + layer.duration }));
    if (category === "motion") return motionTimeline.flatMap((timelinePart) => timelinePart.motionMarkers.filter((marker) => isMotionMarkerOnLayerId(marker, rowKey)).map((marker) => ({ start: timelinePart.start + marker.start, end: timelinePart.start + marker.start + marker.duration })));
    return timeline.filter((part) => (part.layerId ?? "comp") === rowKey).map((part) => ({ start: part.start, end: part.start + part.duration }));
  }

  function startTimelineGapRipple(event: PointerEvent<HTMLDivElement>, gap: TimelineBlankGapSelection) {
    event.preventDefault();
    ...
    function getGapDelta(clientX: number) {
      const rawDelta = getTimelineDragDeltaSeconds({ initialClientX, clientX, initialScrollLeft, scrollLeft: timelineViewportRef.current?.scrollLeft ?? initialScrollLeft, pixelsPerSecond });
      return roundToPrecision(clamp(rawDelta, gap.start - gap.end, 0), timelinePrecision);
    }
    ...
    startTimelinePointerTransaction({
      ...
      onCommit: ({ clientX }) => {
        ...
        if (moves.compositions.length || moves.adjustmentLayers.length || moves.motionMarkers.length || moves.transitionLayers.length) {
        ...
        setSelectedTimelineGap(nextEnd > gap.start ? { ...gap, end: nextEnd } : null);
      },
  ...
  function getTimelineGapShiftMoves(gap: Pick<TimelineBlankGapSelection, "start" | "end">, delta: number): TimelineGapShiftMoves {
    const startsAfterGap = (start: number) => start >= gap.end - 0.000001;
    const nextStart = (start: number) => roundToPrecision(Math.max(0, start + delta), timelinePrecision);
    return {
      gapStart: gap.start,
      gapEnd: gap.end,
      delta,
      compositions: timeline.filter((part) => startsAfterGap(part.start) && !isCompositionLocked(part)).map((part) => ({ compositionId: part.id, start: nextStart(part.start) })),
      adjustmentLayers: adjustmentLayers.filter((layer) => startsAfterGap(layer.start) && !isAdjustmentLocked(layer)).map((layer) => ({ layerId: layer.id, start: nextStart(layer.start) })),
      motionMarkers: motionTimeline.flatMap((timelinePart) => timelinePart.motionMarkers.filter((marker) => startsAfterGap(timelinePart.start + marker.start) && !isMotionMarkerLocked(marker)).map((marker) => ({ markerId: marker.id, start: nextStart(timelinePart.start + marker.start) }))),
      transitionLayers: transitionLayers.filter((layer) => startsAfterGap(layer.start) && !isTransitionLocked(layer)).map((layer) => ({ layerId: layer.id, start: nextStart(layer.start) })),
    };
  }

  function getTimelineGapShiftPreviewMap(gap: TimelineBlankGapSelection, delta: number): TimelineBlockPreviewMap {
    const moves = getTimelineGapShiftMoves(gap, delta);
    return {
      ...Object.fromEntries(moves.compositions.map((move) => {
        const part = timeline.find((item) => item.id === move.compositionId)!;
        return [timelineBlockPreviewKey("composition", part.id), { start: move.start, duration: part.duration }];
      })),
      ...Object.fromEntries(moves.adjustmentLayers.map((move) => {
        const layer = adjustmentLayers.find((item) => item.id === move.layerId)!;
        return [timelineBlockPreviewKey("adjustment", layer.id), { start: move.start, duration: layer.duration }];
      })),
      ...Object.fromEntries(moves.motionMarkers.map((move) => {
        const marker = motionTimeline.flatMap((timelinePart) => timelinePart.motionMarkers.map((item) => ({ timelinePart, marker: item }))).find((item) => item.marker.id === move.markerId)!;
        return [timelineBlockPreviewKey("motion", marker.timelinePart.id, marker.marker.id), { start: move.start - marker.timelinePart.start, duration: marker.marker.duration }];
      })),
      ...Object.fromEntries(moves.transitionLayers.map((move) => {
        const layer = transitionLayers.find((item) => item.id === move.layerId)!;
        return [timelineBlockPreviewKey("transition", layer.id), { start: move.start, duration: layer.duration, midPoint: layer.midPoint }];
      })),
    };
  }
  ...
  function closeSelectedTimelineGap(gap: TimelineBlankGapSelection) {
    ...
    const moves = getTimelineGapShiftMoves(gap, delta);
    if (!(moves.compositions.length || moves.adjustmentLayers.length || moves.motionMarkers.length || moves.transitionLayers.length)) return false;
    clearTimelineBlockPreviews();
    queueTimelineGapSlideAnimation(moves, delta);
    onShiftTimelineGapMarkers?.(moves);
    setSelectedTimelineGap(null);
  ...
  function queueTimelineGapSlideAnimation(moves: TimelineGapShiftMoves, delta: number) {
    const pixelsPerSecond = (timelineRef.current?.getBoundingClientRect().width ?? 1) / Math.max(timelineDisplayDuration, 1);
    pendingGapSlideAnimationRef.current = { moves, fromX: -delta * pixelsPerSecond };
  }

  function getTimelineCompositionElement(partId: string) {
    return timelineViewportRef.current?.querySelector<HTMLElement>(`[data-timeline-composition-id="${CSS.escape(partId)}"]`) ?? null;
  }
  ...
  function runPendingTimelineGapSlideAnimation() {
    ...
    const elements = [
      ...pending.moves.compositions.map((move) => getTimelineCompositionElement(move.compositionId)),
      ...pending.moves.adjustmentLayers.map((move) => getTimelineAdjustmentElement(move.layerId)),
      ...pending.moves.motionMarkers.map((move) => getTimelineMarkerElement(TIMELINE_MOTION_PART_ID, move.markerId)),
      ...pending.moves.transitionLayers.map((move) => getTimelineTransitionElement(move.layerId)),
  ...
  useEffect(() => {
    ...
    function closeGapFromKeyboard(event: KeyboardEvent) {
      ...
      if (editable && !(editable instanceof HTMLInputElement && editable.type === "range")) return;
  ...
  }, [selectedTimelineGap, timeline, adjustmentLayers, motionTimeline, transitionLayers, timelinePrecision]);
  ...
  type MixedTimelineMoveItem = { id: string; kind: "composition" | "adjustment" | "transition" | "motion"; start: number; duration: number; layerId: string; partId?: string; markerId?: string };
  ...
  function getSelectedMixedTimelineMoveItems() {
    ...
    for (const selection of selectedParts) {
      ...
      if (part && !isCompositionLocked(part)) items.push({ id: part.id, kind: "composition", start: part.start, duration: part.duration, layerId: part.layerId ?? "comp" });
    }
    for (const selection of selectedAdjustmentLayers) {
      const layer = adjustmentLayers.find((item) => item.id === selection.layerId);
      if (layer && !isAdjustmentLocked(layer)) items.push({ id: layer.id, kind: "adjustment", start: layer.start, duration: layer.duration, layerId: getAdjustmentLayerRowId(layer) });
    }
    for (const selection of selectedTransitionLayers) {
      const layer = transitionLayers.find((item) => item.id === selection.layerId);
      if (layer && !isTransitionLocked(layer)) items.push({ id: layer.id, kind: "transition", start: layer.start, duration: layer.duration, layerId: layer.layerId ?? layer.effect.effectId });
    }
    for (const selection of selectedMotionMarkers) {
      ...
      if (part && marker && !isMotionMarkerLocked(marker)) items.push({ id: `${part.id}:${marker.id}`, kind: "motion", partId: part.id, markerId: marker.id, start: part.start + marker.start, duration: marker.duration, layerId: getMotionMarkerLayerId(marker) });
    }
    return items;
  }

  function selectedMixedTimelineMoveItems(kind: MixedTimelineMoveItem["kind"], id: string) {
    const items = getSelectedMixedTimelineMoveItems();
    if (items.length <= 1 || !items.some((item) => item.kind === kind && item.id === id)) return [];
    return items;
  }
  ...
  function getAbsoluteMarkerResizeState<T extends { id: string; start: number; duration: number; snapIn?: boolean; snapOut?: boolean }>(markers: Array<AbsoluteTimelineMarker<T>>, markerId: string, sourcePartId: string, action: "start" | "end", deltaSeconds: number) {
    return getTimelineMarkerResizeState({ markers, markerId, sourcePartId, action, deltaSeconds, precision: timelinePrecision });
  }
  ...
  function getTimelineMarkerElement(partId: string, markerId: string) {
    return timelineViewportRef.current?.querySelector<HTMLElement>(`[data-timeline-marker-kind="motion"][data-timeline-marker-part-id="${CSS.escape(partId)}"][data-timeline-marker-id="${CSS.escape(markerId)}"]`) ?? null;
  }
  ...
  function getDropLayerId(category: TimelineLayerCategory, clientY: number) {
    const rect = getTimelineLayerContainerRect();
    const target = getTimelineLayerRowAtClientYClamped(layerLayout, rect, clientY, category)?.row.key;
  ...
  function getLayerDragPreview(category: TimelineLayerCategory, sourceLayerId: string | undefined, clientY: number) {
    const preview = getTimelineBlockLayerPreview(layerLayout, category, sourceLayerId, clientY, getTimelineLayerContainerRect());
  ...
  function isCompositionBlocked(start: number, duration: number, layerId: string, compositionIdsToExclude?: Set<string>): boolean {
  ...
  function startMixedTimelineMove(event: PointerEvent<HTMLElement>, anchor: MixedTimelineMoveItem, items: MixedTimelineMoveItem[]) {
    event.preventDefault();
    event.stopPropagation();
    setSelectedTimelineGap(null);
    const initialClientX = event.clientX;
    ...
    const pixelsPerSecond = (timelineRef.current?.getBoundingClientRect().width ?? 1) / Math.max(timelineDisplayDuration, 1);
    const snapThresholdSeconds = Math.max(0.08, 8 / pixelsPerSecond);
    const compositionIds = new Set(items.filter((item) => item.kind === "composition").map((item) => item.id));
    const adjustmentIds = new Set(items.filter((item) => item.kind === "adjustment").map((item) => item.id));
    const transitionIds = new Set(items.filter((item) => item.kind === "transition").map((item) => item.id));
    const boundaries = getUniversalBlockSnapBoundaries({ excludeCompositionIds: compositionIds, excludeAdjustmentIds: adjustmentIds, excludeTransitionIds: transitionIds });
    ...
    function getMoveDelta(clientX: number, snap: boolean) {
      const timing = getTimelineGroupMoveTiming({ items, anchorStart: anchor.start, deltaSeconds: getDeltaSeconds(clientX), timelineDuration: timelineDisplayDuration, moveMaxStartMode: "start", snap, snapBoundaries: boundaries, snapThresholdSeconds, precision: timelinePrecision });
    ...
    function getNextItems(clientX: number, snap: boolean) {
      ...
      return items.map((item) => ({ ...item, start: roundToPrecision(item.start + delta, timelinePrecision) }));
    }

    function getMotionElement(item: MixedTimelineMoveItem) {
      return item.partId && item.markerId ? getTimelineMarkerElement(item.partId, item.markerId) : null;
    }

    function getElement(item: MixedTimelineMoveItem) {
      if (item.kind === "composition") return getTimelineCompositionElement(item.id);
      if (item.kind === "adjustment") return getTimelineAdjustmentElement(item.id);
      if (item.kind === "transition") return getTimelineTransitionElement(item.id);
    ...
    function applyDrag(clientX: number, snap: boolean) {
      ...
      for (const item of items) {
        const nextItem = nextItems.find((next) => next.id === item.id && next.kind === item.kind);
        const element = getElement(item);
        if (!nextItem || !element) continue;
        applyTimelineBlockPreview(element, { deltaX: (nextItem.start - item.start) * pixelsPerSecond });
      }
    }
    ...
    startTimelinePointerTransaction({
      ...
      updateAutoScroll: updateTimelineDragAutoScroll,
      stopAutoScroll: stopTimelineDragAutoScroll,
      onDragStart: () => { setGlobalTimelineDragActive(true); setDraggingTimelineBlockCategory(anchor.kind === "composition" ? "comp" : anchor.kind === "adjustment" ? "adjust" : anchor.kind === "transition" ? "transition" : "motion"); },
      onPreview: ({ clientX, snap }) => applyDrag(clientX, snap),
      onCommit: ({ clientX, snap }) => {
        const nextItems = getNextItems(clientX, snap);
        const compositionMoves = nextItems.filter((item) => item.kind === "composition").map((item) => ({ compositionId: item.id, start: item.start, targetLayerId: item.layerId }));
        const adjustmentMoves = nextItems.filter((item) => item.kind === "adjustment").map((item) => ({ layerId: item.id, start: item.start, targetLayerId: item.layerId }));
        const transitionMoves = nextItems.filter((item) => item.kind === "transition").map((item) => ({ layerId: item.id, start: item.start, targetLayerId: item.layerId }));
        const motionMoves: TimelineMarkerMove[] = nextItems.flatMap((item) => {
          if (item.kind !== "motion" || !item.partId || !item.markerId) return [];
          if (item.partId === TIMELINE_MOTION_PART_ID) return [{ sourcePartId: item.partId, markerId: item.markerId, targetPartId: item.partId, start: item.start, targetLayerId: item.layerId }];
          const timelinePart = timeline.find((part) => part.id === item.partId);
          return timelinePart ? [{ sourcePartId: item.partId, markerId: item.markerId, targetPartId: item.partId, start: roundToPrecision(item.start - timelinePart.start, timelinePrecision), targetLayerId: item.layerId }] : [];
        });
        ...
        if (adjustmentMoves.length) {
          ...
          else for (const move of adjustmentMoves) onMoveAdjustmentLayer(move.layerId, move.start, move.targetLayerId);
        }
        if (transitionMoves.length) {
          if (onMoveTransitionLayers) onMoveTransitionLayers(transitionMoves);
          else for (const move of transitionMoves) onMoveTransitionLayer?.(move.layerId, move.start, move.targetLayerId);
        }
  ...
  function updateCompositionFromPointer(event: PointerEvent<HTMLElement>, composition: TimelinePart, action: "move" | "start" | "end") {
    if (action === "move") {
      const mixedItems = selectedMixedTimelineMoveItems("composition", composition.id);
      if (mixedItems.length > 1) {
        const anchor = mixedItems.find((item) => item.kind === "composition" && item.id === composition.id);
        if (anchor) {
    ...
    if (isSelectionMove) {
      ...
      function getNextMoveTargets(clientX: number, snap: boolean) {
        ...
        const timing = getTimelineGroupMoveTiming({ items: moveTargets, anchorStart: composition.start, deltaSeconds: getDeltaSeconds(clientX), timelineDuration: timelineDisplayDuration, moveMinStart: composition.start - minStart, moveMaxStartMode: "start", snap, snapBoundaries: boundaries, snapThresholdSeconds, precision: timelinePrecision });
        updateTimelineSnapGuide(timing.guideTime);
        const moveDelta = timing.deltaSeconds;
        return moveTargets.map((target) => ({ ...target, start: target.start + moveDelta }));
      }
      function isAnyLayerBlocked(nextTargets: TimelinePart[], layerTargets: Map<string, string>): boolean {
        return nextTargets.some((nextTarget) => {
          const rowKey = layerTargets.get(nextTarget.id) ?? nextTarget.layerId ?? "comp";
          return isCompositionBlocked(nextTarget.start, nextTarget.duration, rowKey, moveTargetIds);
        });
      }
      function applyDrag(clientX: number, clientY: number, snap: boolean) {
        const nextTargets = getNextMoveTargets(clientX, snap);
        const containerRect = getTimelineLayerContainerRect();
        const cursorLayerId = getTimelineLayerRowAtClientYClamped(layerLayout, containerRect, clientY, "comp")?.row.key;
        ...
        previewTimelineBlocks(previewMapFromBlocks("composition", nextTargets.map((t) => ({ ...t, blocked } as any))));
        for (const target of moveTargets) {
          ...
          applyTimelineBlockPreview(targetElement, { deltaX: 0, deltaY: layerPreview.deltaY, height: layerPreview.height, resizeProperty: "--clipper-composition-resize-width", blocked });
        }
      }
      function clearDragState() {
        setGlobalTimelineDragActive(false);
    ...
    startBlockPointerInteraction({
      ...
      transformTiming: (timing) => {
        if (action !== "start") return { ...composition, start: timing.start, duration: timing.duration };
        const previousTrimStart = composition.trimStart ?? 0;
        const trimStart = roundToPrecision(Math.max(previousTrimStart + timing.start - composition.start, 0), timelinePrecision);
        const start = roundToPrecision(composition.start + trimStart - previousTrimStart, timelinePrecision);
        return { ...composition, start, duration: roundToPrecision(composition.start + composition.duration - start, timelinePrecision), trimStart: trimStart || undefined };
      },
      ...
      onResize: (next) => onUpdateComposition(composition.id, () => ({ ...composition, start: next.start, duration: next.duration, trimStart: next.trimStart })),
      precision: timelinePrecision,
    });
  }
  ...
  function updateAdjustmentFromPointer(event: PointerEvent<HTMLElement>, layer: AdjustmentLayer, action: "move" | "start" | "end") {
    if (action === "move") {
      ...
      if (mixedItems.length > 1) {
        const anchor = mixedItems.find((item) => item.kind === "adjustment" && item.id === layer.id);
        if (anchor) {
          startMixedTimelineMove(event, anchor, mixedItems);
          return;
        }
    ...
    if (isSelectionMove) {
      ...
      function getNextLayer(targetLayer: AdjustmentLayer, cx: number, snap: boolean) {
        const timing = getTimelineGroupMoveTiming({ items: resizeTargets, anchorStart: layer.start, deltaSeconds: getDeltaSeconds(cx), timelineDuration: timelineDisplayDuration, moveMaxStartMode: "start", snap, snapBoundaries: boundaries, snapThresholdSeconds, precision: timelinePrecision });
        updateTimelineSnapGuide(timing.guideTime);
        return { ...targetLayer, start: targetLayer.start + timing.deltaSeconds, duration: targetLayer.duration };
      }
      ...
      function applyDrag(cx: number, cy: number, snap: boolean) {
        ...
        const containerRect = getTimelineLayerContainerRect();
        const cursorLayerId = getTimelineLayerRowAtClientYClamped(layerLayout, containerRect, cy, "adjust")?.row.key;
      ...
      startTimelinePointerTransaction({
        ...
        onCommit: ({ clientX, clientY, snap }) => {
          const containerRect = getTimelineLayerContainerRect();
          const cursorLayerId = getTimelineLayerRowAtClientYClamped(layerLayout, containerRect, clientY, "adjust")?.row.key;
          const layerTargets = computeBulkLayerTargets(layerLayout, "adjust", sourceLayerId, cursorLayerId, resizeTargets.map((l) => ({ id: l.id, layerId: getAdjustmentLayerRowId(l) })), sourceLayerId);
  ...
  function updateTransitionFromPointer(event: PointerEvent<HTMLElement>, layer: TransitionLayer, action: "move" | "start" | "end") {
    ...
    if (action === "move") {
      ...
      if (mixedItems.length > 1) {
        const anchor = mixedItems.find((item) => item.kind === "transition" && item.id === layer.id);
        if (anchor) {
          startMixedTimelineMove(event, anchor, mixedItems);
          return;
        }
    ...
    if (isSelectionMove) {
      ...
      function getNextLayer(targetLayer: TransitionLayer, cx: number, snap: boolean) {
        const timing = getTimelineGroupMoveTiming({ items: resizeTargets, anchorStart: layer.start, deltaSeconds: getDeltaSeconds(cx), timelineDuration: timelineDisplayDuration, moveMaxStartMode: "start", snap, snapBoundaries: boundaries, snapThresholdSeconds, precision: timelinePrecision });
        updateTimelineSnapGuide(timing.guideTime);
        return { ...targetLayer, start: targetLayer.start + timing.deltaSeconds };
      }
      ...
      function applyDrag(cx: number, cy: number, snap: boolean) {
        ...
        const containerRect = getTimelineLayerContainerRect();
        const cursorLayerId = getTimelineLayerRowAtClientYClamped(layerLayout, containerRect, cy, "transition")?.row.key;
      ...
      startTimelinePointerTransaction({
        ...
        onCommit: ({ clientX, clientY, snap }) => {
          ...
          const containerRect = getTimelineLayerContainerRect();
          const cursorLayerId = getTimelineLayerRowAtClientYClamped(layerLayout, containerRect, clientY, "transition")?.row.key;
          const layerTargets = computeBulkLayerTargets(layerLayout, "transition", sourceLayerId, cursorLayerId, resizeTargets.map((l) => ({ id: l.id, layerId: l.layerId ?? l.effect.effectId })), sourceLayerId);
  ...
  function updateMotionMarkerFromPointer(event: PointerEvent<HTMLDivElement>, part: TimelinePart, marker: MotionMarker, action: "move" | "start" | "end") {
    ...
    event.stopPropagation();
    if (action === "move") {
      const mixedItems = selectedMixedTimelineMoveItems("motion", `${part.id}:${marker.id}`);
      if (mixedItems.length > 1) {
        const anchor = mixedItems.find((item) => item.kind === "motion" && item.id === `${part.id}:${marker.id}`);
        if (anchor) {
          startMixedTimelineMove(event, anchor, mixedItems);
          return;
        }
    ...
    function getMoveDragState(clientX: number, clientY: number, snap: boolean) {
      ...
      const state = getTimelineMarkerMoveState({
        ...
        timelineDuration: timelineDisplayDuration,
        snap,
        snapBoundaries: getUniversalTimelineSnapBoundaries(motionKind, movingKeys),
        snapThresholdSeconds,
        motionKind,
        activePartIds,
        layerLayout,
    ...
    function commitResizeDrag(clientX: number, snap: boolean) {
      onResizeMotionMarkers(getTimelineMarkerResizeCommits(getResizeMarkers(clientX, snap), timelinePrecision));
    }
    ...
    function applyDrag(clientX: number, clientY: number, snap: boolean) {
      if (action === "move") {
        ...
        for (const item of dragItems) {
          ...
          const normalizedItemSourceLayer = resolveTimelineMoveSourceLayer(layerLayout, "motion", itemSourceLayer);
          const layerPreview = getLayerMoveDragPreview(layerLayout, normalizedItemSourceLayer, targetLayerId, isLayerLocked, "motion");
    ...
    function clearMotionMarkerDragState() {
      ...
      setDraggingTimelineBlockCategory(null);
      clearTimelineSnapGuide();
      if (action === "move") window.requestAnimationFrame(() => clearTimelineMarkerDragTransforms(dragItems));
      else {
        clearTimelineBlockPreviews();
      }
    }
  ...
  function handleCompositionNativeDragOver(event: DragEvent<HTMLDivElement>) {
    ...
    const hasCompositionDrag = event.dataTransfer.types.includes("application/x-clipper-composition") || isCompositionDragText(textDrag) || Boolean(getActiveCompositionPointerDrag());
  ...
  function handleCompositionNativeDrop(event: DragEvent<HTMLDivElement>) {
    if ((event.nativeEvent as globalThis.DragEvent & { __clipperCompositionDropHandled?: boolean }).__clipperCompositionDropHandled) return;
    ...
    const compositionId = event.dataTransfer.getData("application/x-clipper-composition") || activeCompositionDrag?.compositionId || event.dataTransfer.getData("text/plain") || "";
    const target = getCompositionDropTargetFromEvent(event);
    ...
    event.preventDefault();
    event.stopPropagation();
    if (activeCompositionDrag) previewCompositionDrop(activeCompositionDrag, target.layerId, target.sceneTime);
    flushEffectDragPreview();
    const preview = effectDragPreviewRef.current;
    const sceneTime = preview?.category === "composition" && preview.layerKey === target.layerId && !preview.blocked ? preview.start : target.sceneTime;
    setGlobalTimelineDragActive(false);
    setClipperPointerDragPreview(compositionDragPreviewEvent, false);
    clearTimelineSnapGuide();
    stopTimelineDragAutoScroll();
    setActiveCompositionPointerDrag(null);
    if (isCompositionBlocked(sceneTime, 0.1, target.layerId, new Set())) { requestAnimationFrame(() => updateEffectDragPreview(null)); return; }
  ...
  function dropTransitionEffect(event: DragEvent<HTMLElement>, layerId: string, sceneTime = getDropSceneTime(event)) {
    ...
    if (blocked) { requestAnimationFrame(() => updateEffectDragPreview(null)); return; }
  ...
  function handleSharedPointerDrag<T extends EffectPointerDragDetail | CompositionPointerDragDetail>(
    detail: T | null | undefined,
    eventName: string,
    previewEventName: string | null,
    resolveTarget: (target: { rowKey: string; sceneTime: number }) => { valid: boolean; handlePreview: () => void; handleDrop: () => void } | null
  ) {
    if (!detail) return;
  ...
  useEffect(() => {
    function handleEffectPointerDrag(event: Event) {
      ...
      handleSharedPointerDrag(detail, effectPointerDragEvent, effectDragPreviewEvent, (target) => {
        ...
        if (transitionEffect && transitionRow) {
          return {
            ...
            handlePreview: () => previewTransitionEffectDrop(transitionEffect.id, transitionRow.key, detail.clientX, target.sceneTime, detail.shiftKey),
            handleDrop: () => {
        ...
        if (adjustmentEffect && adjustmentRow) {
          return {
            ...
            handlePreview: () => previewAdjustmentEffectDrop(adjustmentEffect.id, adjustmentRow.key, detail.clientX, target.sceneTime, detail.shiftKey),
            handleDrop: () => {
  ...
  useEffect(() => {
    function handleCompositionPointerDrag(event: Event) {
      ...
      handleSharedPointerDrag(detail, compositionPointerDragEvent, compositionDragPreviewEvent, (target) => {
        const compositionRow = compositionRows.find((row) => row.id === target.rowKey);
        if (compositionRow) {
          return {
            valid: !compositionRow.locked,
            handlePreview: () => previewCompositionDrop(detail, compositionRow.id, target.sceneTime),
            handleDrop: () => {
              const preview = effectDragPreviewRef.current;
              const targetStart = preview?.category === "composition" && preview.layerKey === compositionRow.id && !preview.blocked ? preview.start : target.sceneTime;
              setActiveCompositionPointerDrag(null);
              if (preview?.category === "composition" && preview.blocked) return;
  ...
  return <TimelineShell activeMode={mode} contentWidth={contentWidth} currentTime={currentSceneTime} displayDuration={timelineDisplayDuration} dragActive={timelineDragActive || timelineFileDragActive} dragOverlayLabel={timelineFileDragActive ? "Open timeline" : undefined} laneContentHeight={laneContentHeight} laneRowsStyle={laneRowsStyle} layerRailWidth={layerRailWidth} prerenderCacheCoverage={prerenderCacheCoverage} refs={{ playbackPlayheadRef, timelineRef, timelineViewportRef, timelineLayerRailRef, timelineSnapGuideRef, timelinePanelRef }} timelineName={timelineName} timelineZoom={timelineZoom} ticks={ticks} onModeChange={onModeChange} onTimelineViewportDragLeave={handleTimelineViewportDragLeave} onTimelineViewportDragOver={handleCompositionNativeDragOver} onTimelineViewportDrop={handleCompositionNativeDrop} onTimelineViewportScroll={saveTimelineDisplacement} onTimelineZoomChange={updateTimelineZoom} rulerHandlers={{ onPointerDown: startScrub, onPointerMove: continueScrub, onPointerUp: endScrub, onPointerCancel: endScrub }} renderLayerRail={() => <>
            ...
            {selectedTimelineGap ? <TimelineBlankGapHighlight gap={selectedTimelineGap} rowIndex={layerRows.findIndex((row) => row.key === selectedTimelineGap.rowKey && row.category === selectedTimelineGap.category)} rowStarts={layerRowStarts} rowHeights={layerRowHeights} timelineDisplayDuration={timelineDisplayDuration} /> : null}
            ...
            {compositionRows.map((row) => <TimelineLayerLane key={row.id} hidden={Boolean(row.hidden)} locked={Boolean(row.locked)} overflowVisible={timelineMarkersEditable && isDraggingCompositionBlock} className="block" onDragOver={(event) => { if (timelineMarkersEditable && !row.locked && (event.dataTransfer.types.includes("application/x-clipper-composition") || isCompositionDragText(event.dataTransfer.getData("text/plain")) || getActiveCompositionPointerDrag())) event.preventDefault(); }} onDrop={handleCompositionNativeDrop} onPointerDown={startTimelineSelection} onPointerMove={continueTimelineSelection} onPointerUp={endTimelineSelection} onPointerCancel={endTimelineSelection} onContextMenu={(event) => openBlankTimelineContextMenu(event, row.id)}>
...
function TimelineBlankGapHighlight({ gap, rowIndex, rowStarts, rowHeights, timelineDisplayDuration }: { gap: TimelineBlankGapSelection; rowIndex: number; rowStarts: number[]; rowHeights: number[]; timelineDisplayDuration: number }) {
  ...
  return <div className="pointer-events-none absolute z-30 rounded-[3px] border border-[var(--clipper-accent)] bg-[rgb(var(--clipper-accent-rgb)/0.16)] shadow-[inset_0_0_0_1px_rgb(255_255_255/0.08),0_0_18px_rgb(var(--clipper-accent-rgb)/0.18)]" style={{ left: `${(start / timelineDisplayDuration) * 100}%`, top: rowStarts[rowIndex], width: `${((end - start) / timelineDisplayDuration) * 100}%`, height: rowHeights[rowIndex] }} />;
}
