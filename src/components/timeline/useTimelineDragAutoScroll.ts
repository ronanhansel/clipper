...
export type TimelineDragAutoScrollOptions = {
  ...
  getTimelineVerticalEdgeScrollDelta?: (clientY: number, viewport: HTMLElement) => number;
  onRulerScroll: (scrollLeft: number) => void;
  onScrollPersist?: () => void;
};

export function useTimelineDragAutoScroll({ viewportRef, getTimelineEdgeScrollDelta, getTimelineVerticalEdgeScrollDelta = getDefaultVerticalEdgeScrollDelta, onRulerScroll, onScrollPersist }: TimelineDragAutoScrollOptions) {
  ...
  function scheduleTimelineDragAutoScroll() {
    ...
    const tick = () => {
      ...
      const verticalScrollDelta = verticalEnabledRef.current ? getTimelineVerticalEdgeScrollDelta(clientY, viewport) : 0;
  ...
  function updateTimelineDragAutoScroll(clientX: number, onScroll: () => void, clientY?: number) {
...
function getDefaultVerticalEdgeScrollDelta(clientY: number, viewport: HTMLElement) {
  const rect = viewport.getBoundingClientRect();
  const edgeSize = Math.min(36, Math.max(20, rect.height * 0.1));
  const maxDelta = 10;
  if (clientY < rect.top + edgeSize) return -Math.ceil(((rect.top + edgeSize - clientY) / edgeSize) * maxDelta);
  if (clientY > rect.bottom - edgeSize) return Math.ceil(((clientY - (rect.bottom - edgeSize)) / edgeSize) * maxDelta);
  return 0;
}
