...
const previewRenderResolutionLabels = new Map(
  previewRenderHeightOptions.map((height) => [
    height,
...
const stableSlowGridPresetDescriptions: Record<StableSlowGridPreset, string> = {
  relaxed:
    "Tile sizes tried in order after failures; full means full-width strips: fullx540 -> fullx360 -> fullx270 -> fullx180 -> 3840x180 -> 2560x135 -> 2048x90.",
  balanced:
    "Tile sizes tried in order after failures; starts full-width then narrows: fullx270 -> fullx180 -> 2560x180 -> 2048x135 -> 1536x90 -> 1024x54 -> 768x36.",
  ...
  extreme:
    "Tile sizes tried in order after failures; smallest grids for low-memory exports: 1024x54 -> 768x36 -> 512x24 -> 384x16 -> 256x12.",
};
...
export function SettingsDialog({
  ...
  function updateExportWorkerCount(
    key: keyof ExportWorkerResolutionMapping,
    value: string,
  ) {
    const parsed = Number(value);
    if (!Number.isFinite(parsed)) return;
  ...
  function updateExportTileCount(
    key: keyof ExportTileResolutionMapping,
    value: string,
  ) {
    const parsed = Number(value);
    if (!Number.isFinite(parsed)) return;
  ...
  const updateActionLabel =
    updateStatus.kind === "downloaded" ? "Install and restart" : "Download";
  const updateActionEnabled =
    updateStatus.kind === "available" || updateStatus.kind === "downloaded";
  const runUpdateAction =
    updateStatus.kind === "downloaded" ? onInstallUpdate : onDownloadUpdate;

  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        className="h-[min(680px,calc(100vh-56px))] w-[min(980px,calc(100vw-42px))] gap-0 overflow-hidden p-0"
          ...
          <aside className="border-r border-[#2d313b] bg-[#15171e] p-3">
            <nav className="grid gap-1" aria-label="Settings sections">
              {navItems.map((item) => (
                <button
                  key={item.id}
                  className={`rounded-[9px] px-3 py-2.5 text-left text-xs font-extrabold transition ${activeSection === item.id ? "bg-[#0f1117] text-white" : "text-[#9297a3] hover:bg-[#1e222c] hover:text-[#dfe2ea]"}`}
          ...
          <section className="grid min-h-0 grid-rows-[58px_minmax(0,1fr)_58px] bg-[#202229]">
            <header className="flex items-center justify-between border-b border-[#14161c] px-5">
              <div>
                ...
                <p className="mt-1 text-xs text-[#8f939d]">
                  {activeSection === "general"
                    ? "Manage app-wide behavior and updates."
                    : activeSection === "playback"
                      ? "Control preview and playback diagnostics."
                      : activeSection === "timeline"
                        ? "Tune timeline interaction responsiveness."
                        : activeSection === "export"
                          ? "Tune video rendering and capture behavior."
                          : "Configure experimental and developer-facing editor behavior."}
                </p>
              </div>
              <button
                className={`${appBarButtonBase} px-3 py-1.5`}
              ...
              {activeSection === "general" ? (
                <div className="grid gap-4 rounded-xl border border-[#363b47] bg-[#1b1e26] p-4">
                  <div className="grid gap-1.5">
                    ...
                    <p className="text-xs leading-5 text-[#8f939d]">
                      Choose which coding CLI opens from folder context menus.
                    </p>
                  </div>
                  <label
                  ...
                  <div className="grid gap-1.5">
                    ...
                    <p className="text-xs leading-5 text-[#8f939d]">
                      Clipper can check GitHub releases in packaged builds and
                  ...
                  <label
                    ...
                    htmlFor="auto-download-updates-toggle"
                  >
                    <span className="grid gap-1">
                      <span>Download updates automatically</span>
                      <span className="font-medium leading-5 text-[#8f939d]">
                        When enabled, available updates download after a
                        successful check. You can still install after download
                        when ready.
                      </span>
                    </span>
                    <Switch
                  ...
                  <div className="flex w-full items-start justify-between gap-5">
                    ...
                    <div className="flex shrink-0 items-center gap-2">
                      <button
                        className="rounded-[8px] border border-[#5b6270] bg-transparent px-3 py-2 text-xs font-extrabold text-[#f7f7f8] transition hover:border-[#dfe2ea] hover:bg-white/10 disabled:cursor-not-allowed disabled:opacity-50"
                        type="button"
                        disabled={
                      ...
                      <button
                        className="rounded-[8px] border border-[var(--clipper-accent)] bg-[var(--clipper-accent)] px-3 py-2 text-xs font-extrabold text-[var(--clipper-accent-foreground)] transition hover:bg-[var(--clipper-accent-hover)] disabled:cursor-not-allowed disabled:border-[#3b4150] disabled:bg-[#252a34] disabled:text-[#7f8490]"
                        type="button"
                ...
                <div className="grid gap-4 rounded-xl border border-[#363b47] bg-[#1b1e26] p-4">
                  <div className="grid gap-1.5">
                    ...
                    <p className="text-xs leading-5 text-[#8f939d]">
                      Controls whether Clipper prerenders frames around the
                      playhead for smoother playback.
                    </p>
                  </div>
                  <label
                    className="flex w-full items-start justify-between gap-5 text-xs font-bold text-[#dfe2ea]"
                    htmlFor="prerender-cache-toggle"
                  >
                    <span className="grid gap-1">
                      <span>Prerender frames around the playhead</span>
                      <span className="font-medium leading-5 text-[#8f939d]">
                        Saves nearby preview frames in the project `.cache`
                        folder for smoother playback.
                      </span>
                    </span>
                    <Switch
                  ...
                  {debugSettingsEnabled ? (
                    <label
                      className="flex w-full items-start justify-between gap-5 text-xs font-bold text-[#dfe2ea]"
                      htmlFor="prerender-cache-black-miss-debug-toggle"
                    >
                      <span className="grid gap-1">
                        <span className="flex items-center gap-2">
                          Show black for uncached frames{" "}
                          <span className="rounded-full border border-[#6f7684] px-2 py-0.5 text-[10px] uppercase tracking-[0.08em] text-[#c5cad3]">
                            Debug
                          </span>
                        </span>
                        <span className="font-medium leading-5 text-[#8f939d]">
                          Disables DOM fallback while prerender cache is
                          enabled, making cache misses visible as black frames
                          for stutter debugging.
                        </span>
                      </span>
                      <Switch
                  ...
                  <div className="grid gap-1.5">
                    <strong className="text-sm text-white">
                      Prerender block size
                    </strong>
                    <p className="text-xs leading-5 text-[#8f939d]">
                      Controls the time span rendered per cache block. Changing
                      this clears the existing prerender cache.
                    </p>
                  </div>
                  <label
                    className="grid max-w-[260px] gap-1.5 text-xs font-bold text-[#dfe2ea]"
                    htmlFor="prerender-block-duration"
                  >
                    Block duration (ms)
                    <span className="relative">
                      <Input
                        id="prerender-block-duration"
                        className="pr-10"
                        min={minPrerenderBlockDurationMs}
                        max={maxPrerenderBlockDurationMs}
                        step={10}
                        type="number"
                        value={prerenderBlockDurationMs}
                        onChange={(event) =>
                          updatePrerenderBlockDuration(event.target.value)
                        }
                      />
                      <button
                        aria-label={`Reset prerender block duration to ${defaultPrerenderBlockDurationMs}ms`}
                  ...
                  <div className="grid gap-1.5">
                    ...
                    <p className="text-xs leading-5 text-[#8f939d]">
                      ...
                      controls only scale the displayed result. 1080 renders at
                      1920x1080.
                    </p>
                  </div>
                  <label
                        ...
                        <SelectContent>
                          {previewRenderHeightOptions.map((height) => (
                            <SelectItem key={height} value={String(height)}>
                              {previewRenderResolutionLabels.get(height)}
                            </SelectItem>
                          ))}
                        </SelectContent>
                      </Select>
                      <button
                        aria-label={`Reset preview render height to ${defaultPreviewRenderHeight}px`}
                  ...
                  <div className="flex w-full items-center justify-between gap-5">
                    <span className="grid gap-1 text-xs">
                      ...
                      <span className="font-medium leading-5 text-[#8f939d]">
                        Delete prerender cache folders for every project in the
                        local projects directory.
                      </span>
                    </span>
                    <button
                      className="rounded-[8px] border border-[#5b6270] bg-transparent px-3 py-2 text-xs font-extrabold text-[#f7f7f8] transition hover:border-[#dfe2ea] hover:bg-white/10"
                      type="button"
                      onClick={onClearAllPrerenderCaches}
                    >
                      Clear all caches
                    </button>
                  </div>
                  ...
                  >
                    ...
                    <span className="relative">
                      <Input
                        ...
                        step={0.1}
                        type="number"
                        value={markerDurationSeconds}
                        onChange={(event) =>
                          updateMarkerDuration(event.target.value)
                        }
                      />
                      <button
                  ...
                  >
                    ...
                    <span className="relative">
                      <Input
                        ...
                        step={0.05}
                        type="number"
                        value={timelineEndPaddingFraction}
                        onChange={(event) =>
                          updateTimelineEndPaddingFraction(event.target.value)
                        }
                      />
                      <button
                        aria-label={`Reset timeline end padding to ${defaultTimelineEndPaddingFraction}`}
                        className="absolute right-2 top-1/2 grid h-7 w-7 -translate-y-1/2 place-items-center rounded-md text-[#8f939d] transition hover:bg-[#252a34] hover:text-white"
                        type="button"
                        onClick={() =>
                          onTimelineEndPaddingFractionChange(
                  ...
                  >
                    ...
                    <span className="relative">
                      <Input
                        ...
                        onChange={(event) =>
                          updateTimelinePrecision(event.target.value)
                        }
                      />
                      <button
                        aria-label={`Reset timeline precision to ${defaultTimelinePrecision}`}
                        className="absolute right-2 top-1/2 grid h-7 w-7 -translate-y-1/2 place-items-center rounded-md text-[#8f939d] transition hover:bg-[#252a34] hover:text-white"
                        type="button"
                        onClick={() =>
                ...
                <div className="grid gap-4 rounded-xl border border-[#363b47] bg-[#1b1e26] p-4">
                  <div className="grid gap-1.5">
                    ...
                    <p className="text-xs leading-5 text-[#8f939d]">
                      Controls hidden renderer processes for media export. Heavy
                      8K scenes should use fewer UHD workers; higher values are
                      faster but multiply CPU, RAM, and Chromium tile memory.
                    </p>
                  </div>
                  <label
                    className="grid w-[260px] gap-1.5 text-xs font-bold text-[#dfe2ea]"
                    htmlFor="export-worker-configuration-mode"
                  >
                    Configuration mode
                    <Select
                    ...
                    >
                      <SelectTrigger
                        id="export-worker-configuration-mode"
                        className="h-9"
                      >
                        <SelectValue />
                      </SelectTrigger>
                  ...
                  {exportWorkerConfigurationMode === "unified" ? (
                    <div className="grid w-[260px] gap-3">
                      <WorkerCountField
                        id="export-workers-unified"
                        label="All export resolutions"
                        value={exportWorkerMapping.hd}
                    ...
                    <div className="grid grid-cols-3 gap-3">
                      <WorkerCountField
                        id="export-workers-hd"
                        label="1920x1080 and below"
                        value={exportWorkerMapping.hd}
                        defaultValue={defaultExportWorkerMapping.hd}
                        onChange={(value) =>
                      ...
                      />
                      <WorkerCountField
                        id="export-workers-qhd"
                        label="2560x1440 and below"
                        value={exportWorkerMapping.qhd}
                        defaultValue={defaultExportWorkerMapping.qhd}
                        onChange={(value) =>
                      ...
                      />
                      <WorkerCountField
                        id="export-workers-uhd"
                        label="3840x2160 and above"
                        value={exportWorkerMapping.uhd}
                        defaultValue={defaultExportWorkerMapping.uhd}
                        onChange={(value) =>
                  ...
                  <div className="grid gap-1.5">
                    <strong className="text-sm text-white">
                      Prerender capture tile height
                    </strong>
                    <p className="text-xs leading-5 text-[#8f939d]">
                      Controls prerender/cache capture only. Stable slow media
                      export uses the grid fallback preset above, not this strip
                      height.
                    </p>
                  </div>
                  <label
                    className="grid w-[260px] gap-1.5 text-xs font-bold text-[#dfe2ea]"
                    htmlFor="video-export-tile-height"
                  >
                    Tile height (px)
                    <span className="relative">
                      <Input
                        id="video-export-tile-height"
                        className="pr-10"
                        min={minVideoExportTileHeight}
                        max={maxVideoExportTileHeight}
                        step={1}
                        type="number"
                        value={videoExportTileHeight}
                        onChange={(event) =>
                          updateVideoExportTileHeight(event.target.value)
                        }
                      />
                      <button
                        aria-label={`Reset export tile height to ${defaultVideoExportTileHeight}px`}
                        className="absolute right-2 top-1/2 grid h-7 w-7 -translate-y-1/2 place-items-center rounded-md text-[#8f939d] transition hover:bg-[#252a34] hover:text-white"
                        type="button"
                        onClick={() =>
                          onVideoExportTileHeightChange(
                  ...
                  <div className="grid gap-1.5">
                    <strong className="text-sm text-white">
                      DrawElement fallback tiles
                    </strong>
                    <p className="text-xs leading-5 text-[#8f939d]">
                      Controls the adaptive tiled fallback behind DrawElement
                      for very large outputs. Stable Slow ignores these counts
                      and uses validation-sampled grid capture instead.
                    </p>
                  </div>
                  <div className="grid grid-cols-3 gap-3">
                    <TileCountField
                  ...
                  <div className="grid gap-1.5">
                    <strong className="text-sm text-white">
                      Stable Slow fallback
                    </strong>
                    <p className="max-w-full text-xs leading-5 text-[#8f939d]">
                      Controls Stable Slow grid capture. Smaller grids are
                      slower but survive heavier 4K/8K DOM and SVG scenes.
                    </p>
                  </div>
                  <label
                    className="grid gap-1.5 text-xs font-bold text-[#dfe2ea]"
                    htmlFor="stable-slow-grid-preset"
                  >
                    ...
                    <span className="grid max-w-[260px] grid-cols-[minmax(0,1fr)_2.25rem] items-center gap-2">
                      ...
                      <button
                        aria-label={`Reset stable slow grid fallback to ${defaultStableSlowGridPreset}`}
                    ...
                    <span className="block max-w-full font-medium leading-5 text-[#8f939d]">
                      {stableSlowGridPresetDescriptions[stableSlowGridPreset]}
                    </span>
                  </label>
                  <label
                    className="grid gap-1.5 text-xs font-bold text-[#dfe2ea]"
                    htmlFor="stable-slow-validation-samples"
                  >
                    Validation samples
                    <span className="relative block w-[260px] max-w-[260px]">
                      <Input
                        id="stable-slow-validation-samples"
                        className="pr-10"
                        min={minStableSlowValidationSamples}
                        max={maxStableSlowValidationSamples}
                        step={1}
                        type="number"
                        value={stableSlowValidationSamples}
                        onChange={(event) =>
                          updateStableSlowValidationSamples(event.target.value)
                        }
                      />
                      <button
                        aria-label={`Reset stable slow validation samples to ${defaultStableSlowValidationSamples}`}
                        className="absolute right-2 top-1/2 grid h-7 w-7 -translate-y-1/2 place-items-center rounded-md text-[#8f939d] transition hover:bg-[#252a34] hover:text-white"
                        type="button"
                        onClick={() =>
                          onStableSlowValidationSamplesChange(
                    ...
                    <span className="block max-w-full font-medium leading-5 text-[#8f939d]">
                      Repeated captures of each tile at the same pinned time;
                      higher numbers catch unstable Chromium readback or
                      animation drift.
                    </span>
                  </label>
                </div>
                ...
                <div className="grid gap-4 rounded-xl border border-[#363b47] bg-[#1b1e26] p-4">
                  <div className="grid gap-4">
                    <div className="grid gap-1.5">
                      ...
                      <p className="text-xs leading-5 text-[#8f939d]">
                        Shows developer-only diagnostic controls in their
                        relevant settings sections.
                      </p>
                    </div>
                    <label
                      className="flex w-full items-start justify-between gap-5 text-xs font-bold text-[#dfe2ea]"
                      htmlFor="debug-settings-toggle"
                    >
                      <span className="grid gap-1">
                        <span>Enable debug settings</span>
                        <span className="font-medium leading-5 text-[#8f939d]">
                          When turned off, debug controls are hidden and reset
            ...
            <footer className="flex items-center justify-between border-t border-[#14161c] bg-[#202229] px-5">
              <span className="text-xs text-[#7f8490]">
                Shortcut: Cmd/Ctrl + ,
              </span>
              <button
                className="rounded-[9px] border border-[var(--clipper-accent)] bg-[var(--clipper-accent)] px-4 py-2 text-sm font-extrabold text-[var(--clipper-accent-foreground)] transition hover:bg-[var(--clipper-accent-hover)]"
                onClick={() => onOpenChange(false)}
              >
                Save
              </button>
    ...
    >
      ...
      <span className="relative">
        <Input
          id={id}
          className="pr-10"
          min={minExportWorkerCount}
          max={maxExportWorkerCount}
          step={1}
          type="number"
          value={value}
        ...
        />
        <button
          aria-label={`Reset ${label} workers to ${defaultValue}`}
          className="absolute right-2 top-1/2 grid h-7 w-7 -translate-y-1/2 place-items-center rounded-md text-[#8f939d] transition hover:bg-[#252a34] hover:text-white"
          type="button"
          onClick={onReset}
        >
    ...
    >
      ...
      <span className="relative">
        <Input
          id={id}
          className="pr-10"
          min={minExportTileCount}
          max={maxExportTileCount}
          step={1}
          type="number"
          value={value}
          onChange={(event) => onChange(event.target.value)}
        />
        <button
          aria-label={`Reset ${label} tiles to ${defaultValue}`}
          className="absolute right-2 top-1/2 grid h-7 w-7 -translate-y-1/2 place-items-center rounded-md text-[#8f939d] transition hover:bg-[#252a34] hover:text-white"
          type="button"
          onClick={onReset}
        >
