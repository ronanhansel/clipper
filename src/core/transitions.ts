...
export function applyTransitionLayersToVisualStyle(
  sceneTime: number,
  layers: TransitionLayer[] | undefined,
  frameRate = defaultTransitionFrameRate,
): TransitionVisualStyle {
  return getActiveTransitionLayers(
    layers,
    sceneTime,
  ).reduce<TransitionVisualStyle>((style, layer) => {
    const progress = getTransitionProgress(sceneTime, layer);
    const nextStyle = getTransitionEffectPackage(
      layer.effect.effectId,
    )?.applyVisualStyle?.({ sceneTime, layer, frameRate, progress });
    if (!nextStyle) return style;
    return {
      ...style,
      ...nextStyle,
      filter:
        [style.filter, nextStyle.filter].filter(Boolean).join(" ") || undefined,
...
export function renderTransitionSequence(
  sceneTime: number,
  ...
  const progress = getTransitionProgress(sceneTime, layer);
  return (
    getTransitionEffectPackage(layer.effect.effectId)?.renderSequence?.({
      sceneTime,
      layer,
      frameRate,
      progress,
...
export function getTransitionMarkerTime(
  layer: Pick<TransitionLayer, "start" | "duration">,
) {
  return layer.start + layer.duration / 2;
}

export function normalizeSymmetricTransitionLayer<T extends TransitionLayer>(
  layer: T,
): T {
...
export function getTransitionProgress(
  ...
  const linearProgress =
    finishTime > 0
      ? Math.min(Math.max((sceneTime - layer.start) / finishTime, 0), 1)
      : 1;
  return easeProgress(linearProgress, layer.effect.params?.ease ?? "easeInOut");
}

function defaultTransitionSequenceStyle(
  progress: number,
): TransitionSequenceStyle {
