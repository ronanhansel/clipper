...
export function reconstructFileName(
  displayName: string,
...
export function isSemanticFile(fileName: string): boolean {
  return (
    COMPOSITION_SUFFIXES.some((s) => fileName.endsWith(s)) ||
    TIMELINE_SUFFIXES.some((s) => fileName.endsWith(s))
  );
}

export function nextNumberedSemanticName(
  baseName: string,
  suffix: string,
