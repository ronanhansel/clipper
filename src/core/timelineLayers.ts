import type {
  TimelineAdjustmentLayerState,
  TimelineCompositionLayerState,
  TimelineLayerState,
  TimelineMotionLayerState,
  TimelineTransitionLayerState,
} from "./types";

export type TimelineLayerCategory = "transition" | "adjust" | "motion" | "comp";
export type TimelineLayerStateKey =
  | "compositionLayers"
  | "adjustmentLayers"
  | "motionLayers"
  | "transitionLayers";
export type TimelineLayerStateItem =
  | TimelineCompositionLayerState
  | TimelineAdjustmentLayerState
  | TimelineMotionLayerState
  | TimelineTransitionLayerState;
...
export function getTimelineLayerRowAtY(
  ...
  const index = layout.rows.findIndex((row, rowIndex) => {
    ...
    return (
      y >= layout.starts[rowIndex] &&
      y <= layout.starts[rowIndex] + layout.heights[rowIndex]
    );
  });
  return index < 0 ? null : { row: layout.rows[index], index };
}

export function getTimelineLayerRowAtClientY(
  layout: TimelineLayerLayout,
...
export function getTimelineLayerRowAtClientYClamped(
  layout: TimelineLayerLayout,
  containerRect: Pick<DOMRect, "top"> | null | undefined,
  clientY: number,
  category: TimelineLayerCategory,
  ...
  const categoryRows = layout.rows.flatMap((row, index) =>
      ...
      ? [
          {
            ...
            end: layout.starts[index] + layout.heights[index],
          },
        ]
      : [],
  );
  ...
  return categoryRows.reduce(
    (nearest, item) => {
      const distance =
        y < item.start ? item.start - y : y > item.end ? y - item.end : 0;
      return distance < nearest.distance ? { item, distance } : nearest;
    },
    { item: categoryRows[0], distance: Number.POSITIVE_INFINITY },
  ).item;
}

export function getTimelineLayerDragPreview(
  layout: TimelineLayerLayout,
...
export function getTimelineLayerStateKey(
  category: TimelineLayerCategory,
): TimelineLayerStateKey {
...
export function getTimelineStateLayers(
  state: TimelineLayerState,
  category: TimelineLayerCategory,
  defaults: TimelineLayerState,
): TimelineLayerStateItem[] {
  const key = getTimelineLayerStateKey(category);
  return state[key]?.length ? state[key]! : (defaults[key] ?? []);
}

export function insertTimelineStateLayer<T extends TimelineLayerStateItem>(
  layers: T[],
  layer: T,
  targetLayerId?: string,
  ...
  const targetIndex = targetLayerId
    ? layers.findIndex((item) => item.id === targetLayerId)
    : -1;
...
export function renameTimelineStateLayer(
  state: TimelineLayerState,
  category: TimelineLayerCategory,
  layerId: string,
  name: string,
  ...
  return {
    ...state,
    [key]: getTimelineStateLayers(state, category, defaults).map((layer) =>
      layer.id === layerId ? { ...layer, name } : layer,
    ),
  };
}

export function toggleTimelineStateLayerHidden(
  state: TimelineLayerState,
  category: TimelineLayerCategory,
  layerId: string,
  defaults: TimelineLayerState,
  ...
  return {
    ...
    [key]: getTimelineStateLayers(state, category, defaults).map((layer) =>
      layer.id === layerId
        ? { ...layer, hidden: !layer.hidden || undefined }
        : layer,
    ),
  };
}

export function toggleTimelineStateLayerLocked(
  state: TimelineLayerState,
  category: TimelineLayerCategory,
  layerId: string,
  defaults: TimelineLayerState,
  ...
  return {
    ...
    [key]: getTimelineStateLayers(state, category, defaults).map((layer) =>
      layer.id === layerId
        ? { ...layer, locked: !layer.locked || undefined }
        : layer,
    ),
  };
}

export function moveTimelineStateLayer(
  state: TimelineLayerState,
  category: TimelineLayerCategory,
  layerId: string,
  direction: "up" | "down",
...
export function removeTimelineStateLayer(
  state: TimelineLayerState,
  category: TimelineLayerCategory,
  layerId: string,
  defaults: TimelineLayerState,
...
export function computeBulkLayerTargets(
  ...
  const sourceIdx = categoryRows.findIndex(
    (row) => row.key === normalizedSource,
  );
  const cursorIdx = cursorLayerId
    ? categoryRows.findIndex((row) => row.key === cursorLayerId)
    : sourceIdx;
  ...
  for (const target of moveTargets) {
    ...
    const targetSrcIdx = categoryRows.findIndex(
      (row) => row.key === targetSourceLayer,
    );
    if (targetSrcIdx < 0) {
    ...
    const newIdx = Math.max(
      0,
      Math.min(targetSrcIdx + layerIdxDelta, categoryRows.length - 1),
    );
...
export function resolveTimelineLayerMoveTargets(
  ...
  const cursorLayerId =
    cursorRow?.row.key && !isLayerLocked(category, cursorRow.row.key)
      ? cursorRow.row.key
      : undefined;
...
export function getLayerMoveDragPreview(
  ...
  const effectiveTarget =
    computedLayerId && !isLayerLocked(category, computedLayerId)
      ? computedLayerId
      : undefined;
  return getTimelineLayerDragPreview(layout, sourceLayerId, effectiveTarget);
}

export function getTimelineBlockLayerPreview(
  layout: TimelineLayerLayout,
  category: TimelineLayerCategory,
  sourceLayerId: string | undefined,
  clientY: number,
  ...
  const targetLayerId = getTimelineLayerRowAtClientYClamped(
    layout,
    containerRect,
    clientY,
    category,
...
export function applyTimelineBlockPreview(
  element: HTMLElement,
  options: TimelineBlockPreviewOptions,
) {
...
export function clearTimelineBlockPreview(
  element: HTMLElement,
  resizeProperty = "--clipper-timeline-resize-width",
) {
...
function preserveInlineStyle(
  element: HTMLElement,
  datasetKey: "originalBackground" | "originalColor",
