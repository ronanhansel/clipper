...
export const motionEffectRegistry = new Map<
  MotionEffectId,
  (typeof motionEffectDefinitions)[number]
>(
  motionEffectDefinitions.map((definition) => [
    definition.id as MotionEffectId,
    definition,
  ]),
);
...
export function getMotionBlockEffectKind(block: {
  effectId?: string;
}): MotionBlockEffectKind | null {
  return block.effectId
    ? (getMotionEffectDefinition(block.effectId)?.kind ?? null)
    : null;
}

export function isMotionKind(
  marker: MotionMarker,
...
export function normalizeMotionBlocks(
  blocks: MotionBlock[] | undefined,
): MotionBlock[] {
  const sourceBlocks = blocks ?? [];
  return sourceBlocks.flatMap((block) => {
    const definition = getMotionEffectDefinition(
      block.effectId ?? "clipper.motion.pan",
    );
    ...
    return [
      {
        ...
        perspective:
          normalizePerspective(block.perspective) ?? params.perspective,
...
export function motionMarkersToMotionBlocks(
  markers: MotionMarker[],
): MotionBlock[] {
  return normalizeMotionBlocks(
    markers.map((marker) => ({
      ...marker,
      effectId: marker.effectId ?? getMotionEffectByKind(marker.kind)?.id,
    })),
  );
}

export function getCanonicalMotionMarkers(input: {
  motionMarkers?: MotionMarker[];
}) {
  return motionBlocksToMotionMarkers(input.motionMarkers ?? []);
}

export function getMotionMarkerViews(input: {
  motionMarkers?: MotionMarker[];
}) {
...
export function createDefaultMotionBlock(
  ...
  return (
    getMotionEffectByKind(kind)?.createDefaultBlock(input) ??
    getMotionEffectByKind("pan")!.createDefaultBlock(input)
  );
}

export function createDefaultMotionBlockByEffectId(
  effectId: string,
  input: {
    id: string;
    layerId: string;
  ...
  return (
    getMotionEffectPackage(effectId)?.createDefaultBlock(input) ??
    getMotionEffectByKind("pan")!.createDefaultBlock(input)
  );
}

function normalizeMotionBlockParams(
  block: MotionBlock,
): MotionBlock["params"] & {
  focus?: Point;
  position?: Point;
