...
import { constrainDragDeltaToDominantAxis, getBoundsWithPreviewTransform, getFrameObjectPreviewTransform, getFrameObjectWithPreviewBounds, getObjectDragSnap, getResizedObjects } from "./frameInteraction";
import type { FrameObject } from "./types";

describe("frame interaction", () => {
  const rect = (id: string, bounds: FrameObject["bounds"], hidden = false): FrameObject => ({ id, name: id, type: "rect", selector: `[data-object-id='${id}']`, bounds, hidden, style: {} });
  ...
  it("snaps object drag to frame center", () => {
    const drag = { origin: { x: 0, y: 0 }, partId: "part", objects: [rect("selected", { x: 100, y: 100, width: 200, height: 100 })] };

    expect(getObjectDragSnap(drag, { x: 759, y: 389 }, [], 8)).toEqual({ delta: { x: 760, y: 390 }, guides: [{ axis: "x", position: 960 }, { axis: "y", position: 540 }] });
  });

  it("snaps object drag to object edge", () => {
    const drag = { origin: { x: 0, y: 0 }, partId: "part", objects: [rect("selected", { x: 100, y: 100, width: 100, height: 100 })] };

    expect(getObjectDragSnap(drag, { x: 197, y: 0 }, [rect("target", { x: 300, y: 400, width: 100, height: 100 })], 8)).toEqual({ delta: { x: 200, y: 0 }, guides: [{ axis: "x", position: 300 }] });
  });

  it("excludes selected and hidden objects from drag snap targets", () => {
    const selected = rect("selected", { x: 100, y: 100, width: 100, height: 100 });
    const drag = { origin: { x: 0, y: 0 }, partId: "part", objects: [selected] };

    expect(getObjectDragSnap(drag, { x: 197, y: 0 }, [selected, rect("hidden", { x: 300, y: 100, width: 100, height: 100 }, true)], 8)).toEqual({ delta: { x: 197, y: 0 }, guides: [] });
  });

  it("does not snap object drag beyond threshold", () => {
    const drag = { origin: { x: 0, y: 0 }, partId: "part", objects: [rect("selected", { x: 100, y: 100, width: 100, height: 100 })] };

    expect(getObjectDragSnap(drag, { x: 189, y: 0 }, [rect("target", { x: 300, y: 400, width: 100, height: 100 })], 8)).toEqual({ delta: { x: 189, y: 0 }, guides: [] });
  });

  it("uses closest object drag snap guide", () => {
    const drag = { origin: { x: 0, y: 0 }, partId: "part", objects: [rect("selected", { x: 100, y: 100, width: 100, height: 100 })] };

    expect(getObjectDragSnap(drag, { x: 196, y: 0 }, [rect("far", { x: 300, y: 400, width: 100, height: 100 }), rect("near", { x: 298, y: 400, width: 100, height: 100 })], 8)).toEqual({ delta: { x: 198, y: 0 }, guides: [{ axis: "x", position: 298 }] });
  });

  it("snaps object drag axes independently", () => {
    const drag = { origin: { x: 0, y: 0 }, partId: "part", objects: [rect("selected", { x: 100, y: 100, width: 100, height: 100 })] };

    expect(getObjectDragSnap(drag, { x: 197, y: 297 }, [rect("target", { x: 300, y: 400, width: 100, height: 100 })], 8)).toEqual({ delta: { x: 200, y: 300 }, guides: [{ axis: "x", position: 300 }, { axis: "y", position: 400 }] });
  });
