...
export function isValidExportRawFramePayload(payload: unknown, width?: number, height?: number): payload is ExportRawFramePayload {
  if (!payload || typeof payload !== "object") return false;
  const candidate = payload as Partial<ExportRawFramePayload>;
  if (!isPositiveInteger(candidate.width) || !isPositiveInteger(candidate.height)) return false;
  if (width !== undefined && candidate.width !== width) return false;
  if (height !== undefined && candidate.height !== height) return false;
  if (candidate.pixelFormat !== "bgra" && candidate.pixelFormat !== "rgba") return false;
  return getRawFrameByteLength(candidate) === candidate.width * candidate.height * 4;
}

export function isValidExportRawPostProcessFrameResult(result: unknown, width?: number, height?: number): result is ExportRawPostProcessFrameResult {
  if (!result || typeof result !== "object") return false;
  const candidate = result as Partial<ExportRawPostProcessFrameResult>;
  return candidate.applied === true && typeof candidate.droppedPassCount === "number" && isValidExportRawFramePayload(candidate.outputFrame, width, height);
}
...
export function webGlReadPixelsRgbaToTopLeftRgba(source: Uint8Array, width: number, height: number): Uint8Array {
  const output = new Uint8Array(source.byteLength);
  const stride = width * 4;
  for (let row = 0; row < height; row += 1) {
    output.set(source.subarray((height - row - 1) * stride, (height - row) * stride), row * stride);
  }
  return output;
}
...
export async function applyExportRawPostProcessFrame(input: ExportRawPostProcessFrameRequest, renderers: readonly ExportPostProcessRenderer[]): Promise<ExportRawPostProcessFrameResult> {
  if (!hasExportPostProcessPasses(input.passes)) return { applied: false, outputFrame: input.sourceFrame, droppedPassCount: 0 };
  if (!isValidExportRawFramePayload(input.sourceFrame, input.width, input.height)) throw new Error("Export post-process requires a valid raw source frame.");

  const canvases = [document.createElement("canvas"), document.createElement("canvas")];
  const sourceCanvas = createCanvasFromRawFrame(input.sourceFrame);
  const { sourceFrame, renderedPassCount, droppedPassCount } = renderPostProcessPasses({ ...input, source: sourceCanvas, canvases }, renderers);

  if (renderedPassCount === 0) return { applied: false, outputFrame: input.sourceFrame, droppedPassCount };

  const outputFrame = readCanvasToRawFrame(sourceFrame as HTMLCanvasElement, input.width, input.height);
...
function renderPostProcessPasses(input: RenderPostProcessPassesInput, renderers: readonly ExportPostProcessRenderer[]) {
  const registry = new Map(renderers.map((renderer) => [renderer.kind, renderer]));
  const renderedByKind = new Map<string, number>();
...
function createCanvasFromRawFrame(frame: ExportRawFramePayload): HTMLCanvasElement {
  const canvas = document.createElement("canvas");
  canvas.width = frame.width;
  canvas.height = frame.height;
  const context = canvas.getContext("2d");
  if (!context) throw new Error("Export post-process raw frame bridge requires a 2D canvas context.");
  const bytes = getRawFrameBytes(frame);
  const rgba = new Uint8ClampedArray(frame.pixelFormat === "bgra" ? bgraBytesToRgbaClamped(bytes) : bytes);
  context.putImageData(new ImageData(rgba, frame.width, frame.height, { colorSpace: "srgb" }), 0, 0);
  return canvas;
}

function readCanvasToRawFrame(canvas: HTMLCanvasElement, width: number, height: number): ExportRawFramePayload {
  const gl = canvas.getContext("webgl") as WebGLRenderingContext | null;
  if (gl && !gl.isContextLost()) {
    const pixels = new Uint8Array(width * height * 4);
    gl.readPixels(0, 0, width, height, gl.RGBA, gl.UNSIGNED_BYTE, pixels);
    return { width, height, pixelFormat: "rgba", data: webGlReadPixelsRgbaToTopLeftRgba(pixels, width, height) };
  }
  const context = canvas.getContext("2d");
  if (!context) throw new Error("Export post-process raw frame bridge could not read processed pixels.");
  return { width, height, pixelFormat: "rgba", data: new Uint8Array(context.getImageData(0, 0, width, height).data) };
}
...
function getRawFrameBytes(frame: ExportRawFramePayload): Uint8Array {
  ...
  if (typeof frame.dataBase64 === "string") return base64ToBytes(frame.dataBase64);
...
function getRawFrameByteLength(frame: Partial<ExportRawFramePayload>): number {
  if (ArrayBuffer.isView(frame.data) || frame.data instanceof ArrayBuffer) return frame.data.byteLength;
  if (Array.isArray(frame.data)) return frame.data.length;
  if (typeof frame.dataBase64 === "string") return Math.floor(frame.dataBase64.length * 3 / 4) - (frame.dataBase64.endsWith("==") ? 2 : frame.dataBase64.endsWith("=") ? 1 : 0);
  return -1;
}
