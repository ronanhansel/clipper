...
const sampleBuffer = new Map<string, { count: number; total: number; max: number; windowStart: number }>();
...
function recordSample(sample: PerfSample) {
  ...
  const bucket = sampleBuffer.get(sample.key) ?? { count: 0, total: 0, max: 0, windowStart: now };
  bucket.count += 1;
  ...
  console.info(`[preview-perf] ${sample.key} avg=${avg.toFixed(2)}ms max=${bucket.max.toFixed(2)}ms n=${bucket.count}`);
  bucket.count = 0;
  bucket.total = 0;
  bucket.max = 0;
