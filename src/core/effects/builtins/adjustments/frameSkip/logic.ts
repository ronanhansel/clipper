...
export const frameSkipLogic = {
  applySceneTime: ({ sceneTime, layer, frameRate }) =>
    quantizeFrameSkipTime(
      sceneTime,
      layer.start,
      getFrameSkipEvery(layer),
      frameRate,
    ),
  validate: (layer) =>
    getFrameSkipEvery(layer) < 1
      ? `Adjustment ${layer.name} must skip at least 1 frame.`
      : null,
} as const satisfies Partial<
  Pick<AdjustmentEffectPackage, "applySceneTime" | "validate">
>;
...
export function quantizeFrameSkipTime(
  sceneTime: number,
  start: number,
  every: number,
  frameRate: number,
  ...
  const elapsedFrames = Math.max(
    0,
    Math.floor((sceneTime - start) * frameRate),
  );
