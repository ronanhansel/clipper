...
export const loopStutterLogic = {
  applySceneTime: ({ sceneTime, layer }) =>
    layer.start +
    positiveModulo(Math.max(0, sceneTime - layer.start), getLoopWindow(layer)),
  validate: (layer) =>
    getNumericParam(layer, "window", 0.5) <= 0
      ? `Adjustment ${layer.name} must use a loop window greater than 0.`
      : null,
