...
export class UpdateService {
  ...
  constructor(private readonly options: UpdateServiceOptions) {
    ...
    this.status = this.isSupported()
      ...
      : {
          kind: "unsupported",
          message:
            "Updates are available only in packaged builds with release publishing configured.",
        };
  }
  ...
  initialize() {
    ...
    autoUpdater.on("update-available", (info) => {
      this.setStatus({
        kind: this.autoDownload ? "downloading" : "available",
        message: this.autoDownload
          ? `Downloading Clipper ${info.version}...`
          : `Clipper ${info.version} is available.`,
    ...
    autoUpdater.on("update-not-available", (info) =>
      this.setStatus({
        kind: "not-available",
        message: `Clipper ${app.getVersion()} is up to date.`,
        version: info.version,
      }),
    );
    ...
    autoUpdater.on("update-downloaded", (info) =>
      this.setStatus({
        kind: "downloaded",
        message: `Clipper ${info.version} is ready to install.`,
        version: info.version,
        downloaded: true,
      }),
    );
  ...
  async checkForUpdates() {
    ...
    try {
      ...
      if (result?.updateInfo)
        return this.statusForUpdateInfo(result.updateInfo);
      return this.status;
    } catch (error) {
      const message =
  ...
  async downloadUpdate() {
    ...
    if (this.status.kind !== "available")
      return {
        ...this.status,
        message: "No available update is ready to download.",
      };
    this.setStatus({
      ...this.status,
      kind: "downloading",
      message: this.status.version
        ? `Downloading Clipper ${this.status.version}...`
        : "Downloading update...",
    });
    try {
  ...
  private setStatus(status: UpdateStatus) {
    ...
    for (const window of BrowserWindow.getAllWindows())
      window.webContents.send("clipper:update-status", status);
  }
}
