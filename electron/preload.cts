...
contextBridge.exposeInMainWorld("clipper", {
  ...
  copyText: (text: string) => ipcRenderer.invoke("clipper:copy-text", text) as Promise<void>,
  openAgentTerminal: (relativePath: string, provider: "opencode" | "codex" | "claude" | "gemini") => ipcRenderer.invoke("clipper:open-agent-terminal", relativePath, provider) as Promise<void>,
  ...
  listTemplates: () => ipcRenderer.invoke("clipper:list-templates") as Promise<Array<{ id: string; slug: string; title: string; subtitle: string; entry: string; author: { name: string; github?: string; twitter?: string; email?: string }; files: Record<string, string> }>>,
  ...
  renderVideoExport: (exportId: string, defaultFileName: string, project: unknown, manifestPath: string, scene: unknown, frameRate: number, durationSeconds: number, tileHeight: number, reusePrerenderCache: boolean, exportWidth?: number, exportHeight?: number, mediaExportFormat?: string, exportRenderQuality?: string, exportWorkerMapping?: unknown, exportTileMapping?: unknown, exportRenderMode?: string, stableSlowGridPreset?: string, stableSlowValidationSamples?: number) => ipcRenderer.invoke("clipper:render-video-export", exportId, defaultFileName, project, manifestPath, scene, frameRate, durationSeconds, tileHeight, reusePrerenderCache, exportWidth, exportHeight, mediaExportFormat, exportRenderQuality, exportWorkerMapping, exportTileMapping, exportRenderMode, stableSlowGridPreset, stableSlowValidationSamples) as Promise<string | null>,
