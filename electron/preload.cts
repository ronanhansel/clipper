...
contextBridge.exposeInMainWorld("clipper", {
  ...
  readAppState: () =>
    ipcRenderer.invoke("clipper:read-app-state") as Promise<
      Record<string, unknown>
    >,
  writeAppState: (updates: Record<string, unknown>) =>
    ipcRenderer.invoke("clipper:write-app-state", updates) as Promise<void>,
  ...
  revealFile: (relativePath: string) =>
    ipcRenderer.invoke("clipper:reveal-file", relativePath) as Promise<void>,
  revealAbsolutePath: (filePath: string) =>
    ipcRenderer.invoke(
      "clipper:reveal-absolute-path",
      filePath,
    ) as Promise<void>,
  copyText: (text: string) =>
    ipcRenderer.invoke("clipper:copy-text", text) as Promise<void>,
  openAgentTerminal: (
    relativePath: string,
  ...
  trashFile: (relativePath: string) =>
    ipcRenderer.invoke("clipper:trash-file", relativePath) as Promise<void>,
  renameFile: (relativePath: string, nextRelativePath: string) =>
    ipcRenderer.invoke(
      "clipper:rename-file",
      relativePath,
      nextRelativePath,
    ) as Promise<void>,
  copyFile: (relativePath: string, nextRelativePath: string) =>
    ipcRenderer.invoke(
      "clipper:copy-file",
      relativePath,
      nextRelativePath,
  ...
  listDirectory: (relativePath: string) =>
    ipcRenderer.invoke("clipper:list-directory", relativePath) as Promise<
      { name: string; isDirectory: boolean }[]
    >,
  listTemplates: () =>
    ipcRenderer.invoke("clipper:list-templates") as Promise<
      Array<{
        id: string;
        slug: string;
        title: string;
    ...
    >,
  findProjectFileByName: (directoryPath: string, fileName: string) =>
    ipcRenderer.invoke(
      "clipper:find-project-file-by-name",
      directoryPath,
      fileName,
    ) as Promise<string | null>,
  openCompositionFile: (directoryPath: string) =>
    ipcRenderer.invoke(
  ...
  toggleWindowFullscreen: () =>
    ipcRenderer.invoke("clipper:toggle-window-fullscreen") as Promise<boolean>,
  watchTextFiles: (relativePaths: string[]) =>
    ipcRenderer.invoke(
  ...
  createProject: (projectName: string) =>
    ipcRenderer.invoke("clipper:create-project", projectName) as Promise<
      string | null
    >,
  exportMediaFile: (defaultFileName: string, content: string) =>
    ipcRenderer.invoke(
      "clipper:export-media-file",
      defaultFileName,
      content,
    ) as Promise<string | null>,
  exportBinaryFile: (defaultFileName: string, base64Content: string) =>
    ipcRenderer.invoke(
  ...
  startVideoExport: (
    ...
    ipcRenderer.invoke(
      ...
      width,
      height,
    ) as Promise<{ sessionId: string; filePath: string } | null>,
  writeVideoFrame: (sessionId: string, frameData: Uint8ClampedArray) =>
    ipcRenderer.invoke(
      "clipper:write-video-frame",
      sessionId,
      frameData,
    ) as Promise<void>,
  finishVideoExport: (sessionId: string) =>
    ipcRenderer.invoke(
      "clipper:finish-video-export",
      sessionId,
    ) as Promise<string>,
  cancelVideoExport: (sessionId: string) =>
    ipcRenderer.invoke(
  ...
  clearPrerenderCache: (manifestPath: string) =>
    ipcRenderer.invoke(
      "clipper:clear-prerender-cache",
      manifestPath,
    ) as Promise<void>,
  clearAllPrerenderCaches: () =>
    ipcRenderer.invoke("clipper:clear-all-prerender-caches") as Promise<{
      clearedCount: number;
    }>,
  cancelRenderVideoExport: (exportId: string) =>
    ipcRenderer.invoke(
  ...
  checkForUpdates: () =>
    ipcRenderer.invoke("clipper:check-for-updates") as Promise<UpdateStatus>,
  downloadUpdate: () =>
    ipcRenderer.invoke("clipper:download-update") as Promise<UpdateStatus>,
  installUpdate: () =>
    ipcRenderer.invoke("clipper:install-update") as Promise<UpdateStatus>,
  onVideoExportProgress: (
    ...
    ) => callback(exportId, progress);
    ipcRenderer.on("clipper:video-export-progress", listener);
    return () =>
      ipcRenderer.removeListener("clipper:video-export-progress", listener);
  },
  ...
  onSettingsShortcut: (callback: () => void) => {
    ...
    return () =>
      ipcRenderer.removeListener("clipper:settings-shortcut", listener);
  },
  onCloseEditorTabShortcut: (callback: () => void) => {
    const listener = () => callback();
    ipcRenderer.on("clipper:close-editor-tab-shortcut", listener);
    return () =>
      ipcRenderer.removeListener("clipper:close-editor-tab-shortcut", listener);
  },
  onRestoreEditorTabShortcut: (callback: () => void) => {
    ...
    return () =>
      ipcRenderer.removeListener(
        "clipper:restore-editor-tab-shortcut",
        listener,
      );
  },
  onWindowFullscreenChange: (callback: (fullscreen: boolean) => void) => {
    ...
    return () =>
      ipcRenderer.removeListener("clipper:window-fullscreen-changed", listener);
  },
