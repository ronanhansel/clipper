...
export class RenderEngine {
  ...
  private getElectronChildArgs: (args: string[], electronArgs?: string[]) => string[];
  ...
  private activePreviewVideoEncoders = new Set<ChildProcessWithoutNullStreams>();

  private hardwareEncoderSupport: Set<string> | null = null;

  constructor(deps: RenderEngineDeps) {
    this.ffmpegPath = deps.ffmpegPath;
    this.videoSessions = new VideoExportSessionManager({ ffmpegPath: this.ffmpegPath });
  ...
  startVideoExport(
    ...
    return this.videoSessions.startVideoExport(defaultFileName, frameRate, width, height, filePath);
  }
  ...
  async prerenderFrame(
    ...
    const exportBlockDurationMs = this.clampPrerenderBlockDurationMs(blockDurationMs);
    const frameIndex = Math.round(sceneTime * frameRate);
    const block = exactFrameRange
      ? this.getBoundedPrerenderFrameRange(exactFrameRange, sceneDuration, frameRate)
      : this.getPrerenderBlockRange(frameIndex, sceneDuration, frameRate, exportBlockDurationMs);
    const cacheKey = this.getPrerenderCacheKey(project, scene, frameRate, exportTileHeight, exportBlockDurationMs);
    const logPrefix = `[clipper prerender-cache] scene=${scene.id} frame=${frameIndex} time=${sceneTime.toFixed(3)}`;
    const cachedBlock = await this.readPrerenderBlock(manifestPath, scene, frameRate, block, cacheKey);
    ...
    await this.renderPrerenderBlock(project, manifestPath, scene, sceneDuration, frameRate, exportTileHeight, block, cacheKey, logPrefix);
    const frames = await this.readPrerenderBlock(manifestPath, scene, frameRate, block, cacheKey);
    if (frames.length !== block.endFrame - block.startFrame)
      throw new Error(`Prerender cache block did not produce ${block.endFrame - block.startFrame} frame(s).`);
    return frames;
  }
  ...
  async prerenderVideoBlock(
    ...
    const exportBlockDurationMs = this.clampPrerenderBlockDurationMs(blockDurationMs);
    const frameIndex = Math.round(sceneTime * frameRate);
    const block = this.getPrerenderBlockRange(frameIndex, sceneDuration, frameRate, exportBlockDurationMs);
    const cacheKey = this.getPrerenderCacheKey(project, scene, frameRate, exportTileHeight, exportBlockDurationMs);
    const logPrefix = `[clipper prerender-video-cache] scene=${scene.id} frame=${frameIndex} time=${sceneTime.toFixed(3)}`;
    const cachedBlock = await this.readPrerenderVideoBlock(manifestPath, scene, frameRate, block, cacheKey);
    if (cachedBlock) return cachedBlock;
    console.log(`${logPrefix} render-block start=${block.startFrame} end=${block.endFrame}`);
    await this.renderPrerenderVideoBlock(project, manifestPath, scene, sceneDuration, frameRate, exportTileHeight, block, cacheKey, logPrefix);
    const renderedBlock = await this.readPrerenderVideoBlock(manifestPath, scene, frameRate, block, cacheKey);
  ...
  async clearPrerenderCache(manifestPath: string): Promise<void> {
    await fs.rm(this.getProjectCacheDirectory(manifestPath), { recursive: true, force: true }).catch(() => undefined);
  }
  ...
  async runRenderVideoChildIfRequested(payloadPath: string): Promise<boolean> {
    ...
    const payload = JSON.parse(await fs.readFile(payloadPath, "utf8")) as SupervisedRenderPayload;
    if (childRenderCancelled) return true;
    try {
      await this.renderSceneToRawFrames(payload, undefined, () => childRenderCancelled);
    } catch (error) {
      if (childRenderCancelled) return true;
  ...
  async renderSceneToRawFrames(
    ...
    return renderSceneToRawFramesFromCapture(payload, {
  ...
  private getSupportedHardwareEncoders(): Set<string> {
    ...
    const result = spawnSync(this.ffmpegPath, ["-hide_banner", "-encoders"], { encoding: "utf8" });
  ...
  private registerActiveVideoRenderCancel(exportId: string | undefined, cancel: () => void): () => void {
    if (!exportId) return () => {};
    ...
    return () => {
      ...
      if (controllers!.size === 0) this.activeVideoRenderControllers.delete(exportId);
    };
  }
  ...
  private async waitForProcessClose<T>(closePromise: Promise<T>, forceStop: () => void, timeoutMs: number): Promise<void> {
    let timeout: NodeJS.Timeout | null = null;
    try {
      await Promise.race([
        closePromise.catch(() => undefined),
        new Promise<void>((resolve) => {
          timeout = setTimeout(() => { forceStop(); resolve(); }, timeoutMs);
        }),
      ]);
    } finally {
  ...
  private async writeProcessInput(process: ChildProcessWithoutNullStreams, chunk: Buffer, exportId?: string): Promise<void> {
    this.throwIfVideoRenderCancelled(exportId);
  ...
  private async renderSupervisedFrameRangeChild(
    ...
    const child = spawn(
      process.execPath,
      this.getElectronChildArgs(["--render-video-child", payloadPath], electronArgs),
    ...
    const forceStopChild = () => { if (child.exitCode === null) child.kill("SIGKILL"); };
    const requestChildStop = (reason: string, forceAfterTimeout = false) => {
      ...
      if (child.connected && !childStopMessageSent) {
        ...
        child.send({ type: "clipper:stop-render-child", reason }, () => undefined);
      } else if (!child.connected) child.kill("SIGTERM");
      if (forceAfterTimeout && !childStopForceTimer)
        childStopForceTimer = setTimeout(forceStopChild, EXPORT_PROCESS_STOP_TIMEOUT_MS);
    };
    let cancelledWhileWaiting = false;
    const stopChild = () => { cancelledWhileWaiting = true; requestChildStop("cancel", true); };
    const unregisterCancel = this.registerActiveVideoRenderCancel(exportId, stopChild);
    ...
    const inspectChunk = (chunk: Buffer, stream: "stdout" | "stderr") => {
      ...
      if (isExportOutOfMemoryWarning(combined)) {
        ...
        if (nativeWarningCount <= 3) {
          console.warn(`[clipper export] source=${source} parent-native-warning stream=${stream} path=renderer: ${combined.trim().slice(-600)}`);
        } else if (nativeWarningCount === 4) {
          console.warn(`[clipper export] source=${source} path=renderer suppressing repeated native tile-memory warnings.`);
        }
        if (stream === "stdout") stdoutWarningTail = "";
        else stderrWarningTail = "";
      }
    };
    const inspectNativeWarningText = (text: string, stream: "stdout" | "stderr") => {
      if (!isExportOutOfMemoryWarning(text)) return;
      nativeWarningDetected = true;
      nativeWarningCount += 1;
      if (nativeWarningCount <= 3) {
        console.warn(`[clipper export] source=${source} parent-native-warning stream=${stream} path=renderer: ${text.trim().slice(-600)}`);
      } else if (nativeWarningCount === 4) {
        console.warn(`[clipper export] source=${source} path=renderer suppressing repeated native tile-memory warnings.`);
      }
    };
    ...
    const nativeLogPoll = setInterval(() => {
      fs.readFile(nativeLogPath, "utf8").then((log) => { if (log) inspectChunk(Buffer.from(log.slice(-32000)), "stderr"); }).catch(() => undefined);
    }, 100);
    ...
    try {
      const childClosePromise = new Promise<number | null>((resolve, reject) => {
        child.once("error", (error) => {
          if (cancelledWhileWaiting || this.appShuttingDownRef.current) resolve(null);
          else reject(error);
        });
  ...
  private getSupervisedFrameOutputPath(outputPath: string, frameIndex: number): string {
  ...
  private async removeSupervisedFrameOutputs(outputPath: string, frameRange: ExportFrameRange): Promise<void> {
    await fs.rm(outputPath, { force: true }).catch(() => undefined);
    for (let frameIndex = frameRange.startFrame; frameIndex < frameRange.endFrame; frameIndex += 1) {
      await fs.rm(this.getSupervisedFrameOutputPath(outputPath, frameIndex), { force: true }).catch(() => undefined);
    }
  }
  ...
  private getPrerenderCachePaths(manifestPath: string, scene: Scene, frameRate: number, frameIndex: number): PrerenderCachePaths {
    const directory = path.join(this.getProjectCacheDirectory(manifestPath), this.safeCacheSegment(scene.id), `${frameRate}fps`);
  ...
  private getPrerenderVideoBlockCachePaths(manifestPath: string, scene: Scene, frameRate: number, frameRange: ExportFrameRange): PrerenderVideoBlockCachePaths {
    const directory = path.join(this.getProjectCacheDirectory(manifestPath), this.safeCacheSegment(scene.id), `${frameRate}fps`, "video-blocks");
  ...
  private getPrerenderBlockRange(frameIndex: number, sceneDuration: number, frameRate: number, blockDurationMs: number): ExportFrameRange {
  ...
  private getBoundedPrerenderFrameRange(frameRange: ExportFrameRange, sceneDuration: number, frameRate: number): ExportFrameRange {
    const totalFrames = Math.max(1, Math.ceil(sceneDuration * frameRate));
  ...
  private clampPrerenderBlockDurationMs(value: number): number {
    ...
    return Math.min(Math.max(Math.round(value), MIN_PRERENDER_BLOCK_DURATION_MS), MAX_PRERENDER_BLOCK_DURATION_MS);
  }

  private async readPrerenderBlock(manifestPath: string, scene: Scene, frameRate: number, frameRange: ExportFrameRange, cacheKey: string): Promise<PrerenderedFrame[]> {
    const frames: PrerenderedFrame[] = [];
    for (let frameIndex = frameRange.startFrame; frameIndex < frameRange.endFrame; frameIndex += 1) {
      const sceneTime = frameIndex / frameRate;
      const frame = await this.readPrerenderFrame(this.getPrerenderCachePaths(manifestPath, scene, frameRate, frameIndex), cacheKey, sceneTime, frameRate);
      if (!frame) break;
      frames.push(frame);
    }
    return frames;
  ...
  private async readPrerenderManifest(manifestPath: string): Promise<PrerenderCacheManifest> {
  ...
  private async renderPrerenderVideoBlock(
    ...
    const tempDir = path.join(this.getProjectCacheDirectory(manifestPath), ".tmp", `prerender-video-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`);
    await fs.mkdir(tempDir, { recursive: true });
    try {
      const rendererResult = await this.renderSupervisedFrameRangeChild(
        project, manifestPath, scene, tempDir, frameRate, durationSeconds, frameRange.endFrame, tileHeight, "app-supervised", undefined, undefined, frameRange, "preview-cache",
      );
      ...
      await this.encodeFrameFilesToMp4(rendererResult.outputPath, tempVideoPath, frameRate, frameRange);
      await this.writePrerenderVideoBlock(cachePaths, tempVideoPath, cacheKey, frameRange, frameRate);
  ...
  private async encodeFrameFilesToMp4(inputFrameOutputPath: string, outputPath: string, frameRate: number, frameRange: ExportFrameRange, width: number = FRAME_WIDTH, height: number = FRAME_HEIGHT): Promise<void> {
    ...
    const ffmpeg = spawn(this.ffmpegPath, [
      ...
      "-profile:v", "baseline", "-level", "4.0", "-color_range", "pc",
      "-colorspace", "bt709", "-color_primaries", "bt709", "-color_trc", "bt709",
      "-g", String(frameRange.endFrame - frameRange.startFrame),
      "-keyint_min", String(frameRange.endFrame - frameRange.startFrame),
      "-sc_threshold", "0",
    ...
    let stderr = "";
    ffmpeg.stderr.on("data", (chunk: Buffer) => { stderr += chunk.toString("utf8"); if (stderr.length > 12000) stderr = stderr.slice(-12000); });
    const closePromise = new Promise<string | null>((resolve) => {
    ...
    try {
      for (let frameIndex = frameRange.startFrame; frameIndex < frameRange.endFrame; frameIndex += 1) {
        const frameBuffer = await fs.readFile(this.getSupervisedFrameOutputPath(inputFrameOutputPath, frameIndex));
  ...
  private async renderPrerenderBlock(
    ...
    const tempDir = path.join(this.getProjectCacheDirectory(manifestPath), ".tmp", `prerender-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`);
    await fs.mkdir(tempDir, { recursive: true });
    try {
      const rendererResult = await this.renderSupervisedFrameRangeChild(
        project, manifestPath, scene, tempDir, frameRate, durationSeconds, frameRange.endFrame, tileHeight, "app-supervised", undefined, undefined, frameRange, "preview-cache",
      ...
      for (let frameIndex = frameRange.startFrame; frameIndex < frameRange.endFrame; frameIndex += 1) {
        const frame = await fs.readFile(this.getSupervisedFrameOutputPath(rendererResult.outputPath, frameIndex));
